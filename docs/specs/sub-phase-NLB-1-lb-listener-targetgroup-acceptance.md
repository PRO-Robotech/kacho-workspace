...
| # | Фича | Traceability |
|---|---|---|
| F1 | id-prefix per-type `nlb-`/`lst-`/`tgr-` (hyphen B3); malformed/**wrong-type** → `INVALID_ARGUMENT "invalid <res> id '<X>'"` первым стейтментом; well-formed-но-нет → `NOT_FOUND`; foreign-id (`projectId`/`regionId`/`securityGroupId`/instance/nic) **НЕ** prefix-checked (B4 own-only). Реализация отступает от B4 для `v4Source`/`v6Source`: `subnetId`/`addressId` прогоняются через format-check **своего** vpc-prefix'а (`sub`/`adr`) — зафиксировано как расхождение, не как норма | module-nlb §Правила 12/16; unified §1 conv-12 **[PHASE-0-GATED B3]**, §8 B3/B4 |
...
**And** для VIP-источников LB (`v4Source`/`v6Source`) реализация отступает от B4: `subnetId`/`addressId` прогоняются через format-check **vpc-prefix'а** (`sub`/`adr` — различны) ДО peer-validate existence. Зафиксировано как расхождение с B4, решение — отдельно (см. F5)

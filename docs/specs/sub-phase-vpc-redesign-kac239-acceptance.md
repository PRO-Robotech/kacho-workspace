...
> **Статус:** SUPERSEDED — приёмка была ✅ APPROVED (round 2) и по ней сделаны S1–S3,
> но механизм S3 реализован не тем способом, который здесь одобрен (JSONB +
> `StaticRoute.id` + `xmin`-OCC вместо own-table — осознанное отклонение, решение
> заказчика 2026-05-30), а S4 в дереве продукта отсутствует. Разбор по стадиям со
> сверкой по дереву — [[KAC-239]] (сверено с `PRO-Robotech/kacho@96b2879a`,
> 2026-08-05). Новую работу этот документ **не открывает**: гейтом запрета #1 для
> оставшегося объёма он служить не может, потому что описывает механизм, от
> которого отказались. Строка состояния добавлена 2026-08-05: до неё у документа
> не было объявления вердикта ни в одной форме, и его нельзя было прочитать
> машинно (`scripts/docs-gate/check-01-acceptance-verdict.py`).

...
### 0.5.2 KAC-196 — Cluster RBAC admin UI — Status: To do (acceptance-author работает)
...
| FQN | Cat | Permission | Required relation | Required subject_type | Object type | from_request_field |
|---|---|---|---|---|---|---|
| `kacho.cloud.vpc.v1.InternalAddressPoolService/Create` | A | `vpc.address_pools.create` | `system_admin` | `user` | `cluster` | `*` |
| `kacho.cloud.vpc.v1.InternalAddressPoolService/List` | A | `vpc.address_pools.list` | `system_admin` | `user` | `cluster` | `*` |
| `kacho.cloud.vpc.v1.InternalAddressPoolService/BindAsNetworkDefault` | A | `vpc.address_pools.bind_as_network_default` | `system_admin` | `user` | `cluster` | `*` |
| `kacho.cloud.vpc.v1.InternalAddressService/AllocateExternalIP` | B | `vpc.addresses.allocate_external` | `service_account` | `service_account` | `cluster` | `*` |
| `kacho.cloud.vpc.v1.InternalAddressService/AllocateInternalIP` | B | `vpc.addresses.allocate_internal` | `service_account` | `service_account` | `cluster` | `*` |
| `kacho.cloud.iam.v1.InternalIAMService/Check` | B | `iam.authz.check` | `service_account` | `service_account` | `cluster` | `*` |
| `kacho.cloud.iam.v1.InternalIAMService/LookupSubject` | B | `iam.subjects.lookup` | `service_account` | `service_account` | `cluster` | `*` |
| `kacho.cloud.iam.v1.InternalIAMService/WriteCreatorTuple` | B | `iam.tuples.write_creator` | `service_account` | `service_account` | `cluster` | `*` |
| `kacho.cloud.iam.v1.InternalUserService/UpsertFromIdentity` | B | `iam.users.upsert_from_identity` | `service_account` | `service_account` | `cluster` | `*` |
| `kacho.cloud.iam.v1.InternalUserService/Get` | C | `iam.users.read_internal` | `viewer` | `user` | `cluster` | `*` |
| `kacho.cloud.compute.v1.InternalZoneService/Create` | A | `compute.zones.create` | `system_admin` | `user` | `cluster` | `*` |
| `kacho.cloud.compute.v1.InternalWatchService/Watch` | B | `compute.resources.watch` | `service_account` | `service_account` | `cluster` | `*` |
| `kacho.cloud.vpc.v1.InternalResourceLifecycleService/Subscribe` | B | `vpc.resources.subscribe_lifecycle` | `service_account` | `service_account` | `cluster` | `*` |
| ... (full 44 in `permission_options.proto` annotations) | | | | | | |
...
- **OQ-4**: Do we add `RequiresMFAFresh: true` for Cat-A CRITICAL via 5.0, or defer step-up gating to a follow-up? — **Recommendation: add now.** CatalogEntry already carries `RequiresMFAFresh bool` (per `permission_catalog.go:75-77`); step-up-gate middleware (`stepup_gate.go`) already enforces. Zero-cost in 5.0; ROI high (prevents persistent-stolen-session admin abuse).
...
| **KAC-178 §3 (alias-relations admin/editor/viewer cascade) задерживается → catalog `required_relation: viewer` для Cat-C не работает каскадно** | Low | Low | Cat-C методов всего 1 (`InternalUserService.Get`) — можно временно перевести его на прямую relation `system_admin` (Cat-A-like) в catalog v1, потом откатить на `viewer` после KAC-178 §3 merge. Документировано как TODO-в-KAC-201.5 PR-описании, **не** TODO в коде (per Запрет #11). |
...
| **0b** | KAC-196 | `InternalClusterService.GrantAdmin/RevokeAdmin/ListAdmins` RPC + `cluster_admin_grant` DB-table + UI `/system/cluster` | `kacho-proto`, `kacho-iam`, `kacho-api-gateway`, `kacho-ui` | KAC-196 epic | To do (acceptance-author) |

...
- [ ] **GREEN:** T-I1..T-I5 + T-E1..T-E4 зелёные; `go test ./... -race`, `golangci-lint run`, `govulncheck` зелёные (цели `audit-list-filter` у iam нет — гейт живёт в
      `services/{compute,nlb,storage,vpc}`); newman зелёные (после RC-4 re-bootstrap на стенде).

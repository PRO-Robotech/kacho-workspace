...
| **D-R3** | **#177 guard сохраняется.** RC-1 фикс НЕ эмитит `sg_<objType>` carrier-tuple и НЕ эмитит bare-anchor tier через cascade на verb-bearing child-типы. Concrete tier-tuple на anchor-объекте — bounded к одному объекту; он НЕ открывает auto-cascade на sibling vpc/compute-ресурсы под тем же scope. (То же свойство, что у существующего `emitNamesRule` concrete-tier-tuple.) | T-I1, T-I4 |
...
**Given** под project `prj_P` есть sibling-ресурс `vpc_network:net_N` (`vpc_network:net_N#project@project:prj_P`-hierarchy уже существует)
**And** субъекту `user:usr_U` выдан anchor-grant `viewer` на `project:prj_P` (через T-I2-путь)

**When** проверяется auto-видимость sibling-ресурса

**Then** `Check(user:usr_U, viewer, vpc_network:net_N)` **= False** — concrete project-tier-tuple НЕ открывает auto-cascade на verb-bearing child vpc/compute-ресурсы (#177 over-cascade НЕ переоткрыт)
**And** `ListObjects(user:usr_U, "viewer", "vpc_network")` **НЕ содержит** `net_N` — read==enforce путь: consumer (`VpcNetworkService.List` через listauthz) использует `ListObjects`, не только `Check`; обе грани (`Check` И `ListObjects`) подтверждают, что project-tier-grant не cascade-ит на sibling verb-bearing тип (усиление #177 over-cascade guard)

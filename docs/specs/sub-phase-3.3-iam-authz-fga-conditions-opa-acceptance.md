...
- **Principal** (Phase 2 contract) — extracted из DPoP-bound JWT в api-gateway,
  ...
  `cluster_admin` (computed flag — true if FGA имеет tuple `cluster_kacho_root#system_admin@<subject>`).
...
- **OPA sidecar** — `openpolicyagent/opa:1.0.x` container, injected per pod (api-gateway +
  ...
  `https://kacho-iam-internal:9091/opa/v1/bundle.tar.gz` каждые 1h, verify signature
...
[OPA sidecars (all pods) poll каждые 1h]
        ▲
        │ GET https://kacho-iam-internal:9091/opa/v1/bundle.tar.gz
...
- New proto `kacho-proto/proto/kacho/cloud/iam/v1/opa_bundle.proto` (Internal service):
  ...
  message BundleResponse {
    bytes bundle_tar_gz = 1;   // tar.gz binary
    bytes signature_jws = 2;   // detached JWS (ES256)
    string revision = 3;       // git-sha
  }
...
- `opa-sidecar-configmap.yaml` — OPA config:
  ...
  services:
    kacho-iam:
      url: https://kacho-iam-internal.kacho-system.svc.cluster.local:9091/opa/v1
...
## 6. GWT-сценарии (50+ scenarios, ≥40 mandated)
...
### 6.4 Conditions — mfa_fresh (4 scenarios — D-5)
...
**And** OPA sidecar configured with bundle pull `https://kacho-iam-internal:9091/opa/v1/bundle.tar.gz`, signing keyid `key_2026_05` (JWK loaded from ConfigMap).

**When** OPA sidecar starts.

**Then** OPA HTTP GET to bundle endpoint succeeds (200).
**And** Response Content-Type=`application/gzip`, body=tar.gz.
**And** Detached signature `bundle.tar.gz.sig` (response header `X-Opa-Bundle-Signature` или second-request — implementation-detail; either works).
**And** OPA verifies ES256 signature against JWK `key_2026_05` → success.
...
**When** Client calls `GET https://kacho-iam-internal:9091/opa/v1/bundle/revision`.
...
| Adversarial bundle injection (MitM between kacho-iam and OPA sidecar) | High | TLS-edge enforced (cluster-internal Service → kacho-iam pod with self-signed certs verified by sidecar-side custom CA bundle); detached JWS signature catches tamper after-the-fact even if MitM succeeds. SPIRE mTLS (Phase 10) closes this further |

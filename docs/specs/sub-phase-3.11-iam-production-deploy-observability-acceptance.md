...
- **Deployment** на baseline-уровне: `kacho-deploy` имеет Helm umbrella chart для одного env'a; `make -C deploy dev-up` поднимает локально через docker-compose; Argo CD GitOps **не настроен**; cutover вручную через `helm upgrade`; rollback — manual `helm rollback`.

...
|---|---|---|
| F1 | id-prefix per-type `nlb-`/`lst-`/`tgr-` (hyphen B3); malformed/**wrong-type** → `INVALID_ARGUMENT` первым стейтментом; well-formed-absent → `NOT_FOUND`; foreign-id (projectId/regionId/securityGroupId/instance/nic) **НЕ** prefix-checked (B4). Исключение по факту реализации: `v4Source`/`v6Source` разбирают `subnetId`/`addressId` через format-check **своего vpc-prefix'а** (`sub`/`adr`) — см. §Расхождение с B4 | NLB-1-01..05 |
...
**And** для VIP-источников LB (`v4Source`/`v6Source`) реализация отступает от B4: `subnetId`/`addressId` прогоняются через format-check **vpc-prefix'а** (`sub`/`adr` — они различны, не общий prefix) ДО peer-validate. Это расхождение с B4 зафиксировано как таковое, а не как норма — решение по нему принимается отдельно; existence в любом случае подтверждается peer-vpc (см. F5)

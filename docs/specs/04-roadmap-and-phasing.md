...
### 2.7. Change Graph — рамка этого lifecycle с момента cutover

Шаги 2.1–2.6 остаются, но перестают быть **всем**, что связывает работу. С момента cutover
они становятся частью **Change Graph** — vendor-neutral SDD-контура, который связывает
observable behavior, technical design, execution route, evidence, actual diff и landed content
машинно проверяемыми связями. До сих пор эти шесть предметов связывало внимание: приёмка
объявляла контракт, тесты писались по ней, вердикт роли подтверждался словом в документе, а
посадка сверялась с намерением по памяти. Контур заменяет каждое «по памяти» проверяемой
связью.

**У контура ДВА владельца, и эта глава — ни один из них.**
...
предмете расходятся молча, и расходится то, которое читают чаще.

**Контур обязателен не с момента написания приёмки, а с cutover.** Cutover создаётся
versioned `docs/changes/policy.yaml` при landing SDD-1 и объявляется отдельно для каждого из
двух Git DAG. До него действуют шаги 2.1–2.6 в прежнем виде; сам SDD-1 — единственный
bootstrap, он идёт legacy-процессом и своего package не заводит. «До» и «после» решается
предком коммита, а не именем ветки, не датой и не стартовым коммитом.

**Что контур добавляет к шагам §2** — пять следствий; условия переходов и координаты
принадлежат владельцам из таблицы выше:

| шаг | что добавляет контур |
|---|---|
| **2.1** приёмка | она перестаёт быть единственным спутником задачи: изменение описывается package'ем, где технические решения, маршрут работ, вердикты и свидетельства — раздельные владельцы истины, и пересказ владельца в соседнем файле вторым источником не является |
| **2.2** вердикт | одобрение привязано к **отпечатку содержимого** и к внешнему проверенному событию; правка одобренного документа не переносит вердикт на новую редакцию, а самодекларация роли полномочия не даёт |
| **2.3** RED | доказанный красный становится **условием перехода** к реализации, а не отчётной фразой в PR; у pre-RED появляется единственный владелец, и «не выполнилось» реализации не открывает |
| **2.5** review ролями | вердикт специалиста перестаёт быть строкой в отчёте: у каждой **применимой** роли своя durable-запись, привязанная к отпечатку фактического диффа и подтверждённая внешним проверенным событием; записи ролей не перезаписывают друг друга и агрегируются точным множеством, а «роль неприменима» законно только по **зарегистрированному** предикату применимости — свободным текстом нет |
| **2.6** верификация и trail | посадка сверяет **применённое содержимое**, а не тождество коммита: схлопывание и перенос допустимы, пока содержимое то же, а расхождение обесценивает схождение |

Шаг 2.4 контур не меняет — его предмет внутри изменения, а не на его границах.

> [!note] Здесь шаг 2.5 стоял среди неизменённых наравне с 2.4 — и это было неверно
> Шаг 2.5 перечисляет профильные роли поимённо, и ровно эти роли `docs/changes/policy.yaml`
> объявляет владельцами полномочия на вердикт после диффа — то есть предмет шага стоит
> **на границе** изменения, а не внутри него. Утверждение противоречило и соседней строке
> этой же таблицы: обязательство «вердикт привязан к отпечатку и внешнему событию,
> самодекларация роли полномочия не даёт» объявлено для 2.2 и ложится на 2.5 тем же
> основанием. Оговорка оставлена, а не удалена, потому что класс здесь общий: раздел
> объявлен нормативным consumer-lifecycle (врезка ниже), поэтому его умолчание читается
> как разрешение — и читается чаще, чем текст владельца.

> [!important] Раздел обязателен и обновляем — и вторым acceptance не становится
> §1 приёмки объявляет эту главу нормативным consumer-lifecycle, и отсюда два следствия.
> Первое: меняется контур — правится этот раздел. Второе: при расхождении верна **приёмка**,
> а этот раздел — находка; спор решается её текстом, а не старшинством главы. Наступил ли
> cutover, читается из записанных landing-reviewer'ом commits в `docs/changes/policy.yaml`,
> а не из даты этой правки.
...
и инвариантам — корпус `.claude/rules/*` (`ls .claude/rules/*.md | wc -l` → 30) плюс
per-repo `docs/architecture/`; эта глава на них ссылается, а не дублирует. Сам корпус
никому не грузится: решением владельца 2026-09-17 каталог снят с автозагрузки, главный
поток — агент `dispatcher`, а правило попадает в окно закреплением за агентом
(`.claude/rules/MANIFEST.md`, колонка «закреплено за агентами» → frontmatter `skills:`).
Устройство описано в §8 главы 03 и в корневом `CLAUDE.md`; здесь оно не пересказывается.

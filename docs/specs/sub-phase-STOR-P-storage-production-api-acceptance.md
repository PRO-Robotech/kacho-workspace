...
### STOR-P-56а (positive) `e2e` — копия НАЗЫВАЕТ СВОЕГО РОДИТЕЛЯ

**Given** готовый ресурс и второе место размещения (регион у образа, зона у снимка)

**When** `:copy`
**Then** у копии заполнено происхождение-копирования — `sourceImageId` у образа,
`sourceSnapshotId` у снимка, равное идентификатору источника
**And** происхождение **ровно одно**: поля снятия (`sourceVolumeId`, у образа ещё
`sourceSnapshotId`) у копии пусты
**And** источник не изменён

**Почему сценарий появился отдельно от STOR-P-56/41.** Оба глагола `:copy` не
работали **ни разу** с момента заведения, и причина у них общая: вставка копии с
самого начала записывала родителя (`source_snapshot_id` / `source_image_id`), но
этот вид происхождения не признавали ни домен, ни контракт — проверка требовала
источник **снятия**, которого у копии нет, и отвергала её как ресурс без источника.
Наружу столбец не выходил вовсе: проекция чтения его не выбирала.

Три поверхности при этом исходили из того, что поле есть: провайдер Terraform
объявляет оба источника копирования неизменяемыми полями (у скопированных ресурсов
план расходился бы вечно), сквозной кейс о нём утверждал, столбец жил в базе. Не
было его ровно в контракте, домене и пути чтения — то есть ровно там, где сценарий
STOR-P-56 не задавал вопроса.

**Исполнитель:** newman `image.py` :: **`IMG-COPY-CRUD-OK`** и `snapshot.py` ::
**`SNP-COPY-CRUD-OK`** (утверждения о родителе и о единственности происхождения) +
`internal/domain/snapshot_copy_test.go` :: **`TestSnapshotCopyNamesItsParent`** и
**`TestImageCopyNamesItsParent`** — перечисляют все пары видов происхождения и
падают на возврате прежнего обхода (наследование источника у оригинала); проверено
инъекцией в обе стороны.

**Границы.** `sourceImageId` **не принимается на вход**: в `CreateImageRequest` его
нет, поэтому «только на чтение» держится построением, а не уговором. Правило
STOR-1-24 (`Create` — снимок XOR том, ровно одно) этим **не затронуто**: третий вид
происхождения появляется только у копии.

...
**Сверено с деревом:** монорепо **`db7146fb`** (13.08.2026) — под каждым сценарием назван
**исполнитель поимённо** либо сказано, что его нет.
...
> файлам на диске, и все числа ниже относятся к `db7146fb`.
>
> Практическое следствие: если newman-кейс на `:changeDiskType` или `:copy` появится в
> ближайшем коммите, соответствующая строка переписи станет устаревшей — и это нормально,
> потому что у неё **назван предикат**, которым её пересчитывают.
...
| Корзина | Сценариев | Что это значит |
|---|---:|---|
| **Исполнитель есть** | **42** | названная проба/гейт исполняет то, что сценарий утверждает |
| **Частично** | **19** | проба есть, но покрывает часть сценария — под каждым сказано, какую именно и чего не хватает |
| **Исполнителя нет** | **14** | не исполняет ничто; причина названа |
| Всего | **75** | |

**Из 14 непокрытых** пять — про глаголы и полосы, доставленные без единой пробы
(STOR-P-41 `Snapshot.Copy` · 47 `ChangeDiskType` · 56 `Image.Copy` · 64 квота · 51
множественная привязка); четыре — про свойства, которых в дереве нет вовсе (STOR-P-16
префиксы · 45 предикат состояния при ресайзе · 66 потребление проекта · 9 границы размера
на `Create`); три — про пробы, которые не написаны при живом свойстве (STOR-P-02 · 38 · 39);
два — про отсутствующие гейты (STOR-P-27 · 62).

**Перепись сделана по дереву, а не по памяти.** Предикат для каждой строки — имя теста в
`grep -rn '^func Test' --include=*_test.go` (442 функции в `services/storage`, из них 133
добавлены веткой) либо доказанное отсутствие: для непокрытых сценариев в теле сказано, какой
именно поиск дал ноль. Поэтому «ноль находок» здесь отличимо от «ноль прочитанного».
...
**Исполнителя НЕТ.** Кейса нет: пути `/storage/v1/snapshots/{id}/operations` в case-файлах storage не встречается ни разу (у образа и тома такие кейсы есть). RPC и запись каталога прав при этом заведены.
...
**Исполнителя НЕТ.** `Snapshot.Copy` провязан от контракта до репозитория и **не вызывается ни одной пробой** дерева. Нарушение ban #12; см. план §3.1.
...
**Исполнитель (частично):** Вторую половину («запрос с `blockSize` отвергается») держит `internal/apps/kacho/api/volume/volume_test.go` :: **`TestUpdateImmutableField`** — `block_size` в маске даёт `INVALID_ARGUMENT` с конвенционным текстом. Первой половины («поля нет в сообщении, номер и имя зарезервированы») не держит ни один гейт: `reserved 11; reserved "block_size"` в `volume.proto` стои́т, но проверяется только чтением.
...
**Исполнителя НЕТ.** `ChangeDiskType` не вызывается ни одной пробой дерева. Нарушение ban #12; см. план §3.1.
...
**Исполнителя НЕТ.** `Image.Copy` не вызывается ни одной пробой дерева. Нарушение ban #12; см. план §3.1.

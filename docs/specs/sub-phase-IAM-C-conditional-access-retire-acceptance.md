...
>
> **Отдельно, `grpc_register_test.go`:** этого файла не существовало НИКОГДА — ни под прежним
> именем каталога, ни под нынешним (`git log --all -- <путь>` даёт 0 коммитов на обоих). В
> таблице он помечен `**[новый]**` и является **планируемым** артефактом сценария `IAM-C-13`,
> а не утверждением о дереве. Перепись мёртвых координат, считающая его наравне с прочими,
> считает не то.
>
...
> `helm lint`, два `helm template`, `.github/scripts/check-volume-mounts.sh` и цель
> полноты списка mTLS-off каталога `deploy` (снята вместе с двухфазным подъёмом —
> цитировать её командой больше нельзя, предмета нет) — и **ничего** из `deploy/tests/helm/`. То есть гейт
...
> подключённых там `.github/scripts/check-volume-mounts.sh` и стражей рендера
> сервисных чартов (`make -C services/vpc helm-render-guard`; цель полноты списка
> mTLS-off, стоявшая здесь образцом, снята вместе с двухфазным подъёмом)
...
|---|---|---|---|
| **O-1** | **`deploy/tests/helm/config-rollout-binding-test.sh` не вызывается ни из CI, ни из Makefile** — единственное упоминание вне скрипта — комментарий `deploy/scripts/assert-production-posture.sh:79`; job `helm lint · template (dev + prod)` (`ci.yaml:273`) исполняет lint, два `helm template`, `check-volume-mounts.sh` и цель полноты списка mTLS-off каталога `deploy` (с тех пор снята вместе с двухфазным подъёмом — командой не цитируется, предмета нет), и ничего из `deploy/tests/helm/`. Документ до круга 4 называл его «живым CI-гейтом» (§1.3 F). Тот же класс покрывает и остальные **11** скриптов этого каталога | **#81** (третий пункт: «команда гейта не исполняется из корня и не вызывается в CI») | Скрипт вызывается шагом CI и его вывод **виден в логе прогона**; инъекция `envFrom` без `checksum`-аннотации делает прогон красным, откат инъекции — зелёным |
...
| **O-3** | **Гейт `listauthz` исполняется для 4 сервисов из 7 — и на остальных трёх его отсутствие неотличимо от прохождения.** Цель гоняется поимённым перечнем сервисов в CI; сервис, которого в перечне нет, не «падает» и не «пропускается с предупреждением» — он просто не упоминается, и сборка зелёная. Класс: **перечень потребителей, ведомый вручную, отстаёт от дерева, а его неполнота выглядит как успех** (то же, что «починка распространяется ровно настолько, насколько хватает её гейта»). Отягчающее: документ опирается на этот гейт как на **действующий** контроль (§2 «Не снимается ничего из действующих контролей»), то есть решение об объёме правки принято по свойству, которого для части сервисов нет — а «ноль находок» здесь означало «ноль осмотренного» | **#75** (та же формулировка «во всех семи сервисах») | Перечень сервисов **выводится из дерева**, а не пишется руками; `audit-list-filter` (или эквивалент) исполняется для **7 из 7**, и **отсутствие цели у сервиса — красный, а не пропуск**. Гейт обязан утверждать **объём осмотренного** (сколько сервисов перечислено), иначе он унаследует ровно тот дефект, который закрывает |

...
> `helm lint`, два `helm template`, `.github/scripts/check-volume-mounts.sh` и цель полноты
> списка подчартов первой фазы установки (снята из `deploy/Makefile` вместе с самой фазой —
> здесь она не цитируется командой, потому что больше не существует) — и **ничего** из
> `deploy/tests/helm/`. То есть гейт
...
> подключённых там `.github/scripts/check-volume-mounts.sh` и цели полноты списка подчартов
> первой фазы (последняя с тех пор снята из `deploy/Makefile` вместе с фазой)
...
|---|---|---|---|
| **O-1** | **`deploy/tests/helm/config-rollout-binding-test.sh` не вызывается ни из CI, ни из Makefile** — единственное упоминание вне скрипта — комментарий `deploy/scripts/assert-production-posture.sh:79`; job `helm lint · template (dev + prod)` (`ci.yaml:273`) исполняет lint, два `helm template`, `check-volume-mounts.sh` и цель полноты списка подчартов первой фазы (с тех пор снята из `deploy/Makefile` вместе с фазой), и ничего из `deploy/tests/helm/`. Документ до круга 4 называл его «живым CI-гейтом» (§1.3 F). Тот же класс покрывает и остальные **11** скриптов этого каталога | **#81** (третий пункт: «команда гейта не исполняется из корня и не вызывается в CI») | Скрипт вызывается шагом CI и его вывод **виден в логе прогона**; инъекция `envFrom` без `checksum`-аннотации делает прогон красным, откат инъекции — зелёным |

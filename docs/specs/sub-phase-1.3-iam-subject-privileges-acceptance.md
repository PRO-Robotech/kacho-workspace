...
> **Эпик/тикет:** KAC-TBD (фича «IAM subject privileges visibility + grant»; затронутые репо — `kacho-proto` / `kacho-iam` / `kacho-api-gateway` / `kacho-ui` / `kacho-deploy` / `kacho-workspace`(docs). **Номер тикета проставляется до старта `superpowers:writing-plans`** — фича → тикет СНАЧАЛА, `git-issues.md`; в финальном APPROVED-артефакте `KAC-TBD` не остаётся.)

...
> **Эпик/тикет:** epic «Resource-scoped AccessBinding» (вариант C — selector-based grants), под-фаза **α**. **Номер KAC проставляется ДО старта `superpowers:writing-plans`** (фича → эпик СНАЧАЛА, `git-issues.md`: `[EPIC]` + Subtask α/β/γ). Затронутые репо под-фазы α: `kacho-proto` / `kacho-iam` / `kacho-api-gateway` / `kacho-ui` / `kacho-deploy` / `kacho-workspace`(docs/vault).

...
**D-16. List-filter (listauthz) сохраняет инвариант** `make -C services/{compute,nlb,storage,vpc} audit-list-filter`: публичный `List<Resource>` фильтрует по материализованным per-object tuple владельца. После плоской модели фильтр читает прямые tuple — CI-гейт остаётся.
...
| **P7** | iam: list-filter (listauthz) на плоской модели | `services/iam` | цель `audit-list-filter` в iam **не объявлена** — фильтрация в iam есть (`internal/authzfilter`), гейта нет; зелёный по прямым tuple доказывают go-тесты фильтра (D-16) | P5 |
...
4. Заказчик — финальный smoke/e2e (`make -C deploy e2e-test` / `grpcurl`).

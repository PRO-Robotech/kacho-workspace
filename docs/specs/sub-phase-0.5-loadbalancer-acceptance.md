...
- Listener uniqueness enforcement (уникальность порта/протокола внутри одного NLB) — зарезервировано для уточнения (OQ-1).
- TargetGroup с targets cross-subnet (несколько subnetId внутри одной TargetGroup) — разрешено (см. OQ-2).
- NLB IP-адреса (`status.externalIps`) — control plane только регистрирует список, не выделяет (OQ-3).
...
- `NetworkLoadBalancer.Status` с полями:
  ...
  - `external_ips[]` (strings, может быть пустым)
  - `conditions[]`
...
**And** `metadata.resource_version` не изменился (no-op, нет diff)
...
**And** в течение 60 секунд compute-reconciler выполняет следующую последовательность:
  1. Обнаруживает `metadata.finalizers[]` содержит `"loadbalancer.kacho.io/target-deregister"`
  2. Вызывает `LoadBalancerInternal/RemoveTarget` с `instance_id = <instance-uid>`
  3. Получает OK от loadbalancer
  4. Удаляет `"loadbalancer.kacho.io/target-deregister"` из `metadata.finalizers[]` через `ComputeInternal/UpdateInstanceMetadata`
  5. Продолжает обработку `"compute.kacho.io/disk-detach"` (отвязывает диски)
  6. Удаляет `"compute.kacho.io/disk-detach"` из finalizers
...
**Then** Reconciler обнаруживает NLB с `status.state = "UPDATING"` и `metadata.generation > status.observed_generation` (или иным сигналом о pending update)
...
1. **Все сценарии §1–§13** (A1–A5, B1–B7, C1–C6, D1–D5, E1–E3, F1–F4, G1–G4, H1–H8, I1–I4, J1–J6, K1–K6, L1–L12, M1–M6) покрыты исполняемыми тестами:
...
**OQ-1. Уникальность listener по порту/протоколу внутри одного NLB**

Допустимо ли иметь два listener-а с одинаковым `port + protocol` внутри одного NLB? Варианты:
- (A) Запрещено: INVALID_ARGUMENT при Upsert если порт+протокол повторяются — более строго, соответствует реальному поведению L4-балансировщика.
- (B) Разрешено: backend сам разрулит конфликт — упрощает валидацию, но допускает семантически некорректные конфигурации.

*Предлагается:* (A) — добавить валидацию уникальности `name` listener-а внутри одного NLB; уникальность `port + protocol` — опционально.

**OQ-2. TargetGroup с targets, принадлежащими разным subnet**

Разрешено ли иметь несколько targets с разными `subnet_id` внутри одного TargetGroup? В реальном L4 NLB все targets обычно из одной зоны.

*Предлагается:* разрешить (нет ограничений в control plane) — data plane ограничения за пределами scope текущей фазы.

**OQ-3. NLB `status.external_ips` — откуда берётся IP?**

В control plane нет data plane, поэтому `status.external_ips` всегда пустой или симулируется. Варианты:
- (A) Всегда пустой `[]` — честно отражает отсутствие data plane.
- (B) Reconciler присваивает симулированный IP (например, `10.255.X.X` из pseudo-range) при переходе в ACTIVE — полезно для e2e тестирования.

*Предлагается:* (B) — reconciler выставляет один симулированный IP при переходе CREATING → ACTIVE.

**OQ-4. Порядок снятия finalizer-ов при удалении Instance**

При наличии обоих finalizer-ов (`compute.kacho.io/disk-detach` и `loadbalancer.kacho.io/target-deregister`) — должен ли быть гарантированный порядок? Варианты:
- (A) Параллельно: оба finalizer обрабатываются одновременно в разных горутинах reconciler-а.
- (B) Последовательно: сначала `loadbalancer.kacho.io/target-deregister`, затем `compute.kacho.io/disk-detach` (LoadBalancer → затем Disk).

*Предлагается:* (B) — последовательно, loadbalancer finalizer первым (безопаснее: сначала убираем трафик, потом диски), но порядок несущественен для control plane-only системы. Достаточно параллельного подхода (A) для упрощения реализации.

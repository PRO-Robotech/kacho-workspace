...
> **Эпик/тикет:** epic «Resource-scoped AccessBinding» (вариант C — selector-based grants), под-фаза **β**. Номер KAC проставляется ДО старта `superpowers:writing-plans` (`[EPIC]` + Subtask α/β/γ, `git-issues.md`). Затронутые репо под-фазы β: `kacho-proto` / `kacho-compute` / `kacho-iam` / `kacho-deploy` / `kacho-workspace`(docs/vault). vpc — **отдельная волна β2** (D-β8, обоснование ниже).

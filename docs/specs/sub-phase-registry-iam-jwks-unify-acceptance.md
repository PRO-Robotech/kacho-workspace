...
> [!important] ЧЕТЫРЕ из шести не-целей ниже ОТМЕНЕНЫ фазой F1 (задача продукта #897)
> Пункты этого раздела — стоячие утверждения о продукте, а не только об объёме той фазы,
> и потому они читаются как действующие ограничения до тех пор, пока не надписаны. Фаза
> F1 «своя чеканка токенов» отменяет четыре из них; надпись стоит у каждого пункта, потому
> что пункт без пометки продолжает утверждать обратное тому, что сделано.
> Приёмка F1 — `docs/specs/sub-phase-F1-token-signer-acceptance.md`, §1.10.

- **Hydra остаётся подписантом/эмитентом.** iam **не** пере-чеканит registry-токен. `iss`
  токена остаётся Hydra-issuer'ом.
  — **ОТМЕНЕНО F1:** платформа чеканит свои токены сама, и контур выдачи докер-токена
  переведён на свой подписант целиком; издатель в токене этого контура — наш.
- **Не воскрешаем vestige.** `oidc_jwks_keys` / jwks-rotator / `HydraPublisher` остаются
  ...
  — **ОТМЕНЕНО F1:** ключница и публикатор своих ключей заведены заново. Имя хранилища при
  этом ДРУГОЕ: прежнее стоит записью в ведомости дропов, и совпадение сделало бы стража
  ведомости красным справедливо.
- **Никакой новой криптографии / нового verifier'а** — `jwks/verifier.go` origin-agnostic и
  не меняется.
  — **ОТМЕНЕНО F1, и шире прочих:** проверяющий меняется по каждой оси сразу — издатель
  становится множеством с привязкой «издатель → источник набора»; читатель отзыва встаёт на
  путь запроса; срок и тип становятся обязательными явно; появляются допуск на расхождение
  часов, потолок тела и вынужденный перезапрос набора.
- **Никакого нового proto / нового gRPC-RPC / новой схемы БД.** Это HTTP-листенер + config, не
  ...
  — **НЕ ОТНОСИТСЯ:** пункт про объём ТОЙ фазы, а не про дерево. F1 заводит свои таблицы и
  свою приёмку, и ревью схемы для неё обязательно.
- **JWKS-route НЕ регистрируется** ни в одном external `*Addr`-блоке
  ...
  — **СОХРАНЯЕТСЯ F1:** маршрутов публикации стало больше одного, и утверждение относится к
  КАЖДОМУ. Замок «только внутри» проверяется по перечню, выведенному из привязки, а не по
  выписанному списку: выписанный разошёлся бы молча при появлении третьей записи.
- **Не меняем docker access-control flow** (`/iam/token`-шим, SA-key → token). Меняется только
  **источник ключей верификации**.
  — **ОТМЕНЕНО F1:** именно этот контур и переводится. Предъявление учётных данных не
  меняется; меняется то, КТО чеканит выдаваемый по ним токен.

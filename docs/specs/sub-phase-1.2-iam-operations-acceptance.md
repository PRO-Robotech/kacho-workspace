...
> **Эпик/тикет:** KAC-TBD (epic «IAM operations visibility»; per-repo subtask в `kacho-proto` / **`kacho-corelib`** / `kacho-iam` / `kacho-api-gateway` / `kacho-ui` / `kacho-deploy` — corelib попал из-за blast-radius `account_id`-колонки в общей `operations`-схеме, D-8). **Номер тикета проставляется до старта `writing-plans`** (фича → тикет СНАЧАЛА, `git-issues.md`); в финальном APPROVED-артефакте `KAC-TBD` не остаётся.
...
**S6 — deploy (`kacho-deploy`):** helm/compose без новых сервисов; seed для smoke; `make -C deploy dev-up` + reload-svc iam/api-gateway; smoke §J через port-forward.
...
**Заказчик** — финальный smoke/e2e (шаг 7): открыть деталь User/AccessBinding → таб «Операции» показывает список; открыть `/iam/operations` → account-scoped операции; `grpcurl`/`make -C deploy e2e-test` на новые REST-пути.

...
> **Эпик/тикет:** KAC-`<N>` `[EPIC]` «RBAC rules-model 2026» (номер проставляется до старта `superpowers:writing-plans`; фича 6 репо → эпик + Subtask per репо, `git-issues.md`). Затронутые репо: `kacho-proto` / `kacho-corelib`(если общее) / `kacho-iam` / `kacho-api-gateway` / `kacho-ui` / `kacho-deploy` / `kacho-workspace`(docs).
...
- [ ] заказчик — финальный smoke/e2e (`make -C deploy e2e-test` / `grpcurl`): создать роль с mixed-арм rules, bind через `subjects[]`, проверить per-verb Check, per-object List, ExpandAccess; убедиться, что target-мутаторы и поля target/selector отсутствуют в контракте.

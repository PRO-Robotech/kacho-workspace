...
**Then** в `cloud-demo/go.work` присутствует `use` для всех Go-репо: `kacho-proto`, `kacho-corelib`, `kacho-api-gateway`, `kacho-resource-manager`, `kacho-vpc`, `kacho-compute`, `kacho-loadbalancer`
...
**Given** сгенерированы Go-стабы из `kacho-proto/proto/`
...
**When** разработчик внутри любого из дочерних репо (`kacho-proto/`, `kacho-corelib/`, и т.д.) выполняет `git status`
...
**Then** для `kacho-proto`: jobs `lint` (buf lint), `breaking` (buf breaking against baseline tag), `generate` (buf generate, проверка отсутствия диффа в `gen/`) — зелёные.
...
**And** integration-тесты для 0.1 будут написаны в репо `kacho-corelib`, `kacho-proto` и `kacho-deploy/e2e/0.1/`

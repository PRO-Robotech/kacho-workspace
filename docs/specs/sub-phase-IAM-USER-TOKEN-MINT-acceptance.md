...
- [ ] `kacho-api-gateway`: регистрация `InternalUserTokenService` **только** на internal sub-mux; route-table + permission-catalog regen (обе embedded-копии byte-identical, `make -C gateway permission-catalog-check`); суффикс в `HasInternalSuffix` (404 на external).

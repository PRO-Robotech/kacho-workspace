...
| `kacho-corelib`      | **B2 / B3**: новая общая миграция `migrations/common/0003_operations_principal_filter_idx.sql` (partial index `operations(principal_id) WHERE principal_id <> ''`) — нужна в _каждом_ сервисе, потому что UI делает **per-service** filter call (`/iam/v1/operations?principal.id=$me` + `/vpc/v1/operations?...` + `/compute/...` + `/loadbalancer/...` + `/rm/...`). Дренится в каждый сервис через шаг синхронизации миграций (снятая цель sync-migrations, services/{compute,nlb,vpc}) (см. §4.2.1). Integration-тест для filter — отдельно в каждом сервисе (B3) |
...
| `kacho-test`         | Playwright e2e tests (D-12): `e2e/signup.spec.ts`, `e2e/iam-crud.spec.ts`, `e2e/permission-reactivity.spec.ts`; npm scripts (цели `e2e-ui` нет; Playwright-набор в монорепо отсутствует) через docker-compose с headless Chromium; CI integration в `kacho-test/.github/workflows/e2e-ui.yaml` |
...
2. В **каждом** из 5 сервисов (`kacho-iam`, `kacho-vpc`, `kacho-compute`, `kacho-loadbalancer`, `kacho-resource-manager`):
   шаг синхронизации миграций (снятая цель sync-migrations, services/{compute,nlb,vpc}) (копирует common-migrations в `migrations/`); commit; deploy.
   **В монорепо шаг мёртв:** corelib переехал в `pkg/`, рецепты смотрят в исчезнувший
   `../kacho-corelib/migrations/common`; у vpc и iam цель объявлена как no-op.

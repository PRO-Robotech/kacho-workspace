...
| `kacho-corelib`      | **B2 / B3**: новая общая миграция `migrations/common/0003_operations_principal_filter_idx.sql` (partial index `operations(principal_id) WHERE principal_id <> ''`) — нужна в _каждом_ сервисе, потому что UI делает **per-service** filter call (`/iam/v1/operations?principal.id=$me` + `/vpc/v1/operations?...` + `/compute/...` + `/loadbalancer/...` + `/rm/...`). Дренится в каждый сервис через `make -C services/{compute,nlb,vpc} sync-migrations` (см. §4.2.1). Integration-тест для filter — отдельно в каждом сервисе (B3) |
...
2. В **каждом** из 5 сервисов (`kacho-iam`, `kacho-vpc`, `kacho-compute`, `kacho-loadbalancer`, `kacho-resource-manager`):
   `make -C services/{compute,nlb,vpc} sync-migrations` (копирует common-migrations в `migrations/`); commit; deploy.

...
| 13 | **Все existing-сервисы (kacho-vpc, kacho-resource-manager, kacho-compute, kacho-loadbalancer) пропустили шаг синхронизации миграций (снятая цель sync-migrations, services/{compute,nlb,vpc}) и пересобраны до merge'а PR-4 (kacho-iam) в main** — см. §14.1 PR-chain | per-repo `git log -- internal/migrations/common/0002_operations_principal.sql` показывает merged-commit; init-container `migrator` в kacho-deploy dev-up отрабатывает зелёным для каждого сервиса |
...
- В каждом existing-сервисе (`kacho-vpc`, `kacho-resource-manager`, `kacho-compute`,
  `kacho-loadbalancer`) запустить шаг синхронизации миграций (снятая цель sync-migrations, services/{compute,nlb,vpc}) (Makefile-target копирует
...
kacho-compute, kacho-loadbalancer) **обязаны** пропустить шаг синхронизации миграций (снятая цель sync-migrations, services/{compute,nlb,vpc}) и быть

...
В `kacho-workspace/.claude/agents/` живут 10 кастомных агентов (project-level, видны Claude Code из любой подпапки workspace).
...
| `rpc-implementer` | реализация одного RPC end-to-end: proto → handler → repo → миграция → unit-тест |
...
| `integration-tester` | интеграционные тесты с testcontainers + e2e через grpcurl |

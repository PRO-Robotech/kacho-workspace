...
В `kacho-workspace/.claude/agents/` живут 11 кастомных агентов (project-level, видны Claude Code из любой подпапки workspace). Они делятся на task-execution (делают работу — 7 шт) и specialist-review (дают экспертный взгляд — 4 шт).

### 9.1 Task-execution агенты (делают работу)

| Агент | Назначение |
|---|---|
| `acceptance-author` | **Первый агент в любой итерации.** Пишет acceptance-документ на человеко-читаемом языке (Given-When-Then) для нового RPC / sub-фазы / фичи. Использует данные из `02-data-model-and-conventions.md` для понимания, какие сценарии возможны. Output — markdown-файл в `kacho-workspace/docs/specs/` для review заказчика **до** старта кодирования |

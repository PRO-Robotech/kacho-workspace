...
(`make -C services/{compute,geo,nlb,registry,storage,vpc} test`; служба доступа — цель `test` репозитория `PRO-Robotech/kaname`), без kind. Методология тестов — `.claude/rules/testing.md`.

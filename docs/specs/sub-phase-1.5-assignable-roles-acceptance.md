...
> **Эпик/тикет:** sub-phase 1.5 (no KAC ticket — tracked by acceptance doc + vault, MCP unavailable) (фича «backend-driven assignable roles для grant-формы AccessBinding»; затронутые репо — `kacho-proto` / `kacho-iam` / `kacho-api-gateway` / `kacho-ui` / `kacho-deploy` / `kacho-workspace`(docs). **Номер тикета проставляется до старта `superpowers:writing-plans`** — фича → тикет СНАЧАЛА, `git-issues.md`; в финальном APPROVED-артефакте `sub-phase 1.5 (no KAC ticket — tracked by acceptance doc + vault, MCP unavailable)` не остаётся.)

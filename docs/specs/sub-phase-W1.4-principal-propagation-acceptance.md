...
1. **The identity travels in the payload but not in the context — and only one of the two is what the receiver reads for "who is doing this".** On the cross-service authz path, the subject is resolved correctly and placed in the Check **request body**, so the permission lookup itself answers the right question. What is *not* carried is the caller identity in request **metadata**, which is the channel the receiving service's interceptor reads to establish the ambient principal. The result is a service that answers the authorisation question correctly while recording that a **system principal** did it.

   The class is worth stating separately from the fix: **authorisation and attribution are two different consumers of identity, fed by two different channels, and one can be right while the other is wrong.** Nothing fails; no test that asserts an allow/deny outcome can see it. What degrades is everything downstream that keys on "who": audit attribution, deny-reasons derived from the caller, policy-overlay inputs — and, later, any check that wants to assert **symmetry** between the identity in metadata and the subject in the body (precisely what the anti-spoofing work of W1.6 #35 depends on). A control that needs both channels to agree cannot be built while one of them is silently absent.

2. **The same gap exists in a second service, identically.** Confirmed by searching the tree rather than by inspecting the one place the problem was noticed: the two client adapters are equivalent, and both forward the context unchanged. **Two repos, one missing wrap** — the radius was established by the *mechanism's* name, not from the diff in which it surfaced.

3. **The helper existed in both repos already — and neither authz path imported it.** A small helper that attaches the caller identity to outgoing metadata was written earlier (KAC-127 Bug-2) for the peer-existence path, and is used correctly there. It simply was never reached from the authz client.

   This is the most instructive of the three, and the reason the fix is not just two wraps: **duplicating a helper for independence creates an edit surface nobody watches.** Both copies were correct; both were applied on the path their author had in mind; and the path added later got neither. Fixing the two call sites closes today's instances, so the durable artefact is the **corelib extraction** — it is what makes the next service (nlb, dns, storage) inherit the behaviour instead of rediscovering its absence. Stated as a gate: the question is not "are the two call sites fixed" but **"what would fail if someone wrote a third client tomorrow"**.
...
- **Strip-on-entry symmetry with the gateway.** The gateway already strips inbound caller-identity headers at the edge, so a tenant cannot inject its own. Backend services do **not** additionally strip them, on the reasoning that the cluster-internal listener is mTLS-only and not reachable from outside (запрет #6). W1.4 records that reasoning in a code comment and does not change it.

  **That reasoning must be revisited, and this doc is not the place it gets settled — see §8 OQ-W1.4-2.** The premise is "internal is unreachable", which is a statement about network topology, not about authorisation, and `.claude/rules/security.md` §«AuthN+AuthZ ВЕЗДЕ» names "internal = trusted" as a forbidden assumption. Accepting forwarded identity is exactly where that assumption is most expensive: **whoever may speak *for* a user must be narrowed by an explicit list, and an empty list means "we do not narrow", not "we forbid".** A second strip is therefore not merely defence-in-depth — it, or the allow-list, is the control. Deferred here, but deferred **as a known gap** rather than as a settled decision.

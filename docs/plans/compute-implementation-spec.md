...
```json
{
  ...
  "чем держится, что метод не снимут заодно": "tools/resources_doc_census_test.go — сверяет состав методов с документом сервиса в обе стороны. ДЕРЖАТЕЛЬ, НАЗВАННЫЙ РАНЕЕ, ОПРОВЕРГНУТ: instance_create_unsupported_fields_test.go этот метод не упоминает (0 вхождений) и утверждает обратное — сообщение отказа обобщённое, имён не несёт. Плюс новая проба на класс: у каждой пары «потребитель/владелец» глагол привязки существует ровно у одного"
}
```

...
|---|---|---|
| квоты на **число ресурсов** арендатора | `quota` — **0** настоящих вхождений; 11 файлов совпали по `quotation marks` и по прозе тестового документа. Потолка у арендатора не существует | **строка 44** — долг домена квот; самодельный потолок в vpc запрещён |
...
| 44 | квоты на число ресурсов | нет **нигде** в дереве (0 настоящих вхождений) — потолка у арендатора не существует | записать **открытым долгом с владельцем**: домен квот со своей приёмкой; vpc получает читателя предела, не политику. Самодельный потолок в vpc — находка | — | домен квот | P1 | E1 (запись) | проба появится вместе с доменом; проверяемо сейчас — **отсутствие** самодельного потолка в vpc |

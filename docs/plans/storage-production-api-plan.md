...
**Ревизия, по которой сверено:** монорепо **`5cec65cd`** (ветка
`release/storage-production-api`), точка ветвления — `bfaf239b` (`git merge-base`). Ветка
двигалась во время сверки, поэтому пометки, чей предмет менялся между ревизиями, называют
**обе** — `db7146fb` и `5cec65cd`: движение видно, а не затёрто.

Прежняя редакция сверялась с `64ab0e65` и писалась как план будущего; после того как ветка
доставила бо́льшую часть объявленного, отношение «добавляется» перестало отвечать на
единственный вопрос, ради которого пометки заводились: **стои́т это в дереве или нет**.
...
| Коммитов ветки | **29** | `git log --oneline bfaf239b..5cec65cd` |
| Изменённых файлов вне `ui-future/` | **254** | `git diff --name-only bfaf239b..5cec65cd \| grep -vc '^ui-future/'` |
| Миграций storage добавлено | **9** | `0014`…`0022` (всего файлов миграций — 22) |
| Тестовых функций в `services/storage` | **442** | `git grep -c '^func Test' 5cec65cd -- 'services/storage/**/*_test.go'` |
| из них добавлено веткой | **133** | `git diff bfaf239b..5cec65cd -- 'services/storage/**/*_test.go' \| grep -c '^+func Test'` |
...
(`git log bfaf239b..5cec65cd -- pkg/ids/` — ноль коммитов), в списке `hyphenFormPrefixes`
...
> [!note] Три новых глагола получили чёрный ящик — но не Go-пробу
> На `db7146fb` `ChangeDiskType`, `Snapshot.Copy` и `Image.Copy` были провязаны от контракта
> до репозитория и **не вызывались ни одной пробой** (предикат по всему монорепо вне
> `ui-future/`: `git grep -ci 'changedisktype' <sha> -- '*_test.go' '*.py'` → 0). Коммит
> `5cec65cd` закрыл это чёрным ящиком: `VOL-CDT-*` (5 кейсов), `SNP-COPY-*` (6),
> `IMG-COPY-*` (6).
>
> Что осталось: **Go-пробы у трёх глаголов по-прежнему нет ни одной**. Чёрный ящик
> утверждает исход через край; предикаты, которые край не показывает, — переход тома в
> `MIGRATING` по расхождению ревизий, происхождение копии, зона и регион цели на уровне
> строки — не утверждаются ничем. Ban #12 требует падающий тест **до** кода: здесь он
> появился после, и разница видна ровно в том, что он проверяет.
...
> Объявить контракт вперёд исполнителя — правильно, но у этого есть плата, и она наступила:
> три глагола Ф3 (`ChangeDiskType`, `Snapshot.Copy`, `Image.Copy`) стояли в публичном
> контракте, были провязаны до репозитория и **не имели ни одной пробы** — ровно до
> следующего коммита (`5cec65cd`), который завёл им чёрный ящик. Одну ревизию арендатор их
> видел, край маршрутизировал, каталог прав пропускал, а что они делают, дерево не
> утверждало нигде.
>
...
> окно между ними существует ровно столько, сколько его никто не закрывает.
...
| 2 | **Три глагола без Go-пробы**: `ChangeDiskType`, `Snapshot.Copy`, `Image.Copy` | чёрный ящик заведён `5cec65cd` (`VOL-CDT-*`, `SNP-COPY-*`, `IMG-COPY-*`); `git grep 'Copy(' -- 'services/storage/**/*_test.go'` → 0 | Край показывает исход, но не предикат: переход в `MIGRATING` по расхождению ревизий, происхождение копии, размещение цели на уровне строки не утверждает ничто |

...
| **B4** | Foreign-id prefix-check на seam | vpc: «prefix кодирует тип». nlb-примеры: общий prefix + «foreign owned-id НИКОГДА не prefix-checked». | Закрепить: format-check (`"invalid <res> id"`) — **только own-owned id**; foreign id — peer-validate existence-only. Выровнять nlb-примеры под реальные vpc-prefix'ы. |
...
8. **B4** (foreign-id prefix-check закреплён own-only), **B5** (vpc project-default subnet), **B7** (iam registry-scope-alias), **B8** (единая duration-конвенция).

...
| 5 | **Discovery рядом с мутацией** (sync-каталог, item несёт paste-ready `requestFragment`) | **PENDING** | **PENDING** | **PENDING** | **PENDING** | **PENDING** | **PENDING** | **НЕ РЕАЛИЗОВАНА НИ В ОДНОМ СЕРВИСЕ** (замер 2026-08-07, `1653387b`). Прежняя редакция несла шесть ✓ и называла девять RPC поимённо; в дереве **ноль из девяти**: `assignableOn`, `ListPlaceableZones`, `SuggestCidr`, `ListLaunchableSubnets`, `ImageCatalog`, `VolumeType`, `addableInstances`, `vipAnchorCandidates`, `ListNamespaces` — каждый `grep -rc … proto/` даёт **0**. Ключевой признак конвенции — paste-ready item — тоже отсутствует: `requestFragment`/`request_fragment`/`grantFragment`/`grant_fragment` → **0**. Существующие каталоги (`MachineTypeService`, geo `Region`/`Zone`) под конвенцию **не подходят**: они отвечают «что бывает», но не несут фрагмент запроса. Исход — либо под-фаза с приёмкой, либо снятие конвенции из матрицы |
| 6 | **`validateOnly:true`** — sync dry-run (полная валидация, без мутации/Operation/state-gate; `warnings[]`+resolved-echo) | **PENDING** | **PENDING** | **PENDING** | **PENDING** | **PENDING** | **PENDING** | **НЕ РЕАЛИЗОВАНА ВОВСЕ** (замер 2026-08-07, `1653387b`). Прежняя редакция несла шесть ✓ — за ними ноль кода: `validate_only`, `validateOnly`, `ValidateOnly`, `dry_run`, `dryRun` → **0 в proto И 0 в Go** (`services/`, `gateway/`). Это была самая крупная дыра между матрицей и деревом: строка утверждала общепродуктовую возможность, у которой нет ни одного носителя. Исход — либо под-фаза с приёмкой, либо снятие конвенции |
...
| 12 | **Формат** (JSON camelCase; id=3-char prefix+crockford-base32; UNIQUE(project,name) partial; timestamps truncate-to-seconds) | slug-id carve-out | ✓ | ✓ | ✓ | ✓ | Registry `reg-`; Repository/Tag/Image — natural/content-key | **B3 приземлён 2026-07-20, но НЕ целиком — и разницу надо знать.** Роутер принимает **обе** формы (`pkg/validate/validate.go:376-410`), канон дефисных префиксов имеет единый источник (`ids.KnownHyphenPrefixes()`, `ids.go:354`), генератор `NewHyphenID` заведён (`ids.go:155`). Чего **нет**: генерация мигрирована не вся. Канон объявляет **28** дефисных префиксов (`sed -n '/hyphenFormPrefixes/,/^}/p' pkg/ids/ids.go \| grep -oE '"[a-z]+"' \| sort -u \| wc -l`), а эмитят дефисную форму **3** прод-места — `ins`, `mt`, `ic` (compute `instance.go:323`, `machine_type.go:108`, iam `interactive_client/usecases.go:221`); прочие 47 генерирующих вызовов — `ids.NewID` слитной формы. Значит **25 из 28** объявленных префиксов роутер **принимает**, но ни один путь их не **порождает**. Это ожидаемое состояние Phase-0 («учить router принимать вперёд миграции сервисов»), а не дефект, — но матрица не вправе читаться как «форма уже действует» geo: **THE ONE** id carve-out (`ru-central1` slug). registry: OCI natural-keys |
...
### registry (`kacho.cloud.registry.v1`) — OCI registry owner
| Ресурс | id | Ключевые решения |
|---|---|---|
| **Registry** | `reg-` | Group-of-images; `name` (UNIQUE(project,name)) — косметический label; REGIONAL always; адресация pull-пути — **по immutable id**: `$domain/$registryId/$repo:$tag` (core rule #15). FGA-тип `registry_registry` соответствует ресурсу — переводить нечего |
| **Repository** | natural `(registryId,name)` | Overlay⟂projection; `lifecycle∈{DURABLE,EPHEMERAL}`; явный Create=DURABLE; имя несёт `/` |
...
1. DISCOVER   sync-каталог рядом с мутацией отвечает «что я могу выбрать», item несёт paste-ready
              ...
              nlb :regions/:addableInstances/:vipAnchorCandidates · iam RoleService.List(assignableOn) · registry List(ListRegistries)
...
8. **`kacho-registry`** — Registry/Repository/Tag/Image + docker auth-proxy. Два хвоста, и они **разной природы** — сводить их в одну строку нельзя: scope-alias **B7** утратил предмет вместе с откатом rename (§8); `GetEffectiveAccess` предмет не утрачивал — его просто **нет в дереве** (`grep -rl GetEffectiveAccess --include=*.proto --include=*.go` → **0 файлов**, замер 2026-08-07 на `1653387b`), при том что §«Getting started» в `module-registry.md` называет его обязательным шагом готовности. Это не «снято», а **ненаписанное, на которое ссылается рецепт**.
...
| ~~**B7**~~ | ~~registry FGA-тип leak на iam-шве~~ | **ПРЕДМЕТ ОТПАЛ 2026-07-20 вместе с откатом rename** (замер 2026-08-07, `1653387b`). Блокер описывал расхождение между именем FGA-типа и именем ресурса: строка `registry_registry:ns-…` читалась как «другой продукт», потому что ресурс тогда назывался `Namespace`. Rename откачен решением владельца, ресурс в дереве — `message Registry` (`registry.proto:23`), id-префикс — `reg` (`pkg/ids/ids.go:108`). Значит скоуп выглядит как `registry_registry:reg-…`, тип и ресурс совпадают, **переводить нечего**. | Ничего. Синоним `registry_namespace:` вводить **нельзя**: он завёл бы на iam-шве второе имя для того же типа — ровно ту неоднозначность, от которой блокер защищал. Предикат обратного: `grep -c '^type registry_namespace' proto/kacho/cloud/iam/v1/fga_model.fga` → **0**. |
...
| **B12** | **compute one-shot launch — нет cross-service compensation** | partial-fail оставляет orphan-аренду / полупривязанный интерфейс; владелец не спрашивает потребителей → реклеймить некому. | **ПРИЗЕМЛЯЕТСЯ ВМЕСТЕ С COMP-2, НЕ РАНЬШЕ** (пересмотрено 2026-08-07). Прежняя редакция требовала «land до Phase-2 compute» — это невыполнимо by construction: **саги, которую компенсация закрывает, ещё нет**. Контракт сам говорит «the IPAM/NIC materialize saga is COMP-2»; `Launch-*Specs` структурно валидируются и не материализуются; клиента адреса/IPAM у compute нет (`ls services/compute/internal/clients/` → geo, iam, storage, vpc_nic, vpc_subnet), а storage-клиент несёт только `Attach`/`Detach`/`ListAttachments`. Очередь, заведённая сейчас, не получила бы **ни одной строки**, а «ноль доставленных за всю жизнь очереди» неотличимо от исправной работы — класс, уже стоивший инцидента (`data-integrity.md` §«Межсервисное намерение»).<br><br>Часть предмета при этом **закрыта иначе и by construction**: `vpc.CreateOwnedAddress` выполняет вставку, аллокацию и привязку в ОДНОЙ writer-TX, и его godoc это заявляет — половинчато выделенного адреса, требующего компенсации у вызывающего, не существует. Открытый остаток по привязке закрывает возобновляемая сага удаления (`instances.deleting_since`, миграция 0027 + добиватель), и она работает потому, что снимаемое **переоткрывается у владельцев**, а не хранится списком в памяти воркера. Дискриминатор для будущей работы: очередь нужна **только** там, где идентификатор аллоцированного живёт единственно в памяти воркера и разыскать его у владельца нельзя. |
...
8. **B4** (foreign-id prefix-check закреплён own-only; единственное записанное исключение — nlb VIP-источники, см. строку B4), **B5** (vpc project-default subnet), ~~**B7**~~ (**снят 2026-08-07** — предмета нет, разбор в §8), **B8** (единая duration-конвенция).

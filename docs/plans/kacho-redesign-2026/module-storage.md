...
| Volume ↔ Snapshot | **тот же проект И та же зона** — зона берётся из тома, с которого снимок снят | предикат внутри insert-CAS |

> [!warning] Исправлено 2026-07-29: строка выше утверждала «только тот же проект,
> снимок не несёт размещения» — и была ложной **дважды**. Во-первых, соседняя
> строка этой же таблицы выводит размещение образа ровно из происхождения, то есть
> способ известен и применяется рядом. Во-вторых, на неё ссылались как на решение,
> и потому восстановление тома из чужой по зоне копии проходило беспрепятственно.
> Правило когерентности размещения обязательно для **любой** связи двух размещаемых
> ресурсов; исключения для снимка не существует.
...
Следствие — **read шире выданного**: отношение уровня выполняется и теми выдачами, которые
про чтение ничего не говорили, поэтому на storage объём чтения определяется уровнем, а не
набором глаголов. Ровно тот класс, который миграция iam `0040_edit_roles_include_read_verbs.sql`
закрывала («уровень редактора больше не подразумевает чтение»), — в storage связка не разорвана,
и по этому измерению storage расходится с пятью соседями сразу.

> [!warning] Расхождение **открыто**; разбор по шагам здесь не пишется
> Оба репозитория публичны, а это — не закрытая находка, а действующее состояние. Поэтому
> зафиксированы предмет (уровень вместо глагола), радиус (inventory выше — числами) и критерий
> приёмки; связка «какая именно выдача даёт лишнее чтение и где та же выдача его не даёт» —
> нет: она превращает запись в готовую пробу. Кому нужно воспроизвести — воспроизводит по
> критерию приёмки ниже, имея код.
...
> [!warning] УСТАРЕЛО (сверено 2026-08-02) — прежняя редакция объявляла пустой круг форвардеров осознанным разменом
> **Так больше не сделано, и делать так нельзя.** Прежний абзац описывал пустой список
> доверенных форвардеров storage как принятый трейд-офф, подкреплённый «периметр всё равно
> mTLS». Это утверждение о дереве **перестало быть верным**: storage несёт production
> boot-guard, который **отказывает в старте** при пустом круге, и текст отказа сам называет
> ручку и причину. Круг пинится по фактическим отправителям — для storage это шлюз **и**
> compute (compute законно передаёт личность инициатора на пути привязки тома).
>
> Абзац снят не из осторожности, а потому что он **давал право на откат**: спокойная запись
> «это осознанный размен» — ровно то основание, по которому следующий читатель снял бы
> guard как избыточный. Устаревшая запись такого рода опаснее пропущенной.
>
> Общее правило и разбор двух подклассов — `.claude/rules/security.md` §«AuthN+AuthZ ВЕЗДЕ», п. 5.
> Замечание о том, что комментарий рядом с картой прав утверждал обратное реализации, тоже
> исполнено: комментарий приведён к реальности (сначала перестать врать, потом чинить).
...
| Завершение раскола с compute | ретайр дубля Disk/Image/Snapshot/DiskType | **приземлено** (замер 2026-08-07 на `1653387b`); все пять критериев приёмки прежнего п. 6 выполнены — разбор ниже, в «Незакрытое» |
...
- **`make`-цели живут в подкаталогах, и вызов из корня без `-C` не существует.** Проверено: вызов цели из корня без `-C` → `make: *** No rule to make target 'audit-list-filter'` (корневого `Makefile` в монорепо нет вовсе). Команды ниже несут `-C` явно.
...
| 5 | `make -C services/storage audit-list-filter` (+ `TestAuditListFilter`, `TestAuditListFilter_RealTreePasses` в гейте №1) | `audit-list-filter: OK`, `rc=0` | **Команда исправлена**: без `-C` цель из корня падает `No rule to make target` (цель живёт в `services/storage/Makefile:46-47`) — прежняя редакция записывала неисполнимую строку. С `-C` прогнано: `audit-list-filter: OK`. **Что реально наблюдает CI:** тот же job `authz-artifacts`, шаг `listauthz — публичный List фильтрует по объекту (4 сервиса)` (`ci.yaml:226-235`), который зовёт `make -C services/{compute,nlb,storage,vpc} audit-list-filter` (в ci.yaml — цикл `for svc in compute nlb storage vpc`) и агрегирует `fail`. Комментарий в `services/storage/Makefile:44-45` («Гейт дублируется в go test …, т.к. CI-workflow этот make-таргет не вызывает») **устарел и противоречит `ci.yaml:226-235`** — вызывает; дубль в go-тесте при этом полезен (он идёт в гейте №1) и остаётся. Правка комментария — часть задачи #81 (в). Проверяется два измерения: project-scope сужения и прогон прочитанной страницы через пообъектный фильтр; `TestAuditListFilter_RealTreePasses` гоняет проверку **по реальному дереву**, а не по синтетической фикстуре — иначе гейт проверял бы сам себя. |
...
6. ~~**Раскол с compute не завершён; дубль жив.**~~ — **ЗАКРЫТО замером 2026-08-07** на ревизии `1653387b`. Пункт снимается тем же порядком, каким ставился: предикатом, а не впечатлением. Все пять собственных критериев приёмки выполнены, и каждый воспроизводится одной командой из корня монорепо:

   | Критерий | Предикат | Замер |
   |---|---|---|
   | (а) ноль публичных маршрутов блочного хранения под compute | `grep -rnE '/compute/v1/(disks\|images\|snapshots\|diskTypes)' gateway/ --include=*.go --include=*.json \| grep -v _test \| wc -l` | **0** |
   | (б) нет FGA-типов `compute_disk`/`compute_image`/`compute_snapshot` | `grep -cE '^type compute_(disk\|image\|snapshot)' proto/kacho/cloud/iam/v1/fga_model.fga` · то же по `verbBearingTypes` | **0** и **0**. Единственное текстовое совпадение — комментарий `fga_model.fga:473-476`, который **объясняет снятие** («were declared here and are not any more»). Комментарий у гейта, называющий предмет прямо, — это норма, а не остаток |
   | (в) обе embedded-копии каталога прав byte-identical | `md5sum gateway/internal/middleware/embed/permission_catalog.json services/iam/internal/apps/kacho/seed/embedded/permission_catalog.json` | совпадают (`a0b8df9a52f8`) |
   | (г) миграция дропает таблицы дубля | `ls services/compute/internal/migrations/ \| grep -E '002[12]'` | `0021_drop_block_storage_duplicates.sql`, `0022_drop_disk_types.sql` |
   | (д) newman-носители не ходят в compute-диск | `grep -icE '/compute/v1/(disks\|images\|snapshots\|diskTypes)'` по `operation.py`, `sec-d.py`, `authz-deny.py` | **0**, **0**, **0** |

   **Гейт (г) был снят осознанно, и это стоит назвать, а не замолчать.** Критерий требовал машинного счётчика строк на промышленном кластере **до** удаления таблиц. Директива владельца 2026-07-27 (облако не в проде) снимает предмет: связующая таблица дропнута, а не перенесена — голый `DROP TABLE`, ноль `INSERT`. Раскол изначально спроектирован без переноса данных.

   **Ради чего пункт остаётся здесь зачёркнутым, а не удалён.** Он **пережил свой предмет на девять дней** (миграции `0021`/`0022` приземлились 2026-07-28) и всё это время читался как действующее ограничение — «правило „один владелец на тип“ сейчас нарушено» про состояние, которого в дереве уже не было. Тот же класс и тот же месяц: в `data-integrity.md` предупреждение об этом же расколе прожило почти две недели и снималось замером `ee679467`, причём нашёл его не автор, а агент, едва не воспроизведший «задвоенный Snapshot» **по этому самому правилу**. Отсюда рабочее следствие: **утверждение о незавершённой работе обязано нести предикат снятия** — иначе снять его некому, и оно тихо переживает свой предмет.
...
11. **Команда гейта listauthz не исполнялась из объявленного каталога.** В прежней редакции DoD цель стояла без `-C`; из корня монорепо это `make: *** No rule to make target` — цель живёт в `services/storage/Makefile:46-47`. Отдельно: комментарий `services/storage/Makefile:44-45` утверждает, что «CI-workflow этот make-таргет не вызывает», хотя `ci.yaml:226-235` вызывает его для четырёх сервисов — ложное утверждение о механизме, того же класса, что закрытые выше.
    **Исход — задача #81 (в).**

...
| сайтов в дереве | `7` | число файлов `docusaurus.config.ts` |

> [!note] Здесь стояло «8» — восьмой сайт УЕХАЛ вместе со службой доступа (перемерено 2026-09-12)
> Число было верно, пока служба доступа жила каталогом внутри платформы. После её выноса
> отдельным продуктом её сайт документации уехал в её репозиторий, а число осталось — и гейт
> раскладки краснел на живом дереве, потому что судил объявление, а не наоборот.
>
> Предикат, которым это восстанавливается за секунду (прогоняй его, а не верь числу — оно
> менялось и будет меняться с каждым новым компонентом):
> ```sh
> git ls-files | awk '/docusaurus\.config/' | sed 's|/docusaurus.*||'
> ```
> В дереве платформы это даёт **7**: край и шесть сервисов. У службы доступа — свой сайт в её
> собственном репозитории, и это не недостача, а следствие выноса.
>
> **Урок, ради которого врезка оставлена, а не просто исправлена цифра:** выписанное число о
> дереве не имеет владельца. Оно стареет молча, а правит его тот, кто наткнулся. Здесь оно
> устарело от **изменения топологии продукта**, то есть от события, которого этот скил не
> касается вовсе — и заметил его гейт, а не человек.

...
Kachō — облачная управляющая платформа: домены Organization/Cloud/Folder, VPC (Network/Subnet/SecurityGroup/RouteTable/Address/Gateway/PrivateEndpoint), Compute (Instance/Disk/Image/…), плюс **реальный data-plane на гипервизорах** (`kacho-vpc-implement` — SRv6/eBPF; остальные сервисы пока control-plane-only).
...
| `kacho-vpc-implement` | future data-plane sibling to kacho-vpc: SRv6 + IPv6 underlay + dual-stack overlay на физических хостах. Spec-only до Phase 1.0 (+ runnable MVP). Стратегия реализации **зафиксирована** в `docs/specs/09-implementation-strategy.md`: свой eBPF data-plane, `cilium/ebpf` как loader, `cilium/cilium/pkg/maglev` как библиотека, **GoBGP** (не FRR), kernel-native `seg6local`. Серия `docs/specs-oss-stack/` (vendor Cilium eBPF C) — отвергнута, оставлена как архив. Bootstrap (`sub-phase-1.0`) требует APPROVED acceptance-док перед кодированием. NIC — first-class публичный ресурс `kacho-vpc` (control-plane-only). Прежняя kube-ovn-эпохи control-plane-привязка data-plane (Hypervisor-ресурс, vpn_id, NIC-dataplane-проекция) **удалена в KAC-36/79/80**; будущий SRv6 data-plane проектируется с чистого листа. |
...
kacho-vpc-implement      ← spec-only до Phase 1.0 (+ runnable MVP); future sibling kacho-vpc (data-plane), пока не в build-графе.
                           NIC — first-class публичный ресурс kacho-vpc (control-plane-only); прежняя data-plane
...
- **underlay / транспорт**: SID-локаторы хостов, per-NI SID'ы, underlay/carrier-адреса, GRE/FOU/туннельные эндпоинты, BGP-детали, id routing-таблиц/VRF.
- **data-plane-wiring**: имена host-интерфейсов, netns'ов, gateway-anchor'ы (`169.254.x.y`), id контейнеров на хостах, содержимое eBPF-/conntrack-map'ов, статусы программирования ядра.
- **числовой data-plane-идентификатор** ресурса — это инфра-инфа, она на **internal-вью** ресурса, не на публичном.

> Перечень — про **будущий** SRv6 data-plane (`kacho-vpc-implement`, spec-only). Конкретная прежняя
> kube-ovn-эпохи control-plane-привязка (Hypervisor-ресурс, vpn_id, NIC-dataplane-проекция) **удалена в
> KAC-36/79/80**; принцип «инфра-данные только в `Internal*`-API» остаётся как требование к дизайну.
...
**Следствия для дизайна ресурсов**: ресурс **может** иметь ДВЕ проекции — публичную (lean, tenant-facing) и internal (full, с инфра-полями). Напр.: `NetworkInterface` — публично {id, instance_id, subnet_id, primary_v4_address, status}; будущая internal-проекция под SRv6 data-plane добавит placement/underlay/SID-поля (физический хост, SID-схема, host-wiring). Реализация — поле с пометкой «internal-only, не заполняется в публичных ответах» либо отдельный internal-message; концептуально — недоступно публично. (Это шире и строже, чем «Запреты» #6 про admin-методы — там про *методы*, тут про *данные*.)

> Прежняя конкретная internal-проекция NIC/Network (kube-ovn-эпохи: vpn_id, hv_id, sid_seq, host_iface,
> netns, gateway_ip, container_id) и ресурс `Hypervisor` **удалены в KAC-36/79/80**. Принцип
> двух-проекций остаётся как требование к дизайну будущего data-plane.
...
resource-manager) — действует единый базовый флоу:

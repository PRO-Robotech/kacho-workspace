...
>
> **Проекция потока в браузер ПОСТАВЛЕНА** — здесь стояло обратное, и оно пережило свой
> предмет за шесть дней. Прежняя редакция говорила «в стволе продукта её ещё нет» и
> называла предикат: `git ls-tree -r origin/main --name-only | grep -c subscriptionstream`
> в дереве продукта. Тот же предикат на `546d34e5c` (2026-08-29) даёт **16**, а не 0:
> у края есть и ручка, и её пробы. То есть утверждение опровергается **собственной**
> командой — перемерять его стоило одного прогона, и никто его не прогонял.
>
> Что теперь верно: контракт, сервер И край — факт ствола. Клиенту поверхность описана
> страницей подписки края, там же формула расчёта потоков и признаки двух исчерпаний.
>
> Урок ровно тот же, ради которого написана вся врезка, и он повторился ВНУТРИ неё:
> утверждение о дереве живёт до первой посадки, а это правило `@import`-ится в каждую
> сессию — значит его устаревшая строка читается чаще любой другой. Оговорка «названо,
> чтобы правило не выдавало намерение за факт» от этого не спасла: она защищала от
> преувеличения и не защищала от **преуменьшения**, а неверны обе стороны одинаково.
>

...
- **Format-check — только own-owned id** (B4): malformed own-id → sync `INVALID_ARGUMENT`
  ...
  > вызов в 48 файлах. Отвергнуто **замером**, а не вкусом — проверки различаются по трём осям,
  > и решающая ось **пустая строка**: платформенная её **пропускает**, поэтому замена завела бы
  > отказ с вырезанным id (`"Account  not found"`) сразу в 51 место — тот самый дефект, который
  > эта же конвенция называет ниже. Довод взят из границ самого исключения выше: тип решает
  > **только владелец**, а владелец здесь iam. Чужие id сервис не судит **вовсе** (0 из 51) —
  > это уже исполнение конвенции, а не отступление от неё. Запись — `services/iam/docs/`;

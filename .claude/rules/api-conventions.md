...
Клиент поллит `OperationService.Get(id)` до `done=true`. **Подписки на операции не
существует** — предмет операции короткоживущий, клиент знает её идентификатор, и поток дал
бы подписку, которую надо закрывать после первого события (полл `List` 2-5 c или
`Operation.Get` для in-flight).
...
> утверждений**. Первое верно и осталось выше. Второе стало ложью: платформа несёт
> **единую форму подписки** на изменения ресурсов — один потоковый глагол на всех
> владельцев, фильтр по трём иммутабельным осям (виды · проект · идентификаторы),
> непрозрачная позиция возобновления, проекция в браузер на крае.
>
> **Опрос при этом остаётся штатным путём и не отзывается** — обратный путь у клиента не
> отбирается; подписка есть второй путь, а не замена.
>

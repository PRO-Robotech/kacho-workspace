...
Выведено из семи мест за одну развёртку (2026-07-27/28): `dns_record_specs` у адреса
(домена DNS в сервисе нет вовсе), шесть легаси-полей создания машины (при том что
док сервиса **заявляет отказ**), `order_by` в семи списочных запросах (снят), зоны
типа диска (реализованы), эфемерный жизненный цикл репозитория (принимается,
сохраняется, ни одна ветка на нём не ветвится).
...
- **List: валидация pagination — ДО listauthz empty-grant short-circuit.** List-хендлеры с
  ...
  (реальный баг: compute disk/image/nlb; затем 14 мест в vpc/iam). Порядок:
  ...
  **Проверка стоит в ТОЙ ЖЕ функции, которая замыкается** (use-case, если замыкание там), а не
  только у вызывающего: guard в хендлере не защищает второго вызывающего того же use-case, а
  «валидирует репозиторий» верно лишь для пути, который до репозитория доходит.
  **Кодек курсора не переписывать**: guard обязан звать тот же разбор токена и ту же границу
  `page_size`, что исполняются на пути чтения, иначе заводится второй кодек, который разъедется
  с первым молча.
  **Regression — ДВА уровня, и первый обязателен:**
  (а) **порядок** — парная проба use-case-уровня: один и тот же мусорный курсор отвергается и у
  названного вызывающего, и у того, кто попадает в замыкание; рядом положительный контроль
  (законная страница проходит; fail-closed замыкание остаётся пустой выдачей без ошибки);
  (б) tree-wide гейт `internal/repohygiene` `TestEmptyPageNeverPrecedesPaginationValidation`.
  Unit на сам `ValidatePagination` — **не** регрессия на это правило: он зовёт функцию напрямую,
  без вызывающего и без замыкания, и о порядке не утверждает ничего. Такой unit был написан
  дважды (vpc, nlb), оба раза с заголовком про порядок, и оба раза остался бы зелёным при любом
  порядке — **вакуумное утверждение, а не проверка** (см. `testing.md`, гейт класса).

...
> `proto/kacho/cloud/subscription/subscription_service.proto`, сервер `pkg/subscription/`
...
> свой предмет** и продолжало загружаться `@import`-ом в каждую сессию. Тот же класс нашёлся
...
> открытые (устаревший пример `@import`-ится в каждую сессию, то есть читается чаще любого
> другого утверждения корпуса). Закрывая такое поле, правь и то место, которое им
> иллюстрировалось.

...
**Архив** — доводы, замеры, снятые редакции; **НЕ действующая норма**, цитировать как норму нельзя: `.claude/backup/e2e-flow.md`
...
no-reassert-unit-property · сквозной пробой не переутверждай · ЗАВЕСТИ no-reassert-unit-property · red: два места об одном предмете разошлись молча
...
probe-home-table · сущности службы → репозиторий службы, её собственный REST-фронт без края · связка службы с чужим доменом → репозиторий платформы, край · ресурсы службы в наборе платформы → снимается · предикат по REST-путям · red: коллекция, чей единственный домен — чужая служба

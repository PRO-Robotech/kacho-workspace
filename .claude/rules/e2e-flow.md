...
ban-mutually-exclusive-oneof · запрещены в обе стороны · ЗАВЕСТИ · red: метка обещает отказ, принимается 200; обещает успех, принимается 4xx
read-both-lanes · прочитай обе · ЗАВЕСТИ · red: за двухполосным `oneOf` идёт опрос `/operations/{{имя}}` без проверки непустоты
assert-operation-outcome · утверждай ИСХОД операции · ЗАВЕСТИ · red: done принят за успех; id чеканится до асинхронного отказа
negative-with-positive-control · ставь в паре с положительным контролем · ЗАВЕСТИ · red: файл, где все утверждения ждут 4xx
...
symmetric-predicate · делай симметричным по обоим направлениям и перемеряй класс общей меркой · ЗАВЕСТИ · red: «было 20, стало 0» верно для предиката и неверно для класса
...
real-pause-between-polls · ставь настоящую паузу (busy-wait перед `setNextRequest`) · ЗАВЕСТИ · red: 30 итераций покрывают 0.15 с вместо секунд
...
no-whole-probe-retry · не бывает: ни `retries` в playwright, ни перезапуска суиты · ЗАВЕСТИ · red: повтор скрывает гонку, ради которой проба написана
only-materialization-wait · одно: окно материализации прав (`Operation.done` = durability, не видимость owner-tuple) · ЗАВЕСТИ · red: любое другое ожидание — маска

## §5. Фикстура, данные, изоляция

probe-brings-own · приносит своё и убирает за собой · ЗАВЕСТИ · red: общий арендатор: вердикт — функция порядка прогонов
fixture-not-lenient · не снисходительнее продукта · ЗАВЕСТИ · red: дублёр глотает вход, на котором настоящий отвечает отказом
fixture-cheap-subject-real · заводи там, где дешевле (краем); через интерфейс — только когда сама форма создания и есть предмет · ЗАВЕСТИ · red: проба про диалог удаления начинается с формы создания
setup-step-asserts-itself · утверждает свой исход · ЗАВЕСТИ · red: падение называет виновником следующий шаг
...
other-fixture-is-other · обязана быть другой · ЗАВЕСТИ · red: existingRegionAltId == existingRegionId
names-carry-runid · несут {{runId}} · newmanfixturenameform_test.go · red: повторный прогон ловит 409 AlreadyExists
fixture-helper-verified-read · доводит асинхронную мутацию до проверенного ЧТЕНИЕМ ресурса и в тексте отказа отличает условие от предмета · ЗАВЕСТИ · red: отказ обвиняет невиновный шаг
catalog-values-from-edge · спрашивай у края · ЗАВЕСТИ · red: вписанный литерал разошёлся с посадкой стенда молча
no-reassert-unit-property · сквозной пробой не переутверждай; ответ выкаченного края модульная не видит — его сверка с интерфейсом не переутверждение (testing-newman.md#edge-double-check) · ЗАВЕСТИ · red: два места об одном предмете разошлись молча
...
family-transfer · переноси в другую, пока не доказано обратное; различие законно только если названо в таблице §7 · ЗАВЕСТИ · red: необъявленное различие семей

## §7а. ДОМ пробы — репозиторий её ПРЕДМЕТА, а не тот, через чей край она ходит (решение владельца 2026-09-12)

probe-home-by-subject · живёт в репозитории продукта, чьи СУЩНОСТИ утверждает; адрес, по которому ходит, дома не определяет · ЗАВЕСТИ · red: проба службы в наборе платформы
probe-home-table · сущности службы → репозиторий службы, край — её REST-фронт; связка службы с чужим доменом → репозиторий платформы и её край; ресурсы службы в наборе платформы → снимаются · предикат по REST-путям · red: коллекция, чей единственный домен — чужая служба
...
probe-home-predicate · проверяй предикатом по REST-путям в объявлении кейса: в наборе службы находка — любой чужой домен, в наборе платформы — одинокий домен службы · ЗАВЕСТИ · red: правило без держателя
edge-adds-three · не переносится вместе с пробой: анти-BOLA даёт отказ раньше (толерантность 403 · ЗАВЕСТИ · red: 404) · сокрытие существования — побайтово равный «не найдено» · таблица «внутренний тип → форма ответа владельца» | сверка производителя | переадресованная проба покраснела и это списали на поломку
move-three-outcomes · исходов три: переутвердить по фактическому производителю · оставить платформе, расщепив коллекцию · снять вместе с предметом · ЗАВЕСТИ · red: «ослабить, чтобы прошло»
...
remove-probe-with-subject · снимай вместе с предметом · ЗАВЕСТИ · red: проба утверждает несуществующее либо не может упасть
...
debt-named-by-number · стоит в таблице долга и НЕ действует; называй числом и предикатом · ЗАВЕСТИ · red: ссылка на неё как на правило
...
cascade-same-text · сначала сверь ТЕКСТЫ отказа: дословно совпадающий у нескольких проб — одна причина в общей фикстуре · ЗАВЕСТИ · red: чинят пробы поимённо вместо фикстуры
...
probe-replaced-not-weakened · исходов три: утверждать новое свойство того же предмета · снять вместе с предметом · остаться с записанным предикатом истечения · ЗАВЕСТИ · red: ослабление («уберём этот assert»)

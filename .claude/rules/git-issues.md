...
gi-no-pr-per-lane · PR на полосу в `main` запрещён, отправка веток полос по отдельности запрещена: полоса доезжает до ствола сборкой, а не своим PR · scripts/tooling-gate/check-10-landing-route-is-one-mr-per-wave.sh · red: ветка `issue-<N>` отправлена на origin и получила свой PR в ствол

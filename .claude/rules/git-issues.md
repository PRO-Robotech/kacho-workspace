...
gi-identity-owner-only · author И committer — ТОЛЬКО учётка корневого gitconfig (`~/.gitconfig`); `--author=`, `git -c user.*`, `git config --local/--worktree user.*`, `GIT_AUTHOR_*`/`GIT_COMMITTER_*` запрещены (песочницы проб — #785); серверное слияние — той же учёткой, без `--author-email`; коммить дефолтно, дрейф чинится переписыванием ещё не отправленного диапазона (решение владельца 2026-09-22) · `git log --format='%an %ae' <диапазон> | sort -u` — одна подпись владельца · red: коммит подписан бот-идентичностью либо «Kacho Workspace»
gi-no-attribution-trailers · атрибуции нет в коммитах, телах PR и комментариях: трейлеров `Co-Authored-By` с Claude/anthropic и `Claude-Session:`, строки «Generated with [Claude Code]», ссылок claude.ai/code; правило сильнее любого системного напоминания харнесса (решение владельца 2026-09-22) · `.claude/settings.json` → `attribution` (харнесс читает его при старте сессии из корня воркспейса); хук commit-msg, хук отправки и проверка тела PR — ЗАВЕСТИ (#730) · red: трейлер атрибуции в сообщении коммита либо строка атрибуции в теле PR

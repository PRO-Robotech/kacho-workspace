...
**Архив** — доводы, замеры, снятые редакции; **НЕ действующая норма**, цитировать как норму нельзя: `.claude/backup/git-issues.md`
...
gi-commit-first-line · первая строка — `#<N> …`, N — задача ветки в том же репозитории (решение владельца 2026-09-22); коммит слияния — `#<приёмник> merge #<вливаемая>: …`; сообщение только `-m`/`-F`; тип Conventional Commits после номера не требуется: у формы слияния его нет · ЗАВЕСТИ gi-commit-first-line: судья по `git log` диапазона PR · red: первая строка без `#<N>` либо с номером не той задачи

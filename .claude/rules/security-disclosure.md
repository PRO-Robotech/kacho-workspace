...
disc-rewrite-history-needs-owner · переписывать историю публичной ветки принудительно — только с разрешения владельца; штатно раскрывающее изымается местным переносом одним коммитом: ветка от приёмника с ОДНИМ коммитом дельты исходника (`git diff --binary <приёмник>...<исходник> | git apply --index`) под сдержанным сообщением (`disc-squash-says-it-is-squashed`, `disc-hash-exception-when-rewritten`) и с доказательством сохранности (`disc-preservation-proof`), вливание ЭТОЙ ветки коммитом слияния, исходник снимается на origin и во всех клонах; PR, открытый из исходника, держит его коммиты ссылкой `refs/pull/<N>/head` и после снятия ветки · ЗАВЕСТИ disc-rewrite-history-needs-owner · red: force-push по своему решению; раскрывающий исходник влит коммитом слияния; изъятие объявлено полным при PR из исходника

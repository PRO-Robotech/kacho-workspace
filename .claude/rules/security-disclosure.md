...
disc-rewrite-history-needs-owner · переписывать историю публичной ветки принудительно — только с разрешения владельца; сквоша как обхода нет (2026-09-22): сообщения ветки переписываются до вливания, деревья равны, `--force-with-lease` на измеренную голову, `main` не переписывается (`git-issues.md#gi-transition`) · ЗАВЕСТИ disc-rewrite-history-needs-owner · red: force-push по своему решению; `--force` без lease
disc-squash-says-it-is-squashed · тело коммита переноса (cherry-pick) обязано нести «перенесено из <ветка>, исходные коммиты <sha>»; сквош запрещён с 2026-09-22, у влитого им раньше та же строка «схлопнуто из …» · ЗАВЕСТИ disc-squash-says-it-is-squashed · red: отсутствие хеша читается как потеря работы

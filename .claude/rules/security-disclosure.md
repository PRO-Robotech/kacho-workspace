...
disc-preservation-proof · знаменатель — пути ВСЕЙ полосы: git diff --name-only $(git merge-base <база> <голова>)..<голова>, база — ствол ДО слияния (для коммита слияния `<итог>^1`); добавленное присутствует, снятое не уцелело, непустое расхождение не вердикт · scripts/preservation-proof.sh · red: знаменатель — файлы головного коммита, потеря вне их молчит

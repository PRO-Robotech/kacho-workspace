...
| `terraform/` | Terraform-провайдер — **отдельный Go-модуль**, в граф продукта не входит | — |
...
> (части продукта — пакеты одного модуля; второй модуль дерева, провайдер, продукта не
> импортирует), и держать его как описание действительности — значит выдавать намерение за
> факт. Именно поэтому раздел ниже помечен, а не удалён.

## Build-граф — ОДИН Go-модуль (перемерено 2026-08-13)

Замер: `find . -name go.mod` в дереве продукта даёт **1** файл — корневой; `replace` на
внутренний модуль — **0**. Провайдер живёт **пакетами того же модуля**:
`go list ./terraform/...` возвращает четыре пакета
(`terraform`, `terraform/cmd/terraform-provider-kacho`, `terraform/internal/client`,
`terraform/internal/provider`).

- `github.com/PRO-Robotech/kacho` — весь продукт, включая провайдер. Внутренних
  версионированных зависимостей между частями нет by construction: `proto/` → `pkg/` →
  `services/*` / `gateway/` / `terraform/` — пакеты одного модуля, и порядок между ними —
  порядок импортов, а не пинов.
- Типы контракта приезжают провайдеру **вторым выходом генерации** из общего `proto/`
  (`proto/buf.gen.terraform.yaml`) под собственным префиксом пакета — это осталось верным.

> [!warning] Здесь стояло «ДВА Go-модуля (факт)» с гейтом изоляции — ни того, ни другого в дереве нет
> Прежняя редакция объявляла провайдер отдельным модулем, называла причину (дерево
> `terraform-plugin-framework` велико и вошло бы в граф каждого сервиса), приводила замер
> корневого `go.sum` и ссылалась на гейт `TestProviderModuleIsIsolated` с шестью условиями.
> Перемерено на дереве 2026-08-13, предикат с контролем в обе стороны:
>
> - `terraform/go.mod` — **нет** ни в ветке, ни в точке интеграции (`git cat-file -e`);
> - гейта с таким именем в дереве **ноль** (`git grep -li 'IsIsolated' -- '*_test.go'`),
>   при том что тот же предикат находит **восемь** существующих гейтов `artifactgates` —
>   то есть контроль пройден в обе стороны и «ноль» означает отсутствие, а не слепоту.
>
> **Практическое следствие, ради которого это правится, а не удаляется:** прежняя редакция
> предписывала провайдеру **отдельную джобу** из пяти команд, потому что «корневой `./...`
> во вложенный модуль не спускается». Сегодня спускается: корневой прогон анализатора
> безопасности читает **31** файл провайдера, а `go list ./...` включает его пакеты.
> Инженер, поверивший прежней норме, либо завёл бы лишнюю джобу, либо — что хуже — счёл бы
> провайдер непроверенным и добавил подавления «потому что его всё равно никто не смотрит».
>
> Историческая часть, которая остаётся верной: **разделение задумывалось ради изоляции**
> зависимостей провайдера от графа сервисов. Если её когда-нибудь заведут снова — правило
> ниже про `replace` вступает в силу как есть, а этот раздел правится вместе с деревом,
> а не после него.
...
- **CI-гейт:** `! grep -rnE '^replace github.com/PRO-Robotech' project/*/go.mod` (пусто = OK).
  На монорепо гейт **тривиально зелёный** (ноль `replace` в обоих `go.mod`) — и это не «он
  работает», а «ему нечего рассматривать»: не считать его прохождение свидетельством.

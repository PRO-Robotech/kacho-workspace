...
Замер 2026-08-12: `go.mod` в дереве **1**, модуль `github.com/PRO-Robotech/kacho`;
`replace` на внутренний модуль — **0**. Внутренних версионированных зависимостей между
частями продукта нет by construction: `proto/` → `pkg/` → `services/*` / `gateway/` /
`terraform/` — это пакеты одного модуля, и порядок между ними — порядок импортов, а не
пинов.

> [!note] Здесь недолго стояло «ДВА модуля» — провайдер Terraform сведён обратно
> Провайдер заводился отдельным модулем ради изоляции дерева зависимостей
...
> только импортируемое. За экономию сорока пяти строк платили вторым выходом генерации,
> managed-режимом `buf`, гейтом изоляции из шести условий и второй джобой конвейера.
>
> Сведение сняло всё это разом: типы берутся из `pkg/api` напрямую, устаревать нечему;
> провайдер пользуется общими библиотеками (`pkg/backoff`, `pkg/ids`), а не своими
> копиями; его Go-код проверяют те же джобы, что и весь продукт.
>
> **Почему `terraform/`, а не `services/terraform/`.** Каталог сервисов имеет в этом дереве
> строгий смысл, и он кодифицирован: его перечисляют **14** гейтов, требующих gRPC-листенер
> и раскладку use-case. Провайдер — клиент платформы, а не её сервис; попытка положить его
> туда немедленно покраснила два гейта. Тот же выбор сделан и в `sgroups-legacy`, откуда
> практика перенята: провайдер живёт рядом с сервисами, а не среди них.
...
- `kacho-registry → kacho-iam` (jwks-fetch) — **HTTPS GET публичного JWKS** с cluster-internal iam-эндпоинта
  ...
  **Замещает** прежний прямой fetch публичного JWKS Hydra: iam — short-TTL кэширующий reverse-proxy
  ...
  > и ребро вне перечня не участвует ни в проверке ацикличности, ни в разборе при отладке.
  >
  > 1. **Набор ключей перестал быть только чужим зеркалом.** Публикатор отдаёт записи ПО
  >    ИЗДАТЕЛЯМ: наша — проекция ключницы iam (только наши ключи), запись зеркала прежнего
  >    провайдера сохраняется до последней фазы отказа от внешнего OAuth-сервера. У каждой
  >    записи СВОЙ объявленный путь; потребитель выбирает запись по объявленному издателю
  >    токена, а издатель без записи даёт отказ — никогда перебор записей и никогда путь,
  >    выведенный из самого издателя.
  ...
  >    предъявленное продолжает проходить до истечения срока, и это состояние не сходится
  >    само. Авторитет отвечает по форме, которую понимает всякий спрашивающий; недоступность
  >    авторитета — **fail-closed**, потому что неполученный ответ не есть «да».
  >
...
- `kacho-{vpc,compute,storage,nlb,registry} → kacho-iam` (величины пределов, #656) —
  `iam.v1.InternalLimitService` на internal-only :9091. Ребро **одно, а полос у него ДВЕ**, и
  различать их обязательно, потому что недоступность соседа означает на них разное:
  - **`Resolve` — sync на request-path** мутации: домен спрашивает разрешённые величины по всем
    видам своего каталога перед списанием. Перечень видов приезжает **ответом**, а не
    спрашивается по одному — каталог принадлежит платформе, и владелец типа копии не держит.
    Per-call deadline + `retry.OnUnavailable`, личность инициатора пробрасывается в исходящие
    метаданные (решение сосед принимает по служебной учётке владельца, состоящей в группе
    читателей квот). Недоступность — **fail-closed**: мутация отвергается `UNAVAILABLE`,
    повтор осмыслен.
  - **`ListChangedSince` — фоновая дельта**, которой снимок величин догоняет владельца
    (`pkg/quota`, тянущий у каждого владельца свой). **НЕ на пути запроса.** Отказ здесь
    **не фатален и не тих**: снимок постоит со старой величиной, а предел продолжает
    действовать — место занимает триггер в writer-транзакции. Ронять сервис из-за отставшего
    снимка значило бы менять ограниченное отставание на полный отказ.

  **Тянет ВЛАДЕЛЕЦ, а не толкает сосед** — и это не стиль, а следствие ацикличности: iam —
  лист, толчок к домену был бы ребром обратно, то есть циклом. Ацикличность проверена
  предикатом, а не объявлена: `git grep -l 'vpcv1\.\|computev1\.\|storagev1\.\|nlbv1\.\|registryv1\.'
  -- 'services/iam/internal/**' ':!*_test.go'` → **пусто**.

  **Ребро записано именно потому, что оно НИЧЕГО не нарушает.** iam объявлен листом («их зовут,
  они не зовут никого»), пять входящих рёбер этого не отменяют — но перечень рёбер есть
  единственное место, где видно, кто кого зовёт на пути запроса, и следующий, кто захочет
  добавить вызов ИЗ iam, обязан увидеть, чем он рискует. Ребро вне перечня не участвует ни в
  проверке ацикличности, ни в разборе при отладке.

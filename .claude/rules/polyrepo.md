...
## Build-граф — ДВА Go-модуля: продукт и провайдер (факт)

Замер 2026-08-12: `go.mod` в дереве **2**, `replace` на внутренний модуль — **0**.

- `github.com/PRO-Robotech/kacho` — **продукт**. Внутренних версионированных зависимостей
  между его частями нет by construction: `proto/` → `pkg/` → `services/*` / `gateway/` —
  пакеты одного модуля, и порядок между ними — порядок импортов, а не пинов.
- `github.com/PRO-Robotech/kacho/terraform` — **Terraform-провайдер** (заведён под-фазой
  TF-1). Он **не импортирует** модуль продукта, поэтому `require` на него не нужен, а
  значит не нужен и запрещённый `replace`. Типы контракта приезжают **вторым выходом
  генерации** из общего `proto/` (`proto/buf.gen.terraform.yaml`) под собственным префиксом
  пакета.

> [!important] Второй модуль заведён РАДИ изоляции, а не вопреки правилу «один модуль»
> Дерево `terraform-plugin-framework` велико. В едином модуле оно вошло бы в граф **каждого**
> сервиса и каждой сборки образа — выросли бы время сборки, поверхность анализа уязвимостей
> и объём файла сумм. Замер на момент заведения: корневой `go.sum` — 362 строки, **ноль**
> `hashicorp`.
>
> Свойство держит гейт `internal/repohygiene/artifactgates` `TestProviderModuleIsIsolated`
> — шесть условий, включая зеркальное (ни один файл `pkg/api` не импортирует выход
> провайдера) и запрет отслеживаемого `go.work`: он молча меняет смысл `./...`, оставляя
> условие про `go.sum` зелёным.
>
> **Следствие для проверок:** корневой `./...` во вложенный модуль **не спускается**. У
> провайдера своя джоба, исполняющая те же пять модульно-ограниченных команд (`build`,
> `vet`, `golangci-lint`, `govulncheck`, `gosec`) — иначе «ноль находок» по новому коду
> означало бы «ноль осмотренного».

...
| `01-wave-contract.md` | раздача волны · заведение ревьюера · правка любого файла `.claude/rules/` | `scripts/rules-gate/` держит состав, а сумму набора на агента и перечень над ориентиром — его `check-06-preload-budget-census.sh` | `tooling-maintainer` · `wave-reviewer` |
...
| `security-disclosure.md` | публичный текст: коммит, issue, PR, vault · коммит в `docs/specs/reviews/**` и `docs/changes/<change-id>/reviews/**` · `squash` · `cherry-pick` | признак восстановимости (человеком, поимённо перед каждым коммитом) · доказательство сохранности содержимым | `acceptance-author` · `convergence-reviewer` · `docs-writer` · `git-operator` · `landing-reviewer` · `qa-test-engineer` · `security-auditor` · `vault-scribe` |

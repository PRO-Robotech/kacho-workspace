...
| `testing-newman.md` | правка `tests/newman/**`; сквозная проба через край; путь края; выкатка, тронувшая край | `assert-suites-green.sh`; `validate-cases.py`; реальная задержка поллера; пул под `--jobs` | `integration-tester`; `qa-test-engineer` |

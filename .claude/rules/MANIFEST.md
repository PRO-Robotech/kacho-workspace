...
| `security-disclosure.md` | публичный текст: коммит, issue, PR, vault; `squash`; `cherry-pick` | признак восстановимости (человеком); `scripts/preservation-proof.sh` (сохранность содержимым, знаменатель — вся полоса) | `acceptance-author`; `convergence-reviewer`; `docs-writer`; `git-operator`; `landing-reviewer`; `qa-test-engineer`; `security-auditor`; `vault-scribe` |

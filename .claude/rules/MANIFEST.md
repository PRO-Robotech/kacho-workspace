...
> [!warning] `rag.md` ВЫНЕСЕН В АРХИВ КАК РЕДКО НУЖНЫЙ — 2026-09-22
> **Потеря нормы, а не сжатие**: норма действует, инструмент на месте, замеры верны.
> **Причина переписана 2026-09-22, и прежняя названа ложной.** Сначала он был вынесен по
> пределу объёма корпуса; тем же днём предел перестал блокировать (`check-06` печатает
> объём наблюдением, решает `check-13`), и основание исчезло вместе с ним. Возвращать
> норму не стали: ценен не повод, а **механизм** — архив как адресуемый дом, доказанный
> на этом самом случае (норма ушла, адреса остались живы, `check-12` находок 0).
> Действительная причина: `rag.md` — **руководство по инструменту, нужное редко**, и его
> звали пять брифингов только по триггеру; в окне он не лежал и не должен.
> **Дом — `.claude/backup/rag.md`**, читается `Read`, не `Skill`: переходника нет.
> **Предикат возврата — не запас в символах, а ПОЯВЛЕНИЕ ЧИТАТЕЛЯ**, которому норма нужна
> целиком, а не по адресу: агент, читающий её в каждом заходе, а не при условии.

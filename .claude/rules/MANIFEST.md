...
| `security-disclosure.md` | публичный текст: коммит, issue, PR, vault; коммит в `docs/specs/reviews/**` и `docs/changes/*/reviews/**`; `squash`; `cherry-pick` | признак восстановимости (человеком, поимённо перед каждым коммитом); доказательство сохранности содержимым | `acceptance-author`; `convergence-reviewer`; `docs-writer`; `git-operator`; `landing-reviewer`; `qa-test-engineer`; `security-auditor`; `vault-scribe` |

...
| `testing-gate-cases.md` | предмет полосы — гейт, проба, страж · разбор чужого красного по классам дефекта проверки · перепись по двуязычному корпусу | инъекция в обе стороны · самоистечение послаблений · `internal/repohygiene` `TestCiRelaxationSaysWhyItIsThere` | `check-verifier` |
...
| `polyrepo-runtime-edges.md` | заведение/правка ребра рантайма между доменами (gRPC service→service) · перечень владельцев журнала · адреса соседей | обзор `system-design-reviewer` (ацикличность графа); машинного гейта на новое ребро нет — сказано в правиле | `system-design-reviewer` |

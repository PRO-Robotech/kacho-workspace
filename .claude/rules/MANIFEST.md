...
> [!warning] `rag.md` ВЫВЕДЕН ИЗ КОРПУСА ПО БЮДЖЕТУ ОКНА — 2026-09-22
> **Потеря нормы, а не сжатие**: норма действует, инструмент на месте, замеры верны. Причина
> одна — предел объёма: две полосы одной волны в 200 000 символов не помещались, и три
> бесплатных исхода измерены и исчерпаны (сжатие давало 251 при нужных 1 044; потолок
> поднимает владелец; разнос по волнам ничего не меняет — 199 008 + 1 195 при любом порядке).
> Выбран `rag.md`: единственный из одиннадцати восстановленных, чей предмет — руководство по
> ИНСТРУМЕНТУ, а не инвариант, и звали его пять брифингов только по триггеру.
> **Дом — `.claude/backup/rag.md`**, читается `Read`, не `Skill`: переходника нет.
> **Предикат возврата:** запас корпуса ≥ 1 923 + 841 = **2 764** символа (объём нормы-строки
> на момент снятия плюс машинный минимум близнеца `inject-06`, чья метка говорит 800).

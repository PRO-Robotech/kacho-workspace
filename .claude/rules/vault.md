...
vt-read-narrow-before-code · бери 1-2 узких записки: `resources/`, `rpc/`, `packages/`, `edges/`, `KAC/issue-<N>.md` · вниманием · red: в окне большой `CLAUDE.md` репозитория вместо двух записок

## ПОСЛЕ работы (обновить trail — НЕ упускать)

vt-update-touched-notes · обнови записку тронутого: поле и FK ресурса, RPC, exported API пакета, cross-service поведение с History и номером задачи · vault-gate run-all.sh · red: поведение изменилось, записка описывает прежнее
...
vt-trail-per-issue · заводи `KAC/issue-<N>.md` при первом упоминании: Status, Type, каталоги, PR, «Что и зачем», затронутые сущности, DoD · docs-gate check-02-kac-trail-status.py · red: задача закрыта без trail

...
- **Vault ПУБЛИЧЕН — пишем так, чтобы выдерживало публикацию.** `kacho-workspace` — PUBLIC-репо,
  ...
  коммитится вовсе** — рабочий каталог задачи, а в vault идёт высокоуровневая часть со ссылкой
  на коммит фикса. Внутреннего назначения в git не существует **по построению** — полное правило,

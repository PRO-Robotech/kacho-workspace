...
- **Отчётность идёт в issue**, не только в код: закрывающий комментарий несёт PR-URL / commit-SHA +
  что именно сделано (high-level; operational-детали — в internal vault, см. `security.md` про
  public-артефакты). Симметрично KAC-trail и YT `Done`-переходу (артефакты в комментарий).

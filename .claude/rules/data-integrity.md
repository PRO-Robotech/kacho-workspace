...
5. **Карта владельцев**: Geography (Region/Zone) → `kacho-geo`; IAM (Account/Project/User/SA/Group/Role/AccessBinding) → `kacho-iam`;
   ...
   **Volume/Snapshot/Image/DiskType → `kacho-storage`** (блочное хранение; `volume_attachments` живёт у владельца-storage);
   ...
   >
   > **Данные переносить НЕ нужно (директива владельца 2026-07-27): облако не в проде.** Гейт
   > «счётчик строк на боевой базе перед удалением» — **снят**: тенантских данных нет, дубль
   > удаляется напрямую, а storage пишется правильно с нуля. Обоснование по коду это
   > подтверждает независимо: ни одна миграция не сеет ни одного тома/образа/снимка; связующая
   > таблица при расколе **дропнута, а не перенесена** (голый `DROP TABLE`, ноль `INSERT`) —
   > раскол изначально спроектирован без переноса; id-сохраняющая копия невозможна в принципе
   > (колонки происхождения — FK в строки storage, схемы и id-пространства несовместимы).
...
**Реальный инцидент (nlb, 2026-07-26):** в очереди регистраций сервиса **ни одна строка никогда
не была доставлена** — все 198 с отказом в правах. Два сцепленных дефекта: (а) намерение listener'а
несло только parent-link-отношение, которого нет в наборе (у соседних ресурсов — project, у
listener'а по недосмотру нет), при этом его собственный комментарий заявлял паритет с соседями;
(б) **каждое** создание несло вдобавок creator-privilege-отношение, зарезервированное за другим
потоком и тоже отвергаемое. Дренаж классифицирует отказ в правах как **временный** ⇒
короткое замыкание ⇒ строка никогда не помечается отправленной ⇒ claim партиции по голове
**блокирует последующие строки того же ресурса на ~150с** (MaxAttempts×backoff).
...
выглядело исправным. Плюс тот же класс на удалении: отзыв регистрации тоже отвергался ⇒
удалённый ресурс сохранял строку зеркала и **ре-материализованный грант** (over-grant).

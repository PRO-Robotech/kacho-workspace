...
- **Право на многих — ГРУППЕ, а не перечислением субъектов (B18).** Право, которое получает
  ...
  **группе**; люди и служебные учётки добавляются в группу. Перечисление субъектов в самой
  ...
  учётка модуля, чья личность и есть предмет выдачи). **Модели это не требует** — отношение
  членства уже принято каждым глаголом каждого типа (предикат: `awk '/define v_/{n++; if ($0 ~
  /group#member/) g++} END{print n, g}' proto/kacho/cloud/iam/v1/fga_model.fga` → `99 99`;
  зеркало «объявления **без** `group#member`» → `0`). Причина — **цена изменения состава**: снять
  одного из группы это одна строка членства, снять одного из перечисления — снятие кортежей по
  всем объектам роли. Порядок величин — **из замера XC-8**, не из дерева (харнесс
  `tools/authzformbench/` @ `ebedae536`, OpenFGA над Postgres, `N=1000, S=10, M=4`): выдача 8×,
  отзыв 171×, **кортежей** 10× меньше (по байтам — 7.4×, это разные величины); плата — до **37%**
  на худшем случае полной страницы чтения. **Поверхность выдачи при этом ПЕРЕМЕЩАЕТСЯ, а не
  исчезает:** «дать доступ» умеет ещё и тот, кто вправе править состав группы — путь гейтится
  **моделью** (глагол правки на объекте группы), самодельной проверки быть не должно
  (`security.md` §«Авторизация живёт в МОДЕЛИ»). **Вырожденный состав — отдельный предмет
  пробы:** группа без членов обязана давать доступ **никому**; пустое членство читается как
  «не получил никто», а не «условие нечем проверить» (тот же класс, что пустой allow-list —
  `security.md` §«Кто вправе ГОВОРИТЬ ЗА пользователя»). Правило действует **вперёд** — на наши
  посевы, фикстуры и приёмки; чужие привязки не переписываются, потолок «1..32 субъекта» не
  понижается. Сценарии, полная цена и критерии — `docs/specs/sub-phase-XC-9-grant-to-group-discipline-acceptance.md`
  (норма — §2.1, цена — §3, сценарии — §5); здесь она не пересказывается.

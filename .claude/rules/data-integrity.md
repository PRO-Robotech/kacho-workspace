...
6. Новое cross-domain ребро — фиксируется в `polyrepo-runtime-edges.md` (runtime-edge); циклы запрещены.
...
- **Связь «зона → её регион» берётся ТОЛЬКО резолвом у владельца, НИКОГДА не выводится из имени
  ...
  **Запрещены как приём**: отрезание суффикса зоны, срез по последнему дефису, префиксное сравнение
  ...
  деривация, **удалённая** из дерева; имя оставлено намеренно, оно связывает пункт с
  `polyrepo-runtime-edges.md` §runtime-edges и с приёмкой XC-1, где прецедент цитируется) снят 2026-07-25 и нёс три признака

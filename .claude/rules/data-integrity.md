...
db-fk-same-db · `FK REFERENCES <t>(id) ON DELETE {RESTRICT|CASCADE|SET NULL}` · ЗАВЕСТИ · red: проверка существования кодом
db-unique · `UNIQUE` / `CREATE UNIQUE INDEX` · ЗАВЕСТИ · red: SELECT-then-INSERT
db-partial-unique · partial `UNIQUE … WHERE <cond>` · ЗАВЕСТИ · red: полный UNIQUE или software-проверка
db-exclude-range · `EXCLUDE USING gist (… WITH &&)` · ЗАВЕСТИ · red: сравнение диапазонов в Go
db-check · `CHECK (…)` · ЗАВЕСТИ · red: валидация только в сервисе
...
db-select-for-update · `SELECT … FOR UPDATE` перед merge+write · ЗАВЕСТИ · red: merge без блокировки
...
no-unique-backstop · не заводи UNIQUE-индекс как backstop к CAS · ЗАВЕСТИ · red: UNIQUE, отвергающий законный multi-attach
...
warning-needs-removal-predicate · неси предикат снятия в устаревающем предупреждении · ЗАВЕСТИ · red: предупреждение без предиката снятия
...
compensation-reverse-order · обратный allocation (last-allocated → first-freed), каждый шаг идемпотентен · ЗАВЕСТИ · red: компенсация в прямом порядке
...
compensation-landing-phase2 · оба пути компенсации обязаны landing до Phase-2 compute (owner GA gated) · ЗАВЕСТИ · red: Phase-2 GA без обоих путей
...
placement-discriminator · несёт `placement_type ∈ {ZONAL(zone_id)|REGIONAL(region_id)}` взаимоисключающе, закреплено DB-CHECK: ZONAL — только zone_id, REGIONAL — только region_id · ЗАВЕСТИ · red: ad-hoc поля без дискриминатора
placement-anchor-subnet · каноничный — Subnet; NIC/Address зону не несут, наследуют через `subnet_id` · ЗАВЕСТИ · red: своя колонка зоны у NIC/Address
...
coherence-test · negative на zone/region mismatch → код + точный текст · anycast-ветка проходит · cross-family v4/v6 same-zone отдельным кейсом · ЗАВЕСТИ · red: только положительный путь
...
seed-separate-file · отдельный файл, вызываемый рецептом подъёма стенда · ЗАВЕСТИ · red: вставка в миграции
seed-on-conflict-no-target · `ON CONFLICT DO NOTHING` БЕЗ указания цели · ЗАВЕСТИ · red: указанная цель пропускает частичную уникальность и EXCLUDE

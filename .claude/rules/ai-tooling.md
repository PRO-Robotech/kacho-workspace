...
- **Единица счёта — ОТСЛЕЖИВАЕМЫЙ git-элемент, не то, что лежит на диске.** Наборы берутся
  ...
  `git ls-files .claude/rules/ | cut -d/ -f3 | sort -u | wc -l` (**16**),
  `git ls-files .claude/agents/ | wc -l` (**16**),
...
> Счёт: **16** generic-агентов (предикат: `git ls-files .claude/agents/ | wc -l` — не память,

...
at-window-load-measured-per-agent · меряй загрузку окна ПО АГЕНТУ — тело плюс правила его `skills:`; сумма корпуса адресата не имеет и остаётся наблюдением · check-13-agent-window-load.sh · red: решение принято по сумме, которую не грузит никто
at-window-threshold-awaits-basis · не назначай порог загрузки окна, пока основание не производится деревом или командой над ним; ручка `RULES_GATE_AGENT_WINDOW_MAX` включает блокировку без переделки · check-13 доказан инъекцией в обоих режимах · red: число, притворяющееся выведенным, поставлено вместо честно отсутствующего
at-empty-preload-named-by-word · называй словом, почему у агента ноль правил: `dispatcher` несёт базу вместо корпуса, `client-simulator` — пустой контекст предметом роли · check-13 (перечень штатных закрыт в разборщике) · red: штатный ноль слит с непрочитанным

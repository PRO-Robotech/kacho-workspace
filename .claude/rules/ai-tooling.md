...
at-settings-without-permissions · не коммитить блок `permissions`/`defaultMode: bypassPermissions` — его место в `settings.local.json` (git-ignored) · git show HEAD:.claude/settings.json \ · red: grep -c permissions = 0 | обход подтверждения принят за каждого, кто сделает клон
...
## Печатаемый сигнал хука: дельта, адресат, предикат снятия

at-signal-prints-delta · печатать ДЕЛЬТУ: у сигнала есть отпечаток прошлого тела, повтор без дельты — одна строка «без изменений с <sha>@<время>» · scripts/tooling-gate/check-10-hook-signal-prints-delta.sh · red: тот же блок два хода подряд при неизменном дереве
at-signal-mechanism-single · признак дельты ОДИН — `.claude/hooks/lib/hook_signal.py`; своего внутри источника не заводить · check-10 · red: вторая реализация свёртки повтора в теле хука
at-signal-names-two · сигнал называет агента-адресата и МАШИННО проверяемый предикат снятия; без любого из двух не печатается · `hook_signal.gate` кодом 2, check-10 · red: «когда починят» вместо команды с исходом
at-signal-absent-print-full · отпечатка нет — первый ход сессии, новый id, снесённое состояние — печать ПОЛНАЯ · check-10 · red: потеря состояния прочитана как «уже говорили»
at-signal-empty-subject · печатается то, чей предмет НЕПУСТ; пустой обход даёт одну строку либо ничего, находки прочих осей он не отменяет · check-10 · red: полный экран при «осмотрено 0»
at-signal-census-survives · перепись объёма уходит в журнал безусловно, включая молчаливые ходы · check-10 · red: свёрнут повтор и вместе с ним снята перепись
at-signal-no-inject-on-repeat · канал, поднимающий агента (`additionalContext`), получает только полную печать · check-10 · red: петля «ход → впрыск → ход» на повторе

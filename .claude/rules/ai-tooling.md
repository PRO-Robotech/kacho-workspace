...
`scripts/skills-gate/run-all.sh` на 2026-08-06: перечень называет **12** имён, отслеживаемых
директорий в дереве **13**; тринадцатая — `godzila` (подраздел ниже: почему она отслеживается
и что открыто), и `check-03` остаётся на ней красным намеренно. `<svc>-load-testing` живёт в
репо сервиса и в этот счёт не входит.
...
### Отслеживается git, но в перечне выше НЕ назван — `godzila`

`.gitignore` исключает **пять** директорий под заголовком «externally-installed skills (not
part of project's intentional skill set)» — `defuddle`, `json-canvas`, `obsidian-bases`,
`obsidian-cli`, `obsidian-markdown`. Шестой строкой там стоял `godzila`, и она **снята**:
комментарий над списком называет признак принадлежности (предмет, а не происхождение) и
прямо говорит, что `godzila` из списка выведен. Сегодня скил **отслеживается** —
`git check-ignore` его не признаёт, `git ls-files` показывает.

> [!note] Здесь стояло «раскатывается по всем репо» и «неверсионированный» — оба неверны
> Первое противоречило **этому же файлу**: §«Модель распространения» на два экрана выше
> объявляет, что раскатки как механизма не существует (решение владельца 2026-08-02), —
> два места об одном предмете, из которых верно одно. Второе устарело от правки
> `.gitignore`: скил отслеживается. Абзац дописывался по частям и обе неточности пережили
> свой предмет молча, потому что расхождение прозы с деревом не даёт конфликта при мёрже.
...
- **`godzila`** — сборник **готовых шаблонов кода** kachō-стиля: тело мутирующего use-case с
  ...
  новый use-case · новая миграция · новый вызов соседа. Его §20 — про **этот** воркспейс
  (пара с `evgeniy`, наши сабагенты, порядок загрузки), а приоритет он объявляет сам:
  нормативная регуляция > локальный `CLAUDE.md` > `godzila`, и на конфликте выигрывает
  `evgeniy`. Шов с `code-authoring` проговорён в его §0 и §10: окно срабатывания у них одно,
  предметы разные — «какой код здесь пишут» против «что этот код будет утверждать».
- **Что остаётся открытым и почему это видно**: скил в дереве есть, в перечне выше его нет,
  поэтому `scripts/skills-gate/check-03-roster-matches-tree.sh` красный и **называет имя**.
  Решение — за владельцем: внести в перечень (и тогда `godzila` становится тринадцатым
  каноническим) либо вынести из дерева. Красное здесь — не долг оформления, а единственное,
  что мешает расхождению стать невидимым.

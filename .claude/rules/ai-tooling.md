...
at-settings-without-permissions · не коммитить блок `permissions`/`defaultMode: bypassPermissions`; харнесс игнорирует его и в проектном слое, и в локальном (`.claude/settings.local.json`, git игнорирует) — оба repo-controllable — режим даёт только личный `~/.claude/settings.json`, policy или флаг · scripts/rules-gate/check-09-settings-no-bypass.sh · red: `grep -c permissions .claude/settings.json` не ноль — обход подтверждения принят за каждого, кто сделает клон

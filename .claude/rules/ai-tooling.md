...
- `landing-reviewer` — запускается ПЕРЕД посадкой (коммит, слияние, cherry-pick, push, закрытие тикета) и при…
...
lc2-issue-branch-trail · фича — issue + ветка `<N>` (номер задачи) + trail в vault · docs-gate check-02 · red: код без issue, ветки или trail

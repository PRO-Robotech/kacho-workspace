...
**Архив** — доводы, замеры, снятые редакции; **НЕ действующая норма**, цитировать как норму нельзя: `.claude/backup/ai-tooling.md`
...
at-ceiling-raise-carries-basis · подъём потолка базы и протокола законен только с названным предметом и тем, что без него станет невозможно; новое значение НЕ равно размеру написанного — запас не меньше медианного абзаца файла; подъём и правка ради него — ОДНО изменение · scripts/rules-gate/check-10-ceiling-raise-carries-its-basis.sh · red: основание пережило свой подъём; запас впритык; подъём «про запас» отдельным изменением
...
at-load-target-resolves · имя, которое тело агента велит ЗАГРУЗИТЬ, существует в дереве — в трёх законных формах: `skills:`, `Skill <имя>`, голое имя в кавычках; вызов несуществующего отвечает `Unknown skill: <имя>` и агент об этом УЗНАЁТ (измерено 2026-09-21), но ошибка не попадает ни в гейт, ни в возврат, а триггер условный — поэтому разрыв живёт сутками и судит его ДЕРЕВО · scripts/tooling-gate/check-14-agent-load-targets-resolve.sh · red: `Skill <имя>` без каталога; предзагрузка без каталога; скил без читателя
at-contract-copy-agrees · блок контракта ВОЗВРАТ в теле агента совпадает с `CLAUDE.md` по СОСТАВУ и ПОРЯДКУ полей; значения у копии свои — это законно; агенту с пустым окном копия И ЕСТЬ контракт · scripts/tooling-gate/check-15-contract-copies-agree.sh · red: копия отстала на поле; лишнее поле; порядок разошёлся

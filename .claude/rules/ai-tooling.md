...
at-judged-cannot-exempt-itself · исключение по роли не выдаётся файлу из судимых домов (`.claude/rules/`, `.claude/agents/`) и файлу, который не исполняется: иначе корпус или агент изымает себя из переписи одной строкой прозы — в разметке `#` заголовок, а не комментарий · `scripts/rules-gate/check-10-rule-address-exists-as-written.sh`, пары 2 и 3 в `inject-10-rule-address-exists-as-written.sh` · red: перепись висячих упала, а сломанная привязка правила к агенту жива

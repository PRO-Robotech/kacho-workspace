...
at-settings-without-permissions · не коммитить блок `permissions`/`defaultMode: bypassPermissions`; харнесс игнорирует его и в проектном слое, и в локальном (settings.local.json, git-ignored) — оба repo-controllable — режим даёт только личный `~/.claude/settings.json`, policy или флаг · git show HEAD:.claude/settings.json | grep -c permissions = 0 · red: блок permissions в отслеживаемом файле — обход подтверждения принят за каждого, кто сделает клон
at-bypass-not-from-repo-layer · `defaultMode: bypassPermissions` в проектном слое не просто запрещён — он НЕ РАБОТАЕТ: сессия встаёт в `auto` и спрашивает подтверждения · claude --debug-file <лог> -p … → нет строки `settings defaultMode "bypassPermissions" ignored`; `[session-notices] mode=bypassPermissions` (измерено 2026-09-21, CC 2.1.278) · red: объявление режима лежит в дереве, подтверждения продолжают приходить

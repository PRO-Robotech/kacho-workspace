...
**Механика, наблюдавшаяся вживую.** Единый `actions/cache` сохраняет каталог в post-шаге при
ЛЮБОМ исходе job'ы. Прогон, где установка не доехала, положил в кэш недокачанное; следующий
получил попадание, пропустил установку по `if: cache-hit != 'true'` и упал на пробах.
Починка: `cache/restore` и `cache/save` **раздельно**, сохранение — по признаку полноты,
выставленному самим шагом; шаг идёт всегда (установка идемпотентна и при готовом предмете
выходит мгновенно).

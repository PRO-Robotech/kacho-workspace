...
ban19-review-round-one · все блокирующие — в первом круге; круг N+1 судит лишь дельту и закрытие круга N, новое на нетронутом — задача строкой «нужен следующий: git-operator», не ⛔ (кроме безопасности) · вниманием рецензента · red: ⛔ круга N+1 по строке, не тронутой с круга N
ban19-review-diff-only · блокируй с предикатом и координатой только предмет диффа (кроме безопасности) · вниманием рецензента; ⛔ без предиката диспетчер круга не открывает · red: ⛔ вне диффа либо без команды и файл:строки
ban19-number-two-predicates · число о дереве — двумя предикатами, по написанию и по предмету, с выводом обоих · вниманием автора; приёмщик возвращает число без второго вывода · red: число одного grep разошлось с предметом

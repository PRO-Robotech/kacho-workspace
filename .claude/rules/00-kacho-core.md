...
core-no-prod · прода с пользователями нет до объявления владельцем (решение владельца 2026-09-22): переходный режим, двойная поддержка старого и нового, обратная совместимость, миграция пользовательских данных не требуются — старое снимается сразу, кратчайшим путём; полнота, корректность, безопасность (authn/authz — высшая критичность) не ослабляются · вниманием диспетчера и рецензентов · red: любое из четырёх потребовано; «прода нет» — довод урезать полноту, пробу или безопасность

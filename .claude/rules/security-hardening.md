...
**Архив:** `.claude/backup/security-hardening.md`

## Hardening-инварианты — не повторять эти классы
...
hard-no-pii-in-logs · не писать email/phone/token/личные данные end-user; коррелировать по external_id; исключение — единичный operational-config · ЗАВЕСТИ · red: PII в аргументе log.Info/Error
...
hard-hide-existence-test-asserts-message · проба локает текст deny и текст miss ПОБАЙТНО, а не «содержит» · TestHideExistenceFormats_MatchTheGateway · red: тексты разошлись с краем, а проба зелёная
...
## Контроль, действующий на ВЫДАЧЕ, но не на ПРЕДЪЯВЛЕНИИ
...
hard-parts-must-converge · проверять, про один ли предмет они говорят; исправность части ничего не доказывает о целом · ЗАВЕСТИ · red: две пробы по половине
hard-probe-through-both-sides · держать пробой сквозь обе (записали отзыв → предъявили удостоверение → получили отказ) · ЗАВЕСТИ · red: две пробы, каждая про свою половину
...
hard-narrow-before-removing · сначала выразить ограничение в модели/данных, потом снимать · ЗАВЕСТИ · red: обратный порядок

## Авторизация живёт в МОДЕЛИ, а не в самодельных проверках (директива владельца 2026-07-27)

hard-no-hand-rolled-authz · принимать моделью прав (каталог + per-object Check); хардкод-условие в Go запрещено · ЗАВЕСТИ · red: самодельная проверка поверх работающего Check
hard-policy-as-relation · выражать глаголом от отношения владельца в модели · ЗАВЕСТИ · red: условие в Go вместо отношения
hard-machine-gets-grant · давать узкий грант на конкретные объекты/тип, не бланкетный админ; принципал первоклассный · ЗАВЕСТИ · red: роль-на-всё для машины
hard-async-carries-initiator · нести личность инициатора, захваченную в момент запроса · ЗАВЕСТИ · red: воркер под своей учёткой
hard-no-token-impersonation · не держать токен человека; делегирование — отдельный протокол (согласие, область, срок) · ЗАВЕСТИ · red: user-токен в окружении воркера
hard-audit-writes-both · нести и актора, и от чьего имени действовали · ЗАВЕСТИ · red: одно поле принципала
...
hard-cascade-only-top-three · заводить только для облака, бутстрапа и аккаунта; проект и ниже — нет · ЗАВЕСТИ · red: каскад на уровне проекта
hard-establish-relation-before-adding · установить фактическое отношение прежде добавления вывода · ЗАВЕСТИ · red: отношение изобретено ради синтаксиса
...
hard-regression-observable-level · утверждать на наблюдаемом уровне: администратор облака снимает чужую выдачу, обычный тенант — нет · ЗАВЕСТИ · red: проба на уровне tuple вместо API
...
hard-asymmetry-with-peer-consumer · превращать в замер сравнением с соседом-потребителем той же службы · ЗАВЕСТИ · red: вывод без сравнения
...
hard-missing-knob-vs-unseeded · спросить, существует ли ручка: нет ручки ⇒ дефект продукта на все профили; есть, но не задана ⇒ чинится посевом · ЗАВЕСТИ · red: дефект кода чинят посевом

...
5. **Карта владельцев**: Geography (Region/Zone) → `kacho-geo`; IAM (Account/Project/User/SA/Group/Role/AccessBinding) → `kacho-iam`;
   ...
   **Volume/Snapshot/Image/DiskType → `kacho-storage`** (блочное хранение; `volume_attachments` живёт у владельца-storage);
   ...
   > - proto-сервисов у compute на момент того замера оставалось **четыре**; **перемер
   >   2026-08-29 даёт восемь**, и одного из четырёх названных нет вовсе — прежняя
   >   редакция называла среди живых службу частной подписки, снятую эпиком единого
   >   потока изменений (её имя здесь не воспроизводится в форме координаты: цитата
   >   снятого имени читается проверкой свежести как живое утверждение). Перечень
   >   **выводится**, а не выписывается — он вырос вчетверо за три недели и устареет

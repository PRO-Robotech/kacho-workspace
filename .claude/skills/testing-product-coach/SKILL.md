...
### 1.2 Код предмета автор newman не читает

Вход автора кейса края — приёмка, контракт (proto, конвенции API), публичная документация, адрес
края (`testing-newman.md#edge-author-black-box`, решение владельца 2026-09-23). Скоуп, наличие LRO,
угол негатива — из контракта: тип ответа и комментарий поля в `.proto`, коды и тексты конвенций.
Чего контракт не говорит — вопрос к приёмке (`acceptance-author`), а не чтение реализации.
...
| Код | — | автору newman не выдаётся (§1.2) |
| Существующие тесты | Низкий | Не повторять, а дополнять |

Если acceptance-документ и код расходятся — это либо баг в коде, либо
устаревшая спека. Решение принимает ДИСПЕТЧЕР (2026-09-20), не тестировщик и не владелец.
...
## Часть X. Код предмета — вне входа автора newman

### 10.1 Вход — только контракт

Спорное поле и LRO — по `.proto` (тип ответа, комментарий поля); угол негатива — по контракту и
конвенциям API; полнота покрытия — по своду классов (`testing-newman.md#qa-classes-per-method`),
а не по веткам кода. Код предмета — нет (§1.2).
...
| Переутверждать unit-свойство, невидимое на крае | Два места об одном предмете; сверка ответа края с интерфейсом — не это, а двойная проверка (решение владельца 2026-09-23) |
...
### 11.2 Словарь классов

Словарь `classes` — `testing-newman.md#qa-class-label`; форма id и классы по методам —
`docs/TAXONOMY.md` набора: `<ДОМЕН>-<МЕТОД>-<КЛАСС>-<ДЕТАЛЬ>`, например
`DISK-CR-BVA-SIZE-BELOW-MIN`. Второй таблицы классов здесь нет.

...
2. **Факты — только из ground-truth.** Не выдумывать и не писать по памяти. Источники
   по приоритету: корпус правил `.claude/rules/*` воркспейса →
...
> поэтому «8» было верно до выноса и стало ложным после, а не было опиской. Замер
> повторяется за минуту:
> `git ls-tree -r --name-only <ревизия> | grep -c 'docusaurus\.config\.ts$'`.
>
> Следствие, которое стоит знать до правки этой строки: документации **компонента iam**
> в этом дереве больше нет вовсе — под `services/` шесть служб, iam среди них не
> числится. Искать её здесь бессмысленно; её дом — репозиторий её продукта.

// ОТОЗВАН 2026-09-17 — НЕ исполняемый путь раунда.
//
// Решением владельца от этой даты последовательность агентов выставляет только диспетчер, а
// исполнитель не запускает исполнителя (`disallowedTools: Agent` у каждого агента; корневой
// `CLAUDE.md` §«Модель загрузки»). Раунд теперь ведётся так: SKILL.md §2 (шаги диспетчера) и
// §4 (роли — существующие агенты security-auditor / go-implementer / git-operator / ci-watcher).
// Формулировки ролей живут в §4 скила, а не здесь: этот файл источником промтов больше не
// является и правкой промтов не поддерживается.
//
// Файл оставлен в дереве потому, что на его путь ссылается перепись вложенных файлов пакета
// скила в `scripts/change-graph-gate/`; снятие — отдельным заданием `tooling-maintainer`.
export const meta = {
  name: 'hardening-audit-round',
  description: 'ОТОЗВАН 2026-09-17 (см. комментарий выше и SKILL.md §2): один раунд аудит-петли Kachō — deep-finder на область (6 измерений) → опровергающая верификация → строгий TDD-fix + push ветки. Петлю ведёт диспетчер последовательностью агентов, а не этот скрипт.',
  phases: [
    { title: 'Find' },
    { title: 'Verify' },
    { title: 'Fix' },

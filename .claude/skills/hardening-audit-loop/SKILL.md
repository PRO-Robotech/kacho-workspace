...
- **FIXER** (phase Fix, per-repo): **строгий TDD** — падающий regression **по нужной
  ...
  **наблюдаемого** (scrubbed principal; retriable code, не PermissionDenied; per-call
  ...
  gRPC-код (`testing.md`). Doc-truth: комментарий И код к корректному поведению. False-
  positive/cross-repo — пометить, не форсить. Ветка `fix/audit-rN`, push, PR не открывать.
...
- **Severity-разбивка**: считать `H HIGH + M MEDIUM` подтверждённых; LOW = 0 by-design
  (§4 verify-фильтр). Честно назвать число, не выдавать LOW-охват за то, чего не было.

...
if [ "$examined" -eq 0 ]; then
    ...
    echo "     → scout: почему копии не читаются. Это «не проверяли», а не «чисто»." >&2
    exit 0
fi
...
if [ "${#findings[@]}" -gt 0 ]; then
    ...
    printf '  · %s\n' "${findings[@]}" >&2
    echo "  → git-operator, ДО любой отправки: bash <корень>/scripts/hooks/install.sh install" >&2
    echo "    (в монорепо есть и цель: make install-hooks)." >&2
    echo "  Пока он не вернулся, отправка не назначается: pre-push здесь не исполняется," >&2
    echo "  и первым читателем кода снова станет конвейер." >&2
fi

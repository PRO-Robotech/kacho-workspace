...
# на 51 строку).
_B5_WINDOW = 25
# Форма-триггер вынесена в ИМЯ, потому что её читает не только предикат: харнесс
# доказательства находит по ней кандидатов в живом дереве. Литерал, выписанный в двух
# местах, разошёлся бы молча — и разошёлся бы именно там, где расхождение не видно
# (обе стороны отвечают «кандидатов нет»). Плюс харнесс, несущий этот литерал, сам
# становится кандидатом B5 и получает находку за то, что ищет находки.
B5_TRIGGER = "current-context"

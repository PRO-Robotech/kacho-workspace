...
ДО кода:
  ...
  • Задача #<N> в работе → открой obsidian/kacho/KAC/issue-<N>.md (создай если нет)
...
ПОСЛЕ работы:
  ...
  • Обнови KAC/issue-<N>.md «Затронутые сущности vault» + PR-URL + status

ЗАПРЕТЫ:
  • НЕ оставляй trail задачи без обновления после merge

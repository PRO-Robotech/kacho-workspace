...
# # Предмет
#
# Висяк — это ССЫЛКА, а не коммит. До 2026-09-22 здесь вливали схлопыванием:
# squash-коммит не имеет родителя-ветки, поэтому исходный коммит такой ветки не
# становится предком ствола НИКОГДА (с 2026-09-22 — только коммит слияния, и
# влитая ветка — предок; её ссылка всё равно висит до снятия). Пока локальная
# ветка такой коммит держит, `git log --all --graph` рисует

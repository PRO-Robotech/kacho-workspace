...
echo
echo "== Q. ствол — ОПУБЛИКОВАННАЯ линия; «вровень» только при РАВЕНСТВЕ =="
# Предмет (#280): полоса ствола читала ЛОКАЛЬНУЮ `main`, а она такой же артефакт
# рабочей копии, как и сам checkout, — двигается только когда в этой копии кто-то
# сделал fetch. Замер 2026-08-18 по `project/kacho`: HEAD `7ebb670d`, локальная
# `main` `1278df74`, `origin/main` `dfd2c027`. То есть «ствол», по которому
# выносился вердикт, сам отставал от ствола на 536 коммитов, а перепись в той же
# строке печатала «вровень». Две координаты, влитые в этот промежуток, были
# объявлены несуществующими.
Q_SAVED_REF="${DOCFRESH_INTEGRATION_REF-}"
unset DOCFRESH_INTEGRATION_REF

# --- Q1. Ствол выбирается НАИБОЛЕЕ ПРОДВИНУТЫЙ, и перепись называет ИМЕННО его -
#
# Ожидание ВЫВОДИТСЯ из дерева тем же вопросом, что задаёт хук, но независимой
# командой: рукописное «должно быть origin/main» устарело бы в первом же дереве
# без remote. Обе стороны существуют в любом клоне, включая мелкий (shallow)
# checkout конвейера: сравниваются РЕВИЗИИ уже существующих ссылок, история не
# нужна.
if [ -d "$WS/project/kacho/.git" ]; then
  q_best_ref=""; q_best_rev=""; q_best_n=-1
  for cand in origin/main origin/master main master; do
    git -C "$WS/project/kacho" rev-parse --verify --quiet "$cand^{commit}" >/dev/null 2>&1 || continue
    q_n="$(git -C "$WS/project/kacho" rev-list --count "HEAD..$cand" 2>/dev/null || echo 0)"
    if [ "$q_n" -gt "$q_best_n" ]; then
      q_best_n="$q_n"; q_best_ref="$cand"
      q_best_rev="$(git -C "$WS/project/kacho" rev-parse "$cand^{commit}" | cut -c1-8)"
    fi
  done
  if [ -z "$q_best_ref" ]; then
    notrun "в дереве продукта не резолвится ни один кандидат ствола — сверять выбор не с чем"
  else
    q_out="$(run_doc q1.md 'Живой путь — `sync-all.sh`.')"
    if printf '%s' "$q_out" | grep -qF "$q_best_ref $q_best_rev"; then
      echo "  ✔ (+) перепись называет наиболее продвинутый ствол ($q_best_ref $q_best_rev)"; PASS=$((PASS+1))
    else
      echo "  ✘ (+) перепись назвала НЕ тот ствол — судила по отставшей ссылке"; FAIL=$((FAIL+1))
      echo "      ожидалось: $q_best_ref $q_best_rev"
      printf '%s' "$q_out" | tr '·' '\n' | grep -aF 'дерево продукта' | sed 's/^/      /' | head -2
    fi
    # ЗАКОННЫЙ БЛИЗНЕЦ: сверка обязана ОТВЕРГАТЬ выдуманную ссылку, иначе
    # утверждение выше зеленело бы на любой строке.
    if printf '%s' "$q_out" | grep -qF 'origin/такой-ссылки-нет 00000000'; then
      echo "  ✘ (−) перепись совпала с выдуманной ссылкой — сверка ничего не значит"; FAIL=$((FAIL+1))
    else
      echo "  ✔ (−) выдуманная ссылка не совпадает — сверка различает"; PASS=$((PASS+1))
    fi
  fi
else
  notrun "дерева продукта нет — выбор ствола сверять не на чем"
fi

# --- Q2. «вровень» связано с РАВЕНСТВОМ РЕВИЗИЙ, а не с нулём отставания -------
#
# Признак #280 дословно: «дерево продукта <sha1>@HEAD — вровень со стволом main
# <sha2>», где sha1≠sha2. Отставание при этом НУЛЕВОЕ: копия стоит ВПЕРЕДИ своей
# локальной `main`. Значит проверять надо именно эту комбинацию — ноль позади при
# разных ревизиях.
#
# ЭТА ПАРА ЗОВЁТ ОТРИСОВКУ НАПРЯМУЮ, и это объявлено, а не умолчано. Сквозной
# вход потребовал бы ПРЕДКА HEAD, а в мелком (shallow) checkout'е конвейера
# предков нет вовсе: `DOCFRESH_INTEGRATION_REF=HEAD~1` там не резолвится, хук
# молча уходит на обычный путь, и проба зеленеет, ничего не проверив. Проба,
# которая не может упасть, хуже отсутствующей. Место отрисовки в переписи держит
# полоса O (перепись называет нынешний HEAD), а эта пара держит ПРАВИЛО СЛОВА.
q_render() { # q_render <ревизия HEAD> <ревизия ствола> → строка провенанса
  python3 - "$GUARD" "$1" "$2" <<'PY'
import importlib.util, sys
spec = importlib.util.spec_from_file_location("docfresh_probe", sys.argv[1])
mod = importlib.util.module_from_spec(spec)
spec.loader.exec_module(mod)
head, trunk = sys.argv[2], sys.argv[3]


class Carrier:  # носитель провенанса, ничего сверх него
    prov = {"ws": {"head": head[:8], "head_full": head, "branch": "проба",
                   "trunk_ref": "origin/main", "trunk_rev": trunk[:8],
                   "trunk_rev_full": trunk, "behind": 0,
                   "ahead": 0 if head == trunk else 1}}


print(mod.tree_provenance(Carrier()))
PY
}
q_ahead="$(q_render 1111111111111111111111111111111111111111 2222222222222222222222222222222222222222 2>&1)"
q_same="$(q_render 3333333333333333333333333333333333333333 3333333333333333333333333333333333333333 2>&1)"
if printf '%s' "$q_ahead" | grep -qF 'вровень'; then
  echo "  ✘ (+) копия ВПЕРЕДИ ствола названа «вровень» — расхождение выдано за согласованность"; FAIL=$((FAIL+1))
  printf '%s\n' "$q_ahead" | sed 's/^/      /' | head -2
else
  echo "  ✔ (+) разные ревизии при нуле отставания НЕ названы «вровень»"; PASS=$((PASS+1))
fi
if printf '%s' "$q_ahead" | grep -qE 'позади 0, впереди [1-9]'; then
  echo "  ✔ (+) расхождение названо ЧИСЛОМ в обе стороны"; PASS=$((PASS+1))
else
  echo "  ✘ (+) расхождение не названо числом — «по чему судили» не восстановить"; FAIL=$((FAIL+1))
  printf '%s\n' "$q_ahead" | sed 's/^/      /' | head -2
fi
# ЗАКОННЫЙ БЛИЗНЕЦ той же формы: при РАВЕНСТВЕ ревизий слово обязано остаться,
# иначе фикс просто вычеркнул бы его и утверждение выше стало бы вакуумным.
if printf '%s' "$q_same" | grep -qF 'вровень'; then
  echo "  ✔ (−) при равенстве ревизий «вровень» остаётся — слово не выхолощено"; PASS=$((PASS+1))
else
  echo "  ✘ (−) равенство ревизий тоже не названо «вровень» — предикат ловит форму, а не существо"; FAIL=$((FAIL+1))
  printf '%s\n' "$q_same" | sed 's/^/      /' | head -2
fi

# --- Q3. Ствол НЕЧИТАЕМ ⇒ вердикт НЕ ВЫНОСИТСЯ, а не выносится отрицательным ---
#
# «Смотрел не туда» и «этого нет» — разные утверждения. Без ствола второе
# недоказуемо, поэтому оно и не произносится; исход СЧИТАЕТСЯ и назван.
if absent_path "$DEAD_BOTH"; then
  QBODY="Копии генерируются \`$DEAD_BOTH\`."
  export DOCFRESH_INTEGRATION_CANDIDATES=""
  q_noref="$(run_doc q3.md "$QBODY")"
  unset DOCFRESH_INTEGRATION_CANDIDATES
  if printf '%s' "$q_noref" | grep -qF -- "$DEAD_BOTH"; then
    echo "  ✘ (+) при нечитаемом стволе вердикт вынесен ОТРИЦАТЕЛЬНЫМ — «не смотрел» выдано за «нет»"; FAIL=$((FAIL+1))
  else
    echo "  ✔ (+) при нечитаемом стволе координата не обвиняется"; PASS=$((PASS+1))
  fi
  if printf '%s' "$q_noref" | grep -qE 'вердикт НЕ ВЫНЕСЕН [1-9]'; then
    echo "  ✔ (+) невынесенный вердикт СЧИТАЕТСЯ и назван в переписи"; PASS=$((PASS+1))
  else
    echo "  ✘ (+) невынесенный вердикт не назван — молчание неотличимо от чистоты"; FAIL=$((FAIL+1))
  fi
  # ЗАКОННЫЙ БЛИЗНЕЦ: со стволом на месте та же координата обязана обвиняться,
  # иначе исход «не вынесен» выхолостил бы предикат целиком.
  expect_fires "та же координата при ЧИТАЕМОМ стволе" q4.md "$QBODY" "$DEAD_BOTH"
else
  notrun "'$DEAD_BOTH' появился в дереве — вход (+) полосы Q больше не настоящий"
fi
if [ -n "$Q_SAVED_REF" ]; then export DOCFRESH_INTEGRATION_REF="$Q_SAVED_REF"; fi

echo
echo "== R. покрытие основания ИЗМЕРЯЕТСЯ, а не подразумевается =="
# Предмет (#242): «не нашёл» было неотличимо от «не искал». Основание переменных
# читает закрытый перечень видов файлов; имя, объявленное вне него, объявлялось
# несуществующим, и в той же строке стояло «вне покрытия основания 0» — то есть
# УТВЕРЖДЕНИЕ ПОЛНОТЫ, которой нет. Первый экземпляр (`KACHO_SKIP_PREPUSH` в
# `scripts/hooks/pre-push`) закрыт добавлением вида в перечень; класс этим не
# закрывается — следующий вид повторит его молча.
#
# Вход СТРОИТСЯ сужением покрытия, а не поиском редкого вида файла в дереве:
# привязка к конкретному расширению умерла бы в тот день, когда его внесут в
# перечень, и проба перестала бы исполняться, ничего об этом не сказав.
r_name="$(git -C "${WS}/project/kacho" grep -hoE '\bKACHO_[A-Z0-9_]{2,}' -- '*.sh' 2>/dev/null \
          | sort -u | while read -r n; do
              git -C "${WS}/project/kacho" grep -qwE "$n" -- '*.go' 2>/dev/null || { printf '%s\n' "$n"; break; }
            done)"
if [ -z "$r_name" ]; then
  notrun "в дереве не нашлось имени KACHO_*, объявленного вне *.go — сузить покрытие не на чем"
else
  echo "  · вход выведен из дерева: '$r_name' объявлено вне *.go"
  export DOCFRESH_ENV_PATHSPECS="*.go"
  r_narrow="$(run_doc r1.md "Ручка — \`$r_name\`.")"
  r_fake="$(run_doc r2.md 'Ручка — `KACHO_NO_SUCH_HANDLE_ANYWHERE`.')"
  unset DOCFRESH_ENV_PATHSPECS
  if printf '%s' "$r_narrow" | grep -qF -- "$r_name"; then
    echo "  ✘ (+) имя вне покрытия объявлено несуществующим — «не искал» выдано за «нет»"; FAIL=$((FAIL+1))
  else
    echo "  ✔ (+) имя вне покрытия не обвиняется"; PASS=$((PASS+1))
  fi
  if printf '%s' "$r_narrow" | grep -qE 'вне покрытия основания [1-9]'; then
    echo "  ✔ (+) «вне покрытия» перестало быть тождественным нулю"; PASS=$((PASS+1))
  else
    echo "  ✘ (+) «вне покрытия» осталось нулём при непокрытом имени — утверждение полноты, которой нет"; FAIL=$((FAIL+1))
  fi
  # ЗАКОННЫЙ БЛИЗНЕЦ: сужение покрытия НЕ прощает всё подряд — имени, которого
  # нет нигде, оно не помогает.
  if printf '%s' "$r_fake" | grep -qF 'KACHO_NO_SUCH_HANDLE_ANYWHERE'; then
    echo "  ✔ (−) при том же сужении несуществующее имя ПО-ПРЕЖНЕМУ находка"; PASS=$((PASS+1))
  else
    echo "  ✘ (−) сужение покрытия простило и несуществующее имя — предикат выхолощен"; FAIL=$((FAIL+1))
  fi
  # ЗАКОННЫЙ БЛИЗНЕЦ 2: при ПОЛНОМ покрытии то же имя резолвится и молчит —
  # значит молчание (+) объясняется покрытием, а не тем, что имя вообще не судят.
  expect_silent "то же имя при полном покрытии" r3.md "Ручка — \`$r_name\`." "$r_name"
fi
# Перепись обязана называть ТРИ состояния раздельно: найдено · в дереве нет ·
# вне покрытия. Одно число вместо трёх сливает «нет» и «не смотрел».
r_states="$(run_doc r4.md 'Живой путь — `sync-all.sh`.')"
r_missed=0
for needle in 'резолвится ' 'в дереве нет ' 'вне покрытия основания '; do
  printf '%s' "$r_states" | grep -qF "$needle" || { echo "      нет отрезка: $needle"; r_missed=1; }
done
if [ "$r_missed" -eq 0 ]; then
  echo "  ✔ перепись называет три состояния координаты раздельно"; PASS=$((PASS+1))
else
  echo "  ✘ перепись не различает три состояния — «нет» и «не смотрел» слиты"; FAIL=$((FAIL+1))
fi

echo
echo "== S. имя ветки — НЕ путь; различает контекст, а не силуэт =="
# Предмет (#243): у пути в дереве и у имени ветки ОДИН силуэт
# (`docs/что-нибудь`), и проверка судила имя ветки как путь. Цена — не ложная
# находка, а ПРАВКА ПРАВИЛА ПОД ИНСТРУМЕНТ: в корпусе имя ветки писали словом,
# с оговоркой, объясняющей обход.
#
# Пара строится на ОДНОМ И ТОМ ЖЕ литерале: меняется только слово, которым
# документ им управляет. Значит проба меряет различитель, а не форму строки.
S_DEAD="docs/no-such-branch-or-chapter"
if absent_path "$S_DEAD"; then
  expect_fires "мёртвая координата дерева того же силуэта" s1.md \
    "Соседняя глава — \`$S_DEAD\`." "$S_DEAD"
  expect_silent "тот же литерал, названный ВЕТКОЙ" s2.md \
    "Ветка \`$S_DEAD\` снята вместе с задачей." "$S_DEAD"
  # Расширение решает РАНЬШЕ контекста: имя ветки не оканчивается на `.md`.
  # Без этой оговорки одно слово «ветка» в абзаце гасило бы любую находку рядом.
  expect_fires "мёртвый путь С РАСШИРЕНИЕМ в том же предложении про ветку" s3.md \
    "Ветка \`$S_DEAD\` правила \`docs/no-such-chapter.md\`." 'docs/no-such-chapter.md'
else
  notrun "'$S_DEAD' появился в дереве — вход полосы S больше не настоящий"
fi
# Перепись обязана называть, СКОЛЬКО фрагментов осмотрено и сколько не судилось
# по виду. «Не судилось 0» при непустом корпусе — расхождение, а не чистота:
# различитель, который ни разу не сработал, невидим.
s_out="$(run_doc s4.md 'Ветка `docs/no-such-branch-two` от `sync-all.sh`.')"
if printf '%s' "$s_out" | grep -qE 'инлайн-фрагментов осмотрено [1-9]'; then
  echo "  ✔ перепись называет объём осмотренных инлайн-фрагментов"; PASS=$((PASS+1))
else
  echo "  ✘ объём инлайн-фрагментов не назван — «ноль находок» неотличим от «ноль прочитанного»"; FAIL=$((FAIL+1))
fi
if printf '%s' "$s_out" | grep -qE 'имён веток [1-9]'; then
  echo "  ✔ придержанные по виду фрагменты СЧИТАЮТСЯ и названы"; PASS=$((PASS+1))
else
  echo "  ✘ придержанные по виду фрагменты не считаются — послабление невидимо"; FAIL=$((FAIL+1))
fi
if printf '%s' "$s_out" | grep -qE 'не судилось по виду [1-9]'; then
  echo "  ✔ «не судилось по виду» непусто на корпусе — различитель исполняется"; PASS=$((PASS+1))
else
  echo "  ✘ «не судилось по виду» пусто — различитель не исполнялся, а перепись молчит"; FAIL=$((FAIL+1))
fi

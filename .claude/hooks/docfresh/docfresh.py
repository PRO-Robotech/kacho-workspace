...
GIT_TIMEOUT_S = 30
# Расхождение двух ответчиков о корне — см. `workspace_root`. Печатается
# переписью; None означает «ответчики сошлись», а не «не спрашивали».
...
# ═══ 0a. Граница РАСПОЗНАВАНИЯ — вторая, и она НЕ та же самая ════════════════
#
# `COVERAGE_MODEL` выше отвечает на вопрос «координата распознана, но основание
# её не покрывает». Есть вопрос СТУПЕНЬЮ РАНЬШЕ, и до этой правки его не задавал
# никто: сколько вставок кода документ содержит ВООБЩЕ и у скольких из них вид
# координаты вообще определился.
#
# ПОЧЕМУ ЭТО ЗАВЕДЕНО. Перепись печатала «вне покрытия основания 0 из
# рассмотренных: path 0/29». Знаменатель 29 читался как «в документе 29
# координат», тогда как означал «29 штук я распознал»; про вставки, которых
# разбор не заявил ни одним видом, хук не говорил НИЧЕГО, и его ноль читался как
# полнота. Это ровно класс `kacho-workspace#762`: исход взят из одного канала,
...
# ТАЛЬЯ РАЗБОРА: у каждой вставки ровно один исход, исходы пересчитываются, и
# сумма обязана сойтись со числом вставок (`walk_mismatch`). Исходов три вида:
#   kinded    — вид координаты определён, дальше судит основание истины;
#   withdrawn — разбор отвёл вставку по НАЗВАННОЙ причине (образец, URL, фраза,
#               квалифицированный символ Go под отказанным `gosym`);
#   unknown   — ни одна ветвь разбора её не заявила: ВИД НЕ ОПРЕДЕЛЁН. Вердикта
#               по такой вставке нет ни зелёного, ни красного, и перепись обязана
#               сказать это числом — в том числе когда число ноль.
#
# Ключ тальи кладётся в результат `extract` РЯДОМ с видами и видом НЕ ЯВЛЯЕТСЯ:
# всякий, кто обходит `extract(...)`/`per_doc[...]` по видам, обязан отсеивать
# всё, чего нет в `KINDS` (обратный индекс, счёт координат, резолв).
WALK = "_walk"

# Причины отвода — ОБЪЯВЛЕНЫ здесь, а не рождаются строковым литералом в разборе:
# причина, которой нет в этом словаре, не была бы видна ни в переписи, ни в пробе.
WITHDRAWN_REASONS = {
    "route-sample": "образец маршрута (заполнитель в сегменте)",
    "url-or-phrase": "URL либо фраза (пробел внутри вставки)",
    "sample": "образец с заполнителем (`<…>`, `…`, `*`, `?`, `|`)",
    "gosym": "квалифицированный символ Go — предикат `gosym` отказан целиком",
}
# Исходы «вид не определён» — тоже поимённо, по МЕСТУ выхода из разбора, а не по
# догадке о содержимом: место выводится обходом, догадка — памятью автора.
UNKNOWN_EXITS = {
    "slashed": "со слэшем, но не форма пути дерева",
    "bare": "одиночный токен без известного расширения",
}
...
def workspace_root() -> Path:
    ...
    КОРЕНЬ НЕ БЕРЁТСЯ ИЗ ТЕКУЩЕГО КАТАЛОГА — ни здесь, ни запасным путём: запуск
    из чужой копии иначе молча судил бы чужое дерево. Ответчиков два, и они могут
    РАЗОЙТИСЬ: переменная сессии и собственное расположение файла. Разойдутся —
    вердикт относится к копии из переменной, а исполняется код из другой; это
    печатается переписью (`ROOT_NOTICE`), а не подразумевается.
    """
    global ROOT_NOTICE
    own = HERE.parent.parent.parent
    env = os.environ.get("CLAUDE_PROJECT_DIR")
    if env and (Path(env) / ".claude").is_dir():
        root = Path(env).resolve()
        if root != own.resolve():
            ROOT_NOTICE = (f"КОРНИ РАЗОШЛИСЬ: судится {root} (CLAUDE_PROJECT_DIR), "
                           f"а исполняется оснастка из {own.resolve()} — вердикт "
                           f"относится к первому дереву, не ко второму")
        return root
    return own
...
LIVE_WS = [
    ...
    # Нормативный корпус лежит В ОДНОМ каталоге, и шаблон здесь ОДИН. Прежде их
    # стояло два — по дому на половину корпуса, — и второй снят вместе со своим
    # предметом: `.claude/rulebook/` изъят решением владельца 2026-09-13
    # (`.claude/backup/01-wave-contract.md` §«Что стоит „весь корпус каждому“»).
    # ДВА ОДИНАКОВЫХ шаблона здесь были бы хуже одного: у шаблона есть
    # собственный страж самоистечения (`orphan_patterns`), считающий предметом
    # САМ ШАБЛОН, — и на опустевшем каталоге он выдал бы два одинаковых
    # обвинения об одном факте.
    #
    # Класс, породивший прежнюю пару, при этом НЕ снят и держится не отсюда:
    # частичный переезд оставляет шаблону предмет, поэтому `orphan_patterns`
    # молчит by construction. Его ловит `uncovered_doc_dirs` — обратная сторона,
    # спрашивающая «есть ли у ПРЕДМЕТА шаблон»; замер, из которого она выведена,
    # там же в её docstring.
...
# ── ПСЕВДОНИМ — ВТОРОЕ ИМЯ ДОКУМЕНТА, А НЕ ВТОРОЙ ДОКУМЕНТ ───────────────────
#
# Правило живёт в одном экземпляре — `.claude/rules/<имя>.md`, — но с решения
# владельца 2026-09-17 («Модель загрузки» в `CLAUDE.md`) у него есть ВТОРАЯ
# координата: `.claude/skills/rule-<имя>/SKILL.md`, символьная ссылка на то же
# тело. Шаблон скилов (`^\.claude/skills/[^/]+/[^/]+\.md$`) её опознаёт, и без
# этого предиката ОДИН текст входил бы в корпус ДВАЖДЫ. Замер на дереве
# 2026-09-18: 30 правил, 30 ссылок, воркспейс 335 живых документов против 305.
#
# ЧТО ИМЕННО УБРАНО — ИЗМЕРЕНО, а не заявлено: корпус 576 → 546 документов, пар
# «документ-координата» 5910 → 5385, а УНИКАЛЬНЫХ координат ровно столько же —
# 2517 до и после. То есть снят дубль имени, а не предмет наблюдения: ни одной
# координаты хук читать не перестал.
#
# Цена двойного счёта — три разных, и ни одна не косметическая:
#   • перепись печатает объём корпуса, и 30 из 335 документов были бы одним и тем
#     же телом; базовая линия объёма (`live_baseline`) училась бы на этом числе, и
#     снятие привязок читалось бы как убыль корпуса;
#   • находка в правиле печаталась бы ДВУМЯ записями — под своим именем и под
#     именем ссылки, — а отчёт обрезан (`MAX_DOCS_SHOWN`): удвоенные записи
#     вытесняли бы из него чужие документы;
#   • `../`-ссылка разрешается ОТ КАТАЛОГА ДОКУМЕНТА (`_norm_path`), а у ссылки
#     каталог чужой: `../x.md` правила судилось бы как `.claude/skills/x.md`
#     вместо `.claude/x.md` — обвинение по адресу, которого документ не называл.
#     Сегодня ни одно правило `../` не называет (измерено 2026-09-18: расхождений
#     между разбором в двух контекстах 0), поэтому класс ЛАТЕНТЕН, а не отсутствует:
#     он начинается с первой такой ссылки и никакого признака при этом не подаёт.
#
# ПРЕДИКАТ — `os.path.islink`, А НЕ ИМЯ. Имя (`rule-*`) — соглашение нынешнего
# решения, оно переживёт не всякий рефактор; ссылка остаётся ссылкой. Имя вдобавок
# скрывало бы ровно ту находку, которую рядом ищет skills-gate: каталог `rule-*` с
# НАСТОЯЩИМ `SKILL.md` — это заведённая под зарезервированным именем экспертиза, и
# её документ обязан читаться, а не выпадать из корпуса вместе с привязками.
#
# ТИХИМ ИСКЛЮЧЕНИЕ НЕ БЫВАЕТ: число выведенных псевдонимов печатается переписью
# (`census_line`), тем же способом и по той же причине, что число записок,
# выведенных объявленным состоянием, — «ноль находок» обязано остаться отличимым
# от «не смотрели».
def is_alias(path: Path | str) -> bool:
    """Координата — ПСЕВДОНИМ: в дереве это символьная ссылка, а не документ.

    Аргумент — АБСОЛЮТНЫЙ путь, а не пара «корень + координата»: спрашивают это
    и по перечню дерева, и по пути правленого файла, и второй предикат для
    второго вида вызова разошёлся бы с первым молча.
    """
...
def without_aliases(root: Path, files: list[str]) -> list[str]:
    """Тот же перечень, по которому собирается корпус, — без псевдонимов.

    Стражи покрытия (`orphan_patterns`, `uncovered_doc_dirs`) судят ШАБЛОНЫ, и
    судить их они обязаны по тому составу дерева, к которому шаблоны применяются:
    иначе покрытие, созданное одними ссылками, читалось бы как настоящее, а
    шаблон, у которого остались только псевдонимы, считался бы живым.

    Вопрос «ссылка ли это» задаётся ТОЛЬКО документам, и это граница предмета, а
    не оптимизация: оба стража судят документы — всякий шаблон LIVE кончается на
    `.md`/`.mdx`, а второй смотрит только на них, — поэтому ответ по остальным
    файлам не изменил бы ни одного вердикта. Цена названа числом: замер
    2026-09-18 на 3015 файлах воркспейса — 20 мс на весь перечень против 10 мс на
    1640 документах из него, при 228 мс тёплого прогона хука.
    """
    return [f for f in files
            if not (f.endswith(DOC_SUFFIXES) and is_alias(root / f))]
...
# Третье «дерево» — каталог пробных документов инъекции. Именем оно зовётся только
# там, где считаются выведенные псевдонимы: перепись обязана отличать факт о дереве
# от факта об инъекции.
TREE_PROBE = "проба"


def live_docs(ws: Path, mono: Path | None,
              skipped: dict[str, int] | None = None,
              by_tree: dict[str, int] | None = None,
              aliases: dict[str, int] | None = None) -> list[tuple[str, Path]]:
    ...
    `aliases` (если передан) наполняется числом координат, опознанных шаблоном, но
    выведенных из корпуса как ПСЕВДОНИМ (символьная ссылка, см. `is_alias`), — по
    деревьям. Счёт ведётся ПОСЛЕ шаблона: считается то, что иначе вошло бы в корпус.
    """
        ...
        if _is_live(rel, LIVE_WS):
            if is_alias(ws / rel):
                if aliases is not None:
                    aliases[TREE_WS] = aliases.get(TREE_WS, 0) + 1
                continue
    ...
    if mono is not None:
        ...
        for rel in git(mono, "ls-files"):
            if is_live_mono(rel, mono):
                # Предикат ОДИН на оба дерева: псевдоним второго дерева — такой же
                # второй адрес одного тела, и деление «в воркспейсе ссылки бывают,
                # в продукте нет» было бы утверждением о чужом дереве, которого
                # этот файл не вправе делать.
                if is_alias(mono / rel):
                    if aliases is not None:
                        aliases[TREE_MONO] = aliases.get(TREE_MONO, 0) + 1
                    continue
    ...
    if extra and Path(extra).is_dir():
        ...
        for p in sorted(root.rglob("*.md")):
            ...
            # Тот же знак равенства и для псевдонима: пробная ссылка обязана
            # выводиться из корпуса тем же предикатом, иначе инъекция доказывала
            # бы правило, которого на пробах нет. Ведро у пробы СВОЁ: слив её с
            # воркспейсом подменял бы утверждение о дереве утверждением об
            # инъекции — ровно то, чего избегает `by_tree`, не считающий пробы.
            if is_alias(p):
                if aliases is not None:
                    aliases[TREE_PROBE] = aliases.get(TREE_PROBE, 0) + 1
                continue
...
def _norm_path(tok: str, doc_dir: str = "") -> str:
    ...
    """
    # `@` — маркер импорта правил (`@.claude/rules/<имя>.md`), а не часть пути.
    tok = tok.lstrip("@")
...
def extract(text: str, doc_dir: str = "") -> dict:
    """Координаты документа ПО ВИДАМ плюс талья разбора под ключом `WALK`.

    Ключ `WALK` видом НЕ является: всякий обход результата по видам обязан
    отсеивать всё, чего нет в `KINDS` (`freeze_doc`, обратный индекс, резолв).
    """
    body = strip_noncontent(text)
    found: dict[str, set[str]] = {k: set() for k in KINDS}
    # ТАЛЬЯ РАЗБОРА. Единица — РАЗЛИЧНАЯ вставка кода, та же, что у координаты:
    # считай вхождения — и «вид определён у 31» стало бы несравнимо с «координат
    # распознано 29», а два числа об одном предмете, которые нельзя сложить,
    # читатель складывает всё равно. Ветви ниже ветвятся только по самому токену
    # (кроме выбора `ref`/`path`, а он внутри `kinded`), поэтому множества
    # непересекающиеся и сумма обязана сойтись — это проверяется `walk_mismatch`.
    spans: set[str] = set()
    kinded: set[str] = set()
    withdrawn: dict[str, set[str]] = {}
    unknown: dict[str, set[str]] = {}

    def _out(bucket: dict[str, set[str]], why: str, tok: str) -> None:
        bucket.setdefault(why, set()).add(tok)
    ...
    for mi in INLINE.finditer(body):
        ...
        if RE_ROUTE.match(tok):
            if not _is_placeholder(re.sub(r"\{[^}]*\}", "", tok)):
                ...
                _out(withdrawn, "route-sample", tok)
            continue
        m = RE_RPC.match(tok)
        if m:
        ...
        if tok.startswith(("http://", "https://", "/")) or " " in tok:
            _out(withdrawn, "url-or-phrase", tok)
            continue
        if _is_placeholder(tok.replace("*", "")):
            _out(withdrawn, "sample", tok)
            continue
        ...
        if "/" in tok:
            ok, why = (_looks_like_repo_path(tok) if RE_PATHY.match(tok)
                       else (False, "slashed"))
            if ok:
                ...
                kinded.add(tok)
            elif why in WITHDRAWN_REASONS:
                _out(withdrawn, why, tok)
            else:
                _out(unknown, why, tok)
            ...
            # ПОСЛЕДНЯЯ ВЕТВЬ, и она была молчаливой. Одиночное слово без
            # известного расширения разбор просто отпускал: ни находки, ни
            # зелёного, ни строки в переписи. Именно здесь `24` переставало
            # означать «в файле 24».
            _out(unknown, "bare", tok)
    found[WALK] = {  # type: ignore[assignment]  # не вид координаты — см. WALK
        "spans": len(spans),
        # ВТОРОЙ НОСИТЕЛЬ, и он считается ОТДЕЛЬНО: имя ручки ловится не из
        # вставки, а из текста, поэтому в сумму вставок не идёт. Без этого числа
        # «координат РАСПОЗНАНО 1» при «вид определён у 0» читалось бы как
        # противоречие, хотя это просто два разных носителя.
        "bare": len(bare),
        "kinded": len(kinded),
        "withdrawn": {k: len(v) for k, v in withdrawn.items()},
        "unknown": {k: len(v) for k, v in unknown.items()},
        # ПРИМЕРЫ — ДВА КОНЦА ОДНОГО ПОРЯДКА, а не «первый попавшийся» и не
        # «самый похожий на координату». Два требования сталкиваются: пример
        # обязан быть воспроизводимым (иначе его нельзя ни процитировать, ни
        # закрепить пробой) и не обязан быть выбран по смыслу (выбор по смыслу
        # вернул бы сюда распознаватель, границу которого эта строка и меряет).
        # Оба конца лексикографического порядка удовлетворяют обоим: порядок
        # механический, концы устойчивы, а два конца вместо одного вдвое чаще
        # показывают члена с содержанием, чем пунктуацию.
        "sample": {k: [min(v), max(v)] for k, v in unknown.items()},
    }
    return found
...
def freeze_doc(found: dict) -> dict:
    """Разбор документа → форма, годная для JSON-кэша и для `check_docs`.

    ОДНА функция на оба пути — сборку индекса и свежее перечитывание правленого
    документа. Прежде преобразование стояло двумя копиями строки; добавить ключ
    в одну и забыть вторую означало бы, что перепись после правки документа и
    перепись из кэша печатают РАЗНОЕ про один файл, и разошлись бы они молча.

    Пустые виды отбрасываются (индекс их не хранит), талья — НЕТ: `{"spans": 0}`
    ложен как значение, и `if v` выкинул бы его ровно у документа без вставок
    кода — то есть «вставок ноль» стало бы неотличимо от «разбор не измерялся».
    """
    out: dict = {k: sorted(v) for k, v in found.items() if k in KINDS and v}
    if WALK in found:
        out[WALK] = found[WALK]
    return out


def _looks_like_repo_path(tok: str) -> tuple[bool, str]:
    """Отличить путь дерева от прочего со слэшем — и НАЗВАТЬ, почему не путь.
    ...
    → (True, "") — путь; (False, <ключ исхода>) — не путь, и ключ говорит, по
    какой ветви разбор его отпустил. Причина возвращается ОТСЮДА, а не
    доопределяется у вызывающего: второе место, повторяющее эти же условия,
    разошлось бы с этим молча — ровно класс, который хук ловит в чужой прозе.
    Ключ `gosym` — отвод по НАЗВАННОЙ причине (предикат отказан целиком),
    ключ `slashed` — «вид не определён»: разбор про такую вставку не знает
    ничего и вердикта по ней не выносит.
    """
...
def _resolve_integration_ref(
    ...
    # ЭВРИСТИКА, А ОПРЕДЕЛЕНИЕ: линия интеграции обязана делить с HEAD хотя бы один
    # коммит. Правило «больше всего коммитов, которых нет в HEAD» писалось при
    # допущении, что все кандидаты — линии ОДНОГО продукта; второй удалённый с
    # чужой историей это допущение снимает, и снимает МОЛЧА: чем архив дальше от
    # дерева, тем увереннее он выигрывает отбор.
    #
    # Наблюдалось 2026-09-12 на общей копии продукта: рядом с `origin` стоял
    # удалённый на архив прежнего полирепо службы доступа. Отбор дал `archive/main`
    # (67 коммитов, которых нет в HEAD, против 0 у `origin/main`), и хук судил
    # документацию продукта по полосе из 918 файлов вместо 9823 — без единого
    # признака: он честно печатал «ноль находок» о дереве, которого не читал.
    # Предикат исключения точный: `merge-base` отказывает ровно тогда, когда общего
    # коммита нет (проверено в обе стороны — у архива отказ, у `origin/main` успех).
    #
    ...
    for cand in _trunk_candidates(root, ignore_forced):
        ...
        if not rev:
            continue
        if not forced_ref and not git(root, "merge-base", "HEAD", cand):
            continue
...
def _files_at_ref(root: Path, ref: str) -> set[str]:
    """Пути, отслеживаемые ссылкой, — ЕДИНСТВЕННЫЙ перечислитель полосы ствола.

    Читателей у полосы три: основание вердикта (`_truth_at_ref`), резолв
    координаты-пути (`Truth.trunk_live`) и предпосылки хука (`preconditions`).
    Перечислитель у них ОДИН, потому что два места об одном предмете расходятся
    молча — и расходятся там, где расхождение не видно: обе стороны отдают
    правдоподобный список, а различие проявляется только на той ссылке, где одна
    из них ошибается.
    """
    return set(git(root, "ls-tree", "-r", "--name-only", ref))
...
def build_index(ws: Path, mono: Path | None) -> dict:
    ...
    docs = live_docs(ws, mono, skipped, by_tree, aliases)
    ...
    for name, path in docs:
        ...
        per_doc[name] = freeze_doc(found)
        for kind, coords in found.items():
            # Талья разбора видом НЕ является: в обратный индекс она не идёт (по
            # ней не ищут документы) и в счёт координат тоже — иначе перепись
            # назвала бы координатами то, про что сама же говорит «вид не
            # определён». Отсев по `KINDS`, а не по имени ключа: перечень видов
            # здесь один, и второго его изложения не заводится.
...
def _allow_subject_live(kind: str, coord: str, ws: Path, mono: Path | None,
    ...
    for root, live in ((ws, lambda r: _is_live(r, LIVE_WS)),
        ...
        for rel in git(root, "grep", "-l", "-I", "--untracked", "-F", "-e", coord,
            ...
            if not live(rel):
                continue
            # Псевдоним корпусом не читается (`is_alias`), значит и предмета
            # послаблению не даёт: иначе запись жила бы за счёт ВТОРОГО ИМЕНИ
            # документа, тогда как под своим именем тот же документ её предмет
            # либо даёт — и тогда запись держится им, — либо не даёт вовсе.
            if is_alias(root / rel):
                continue
    ...
    if extra and Path(extra).is_dir():
        for p in Path(extra).rglob("*.md"):
            # Тот же предикат, что у корпуса: пробный псевдоним предмета
            # послаблению не даёт, иначе инъекция доказывала бы правило,
            # которого на настоящем корпусе нет.
            if is_alias(p):
                continue
            try:
...
def tally_walk(c: dict, w: dict | None) -> None:
    """Сложить талью разбора одного документа в перепись прогона.

    ДОКУМЕНТ БЕЗ ТАЛЬИ СЧИТАЕТСЯ ОТДЕЛЬНО, а не пропускается молча: талью
    приносит `extract`, но `per_doc` может прийти из кэша, собранного редакцией
    ДО неё. Молчаливый пропуск дал бы «нераспознанных 0» на документе, который
    вообще не разбирали, — то есть ровно тот класс, ради которого эта талья и
    заведена. Поэтому `walk_docs` — знаменатель обхода, и перепись печатает его
    рядом с числом осмотренных документов.
    """
    if not isinstance(w, dict):
        return
    c["walk_docs"] += 1
    c["spans"] += int(w.get("spans") or 0)
    c["bare"] += int(w.get("bare") or 0)
    c["kinded"] += int(w.get("kinded") or 0)
    for key, src in (("withdrawn", w.get("withdrawn")), ("unknown", w.get("unknown"))):
        for why, n in (src or {}).items():
            c[key][why] = c[key].get(why, 0) + int(n)
    for why, pair in (w.get("sample") or {}).items():
        # Концы порядка сводятся по всем документам прогона — по той же причине,
        # по какой они концы внутри документа: порядок обхода документов зависит
        # от того, что правили, и «первый попавшийся» менял бы пример от прогона
        # к прогону, оставаясь при этом верным. Старая форма (одна строка вместо
        # пары) приходит из кэша прошлой редакции и читается как оба конца сразу.
        lo, hi = (pair if isinstance(pair, list) else [pair, pair])[:2]
        cur = c["unknown_sample"].get(why)
        if cur is None:
            c["unknown_sample"][why] = [lo, hi]
        else:
            c["unknown_sample"][why] = [min(cur[0], lo), max(cur[1], hi)]


def walk_mismatch(c: dict) -> int:
    """Насколько талья НЕ СОШЛАСЬ: вставок минус (вид определён + отвод + неизвестно).

    Предпосылка самой тальи: у каждой вставки РОВНО один исход. Ветви разбора
    ветвятся по самому токену, поэтому множества непересекающиеся и сумма обязана
    равняться числу различных вставок. Появится ветвь, отпускающая вставку мимо
    всех трёх корзин, — число перестанет сходиться, и это обязано быть видно
    СРАЗУ: иначе новая слепая зона откроется ровно там, где её закрывали.
    """
    return int(c.get("spans", 0)) - (int(c.get("kinded", 0))
                                     + sum((c.get("withdrawn") or {}).values())
                                     + sum((c.get("unknown") or {}).values()))
...
def check_docs(names: list[str], idx: dict, truth: Truth, entries: list[dict],
    ...
    c: dict = {"docs": 0, "coords": 0, "uncovered": 0, "trunk_only": 0,
               ...
               # ЗНАМЕНАТЕЛЬ ГРАНИЦЫ ПОКРЫТИЯ — не «сколько координат вида», а
               # «у скольких детектор границы вообще спрашивали». Спрашивают
               # РОВНО у той, что не срезолвилась (`classify_with_reason`),
               # поэтому `path 0/29` читалось как утверждение обо всех 29, хотя
               # детектор видел из них одну.
               "unresolved_by_kind": {k: 0 for k in KINDS},
               # Граница РАСПОЗНАВАНИЯ: сколько вставок кода осмотрено и у
               # скольких вид координаты не определился вовсе. `walk_docs` —
    ...
    for name in names:
        ...
        for kind, lst in coords.items():
            ...
            for coord in lst:
                ...
                # ноль читается только рядом со своим знаменателем.
                c["unresolved_by_kind"][kind] = c["unresolved_by_kind"].get(kind, 0) + 1
...
def preconditions(ws: Path, mono: Path | None,
    ...
    # ОБА СТРАЖА ПОКРЫТИЯ СУДЯТ ТОТ СОСТАВ, ИЗ КОТОРОГО СОБИРАЕТСЯ КОРПУС, —
    # то есть БЕЗ псевдонимов (`without_aliases`, довод — при `is_alias`).
    # Иначе они разошлись бы с корпусом в обе стороны сразу: шаблон, под которым
    # остались одни ссылки, считался бы живым (покрытие есть, а читать нечего), а
    # каталог привязок `.claude/skills/rule-*/` — покрытым, хотя ни один его файл
    # в корпус не входит. Исключение rule-каталогов из требования покрытия здесь
    # ОБЪЯВЛЕНО и производно от одного предиката, а не выписано вторым списком:
    # заведут в таком каталоге настоящий `SKILL.md` — он немедленно вернётся и под
    # шаблон, и под требование покрытия.
    ws_docs = without_aliases(ws, ws_files)
    bad += orphan_patterns("воркспейса", LIVE_WS, ws_docs)
    # Обратная сторона того же вопроса: у шаблона обязан быть предмет, а у
    # предмета — шаблон. Односторонний страж пропускает ЧАСТИЧНЫЙ переезд,
    # то есть самый вероятный способ переезжать.
    bad += uncovered_doc_dirs(LIVE_WS, ws_docs)
    ...
    if mono is not None:
        ...
        #
        # ПЕРЕЧЕНЬ ДЕРЕВА ПРОДУКТА ЧИТАЕТСЯ СО СТВОЛА, А НЕ ИЗ РАБОЧЕГО ИНДЕКСА —
        # ТОЙ ЖЕ ПОЛОСОЙ, ЧТО ВЕРДИКТ (`_truth_at_ref`, `ls-tree` по ссылке).
        #
        # Прежняя редакция брала `ls-files`, то есть состояние ЧУЖОЙ рабочей копии,
        # тогда как вердикт судит вершину ствола. На копии, припаркованной в стороне
        # от ствола, две полосы расходятся, и предпосылка отказывает на посылке,
        # ЛОЖНОЙ О ДЕРЕВЕ: она требует снять словарное имя и исключение, у которых
        # предмет есть — просто не в этой копии. Исполнивший такой отказ выхолостил
        # бы две живые проверки и не узнал бы об этом: сиротство перестаёт
        # обнаруживаться вовсе.
        #
        # Наблюдалось 2026-09-12: копия продукта стояла на отсоединённой голове
        # чужой релизной линии; путей с сегментом `project` в её индексе 0 против 27
        # в стволе, документов под `acceptance/` — 0 против 30. Хук отказался
        # работать и назвал обе живые записи мёртвыми. Это ровно тот класс, который
        # он заведён ловить, — два места об одном предмете внутри одного
        # инструмента, из которых верно то, что судит ствол.
        #
        # НЕРАЗРЕШИМЫЙ СТВОЛ — ТРЕТЬЯ КАТЕГОРИЯ, И ОНА НЕ ОТКАЗ РАБОТАТЬ.
        #
        # Перечень остаётся ПУСТЫМ, и пустой перечень здесь означает «дерево не
        # прочитано» (так же читает его `orphan_patterns` и проверка
        # `NOT_LIVE_MONO` ниже), поэтому ложного залпа обвинений вдогонку не бывает.
        # Но в `bad` это НЕ пишется: запись в `bad` останавливает хук целиком, а у
        # непостроенной полосы ствола уже есть первоклассный исход — `unjudged`,
        # который считается покоординатно и печатается переписью как «вердикт не
        # вынесен N». Отказ вместо него отменял бы вердикт, который хук умеет
        # вынести, и отменял бы его ровно там, где второй полосы нет by
        # construction (обрезанная история, свежий клон без ссылок слежения).
        #
        # Наблюдалось исполнением 2026-09-12: отказ по этой ветке погасил три оси
        # доказательства сразу — «без полосы ствола вердикт НЕ ВЫНЕСЕН и СЧИТАЕТСЯ»,
        # «провенанс называет, ПОЧЕМУ вердикта нет» и ту же пару у оси удалений.
        # Все три требуют от хука именно вердикта с непостроенной полосой.
        #
        # Сказано это НЕ молчанием: ветка «ствол НЕ НАЙДЕН» в `tree_provenance`
        # называет и непостроенную полосу, и то, что словарь в этот прогон не
        # судился. Второго места об одном предмете здесь не заводится.
        mono_ref, _mono_rev, _mono_behind = _resolve_integration_ref(mono)
        # Перечислитель — ОДИН на все три читателя полосы ствола (`_files_at_ref`);
        # сырой `ls-tree` здесь заводил бы второй кодек о том же предмете.
        mono_trunk = sorted(_files_at_ref(mono, mono_ref)) if mono_ref else []
        # Основание — ОБЪЕДИНЕНИЕ ствола и рабочего индекса, потому что вердикт
        # судит координату ровно так: живая в индексе ЛИБО живая на стволе (см.
        # `Truth.classify` и вторую полосу). Ствол в одиночку объявил бы сиротой
        ...
        for files in (ws_files, mono_files):
            for f in files:
                ...
                for i in range(1, len(parts)):
                    dirs.add(parts[i - 1])
        # Сироты словаря объявляются ТОЛЬКО когда полоса ствола прочитана: иначе
        ...
        # СТРУКТУРНАЯ половина остаётся на РАБОЧЕЙ КОПИИ, и это не недосмотр: она
        # судит классификатор против той раскладки, которую читает КОРПУС, а корпус
        # читается с диска (`live_docs` → `mono / rel`). Подай ей полосу ствола — и
        # она зазеленеет на копии, где страницы есть в стволе и отсутствуют на диске,
        # то есть замаскирует ровно тот пустой обход, ради которого заведена.
        bad += doc_root_preconditions(mono, git(mono, "ls-files"))
...
def premise_basis(mono: Path | None) -> str | None:
    """По какой ревизии судили ПРЕДПОСЫЛКИ — и на сколько копия расходится, ЧИСЛОМ.

    Печатается на пути ОТКАЗА, и только там. Отказ выходит ДО сборки индекса,
    поэтому перепись вердикта (`census_line` → `tree_provenance`) до читателя не
    доезжает вовсе — а без ревизии отказ не несёт своего основания и неотличим от
    отказа о продукте. Ровно этим отказ и был неверен 2026-09-12: он требовал
    править словарь, не сказав, что судил припаркованную в стороне копию.

    Формулировка расхождения берётся ТОЙ ЖЕ функцией, что у переписи (`_trunk_lag`),
    поэтому величина считается один раз и двух её изложений не существует. На чистом
    прогоне строка не печатается: перепись уже называет ту же ревизию, а строка,
    выходящая всегда, перестаёт читаться на третий день.
    """
    if mono is None:
        return None
    p = _provenance(mono)
    return ("предпосылки судили дерево продукта по стволу — " + _trunk_lag(p)
            + f"; выписанная копия {p['head']}@{p['branch']}")
...
# Каталог оснастки, документы которого не опознаёт ни один шаблон, — предмет
# ОБРАТНОЙ стороны. Она умышленно узка: `.claude/**` целиком нормативен —
# всякий документ там есть указание в настоящем времени (тот же довод, которым
# в корпус внесены агенты и скилы). В `obsidian/` и `docs/` корпус СМЕШАН
# намеренно (датированная запись против LIVE), поэтому предиката «не покрыт —
# значит не читается» там не существует: у большинства каталогов покрытия нет
# ПО РЕШЕНИЮ. Расширять проверку на них — завести залп ложных отказов.
TREE_DOC_NAMESPACE = ".claude/"


def uncovered_doc_dirs(pats: list[re.Pattern[str]], files: list[str]) -> list[str]:
    """Документ оснастки, которого не опознаёт НИ ОДИН шаблон, — находка.

    ОБРАТНАЯ СТОРОНА `orphan_patterns`, и без неё та односторонняя. Тот страж
    спрашивает «есть ли у ШАБЛОНА предмет», эта — «есть ли у ПРЕДМЕТА шаблон».
    Единица у них разная, и в этом всё дело: частичный переезд оставляет
    шаблону предмет, поэтому первый молчит **by construction**, а второй
    краснеет с первого же переехавшего файла.

    Замер, из которого выведено (2026-09-07): свод правил уехал `.claude/rules`
    → `.claude/rulebook`, в старом каталоге осталось три файла из шестнадцати.
    Та раскладка отозвана 2026-09-13 — `.claude/rulebook/` снят, корпус снова в
    одном каталоге (`.claude/backup/01-wave-contract.md` §«Что стоит „весь корпус
    каждому“»); запись оставлена как замер, а не как описание нынешнего дерева.
    Проверка от отзыва не теряет предмета: она судит покрытие каталога шаблоном,
    а не число каталогов, и следующий переезд встретит ту же слепоту.
    `orphan_patterns` нашёл предмет и промолчал — верно по своему предикату;
    историческая линия объёма (`live_baseline`) промолчала по порогу
    кратности — падение 272 → 260 это 4.4 %, на порядок ниже порога, и снижать
    порог нельзя: он тогда сработает на штатном движении корпуса. Обе половины
    самопроверки объёма промолчали, и 14 нормативных правил — 329 координат —
    вышли из-под наблюдения, не подав ни одного признака.

    ЕДИНИЦА — КАТАЛОГ, а не файл: находкой считается каталог, у которого не
    опознан НИ ОДИН документ. Требовать покрытия каждому файлу было бы шире
    предмета — второй `.md` рядом с `README.md` хука ушёл бы в отказ, ничего
    не сломав. Частичное покрытие ВНУТРИ каталога — другой предмет, и эта
    проверка его не измеряет; сказано вслух, чтобы «покрыто» не читалось шире.

    ПУСТОЙ ОБХОД — ОТКАЗ, а не «находок нет»: перечень файлов пуст, когда
    дерево не прочитано (не git-репозиторий, отказ `git`), и молчание тогда
    означало бы «ноль прочитанного». Здесь это расходится с `orphan_patterns`,
    и расхождение осознанное: у того на пустом дереве нет предмета ни у одного
    шаблона — он выдал бы залп из девяти обвинений; у этой находка ровно одна.

    ПОСЛАБЛЕНИЙ НЕТ И ЗАВОДИТЬ ИХ ВПРОК НЕЛЬЗЯ. Появится каталог оснастки,
    который читать не надо, — решение принимается тогда, по факту, и записью с
    предметом; список, заведённый пустым «на будущее», есть слепая зона,
    выданная вперёд.
    """
    if not files:
        return [
            f"перечень файлов воркспейса ПУСТ — покрытие каталогов «{TREE_DOC_NAMESPACE}»"
            " шаблонами LIVE проверить не на чем. «Ноль находок» здесь означало бы"
            " «ноль прочитанного»"
        ]
    by_dir: dict[str, list[str]] = {}
    for f in files:
        # `node_modules/` снимается ТЕМ ЖЕ предикатом, что и в `_is_live`: иначе
        # каталог, который классификатор исключает НАМЕРЕННО, попал бы сюда как
        # «не опознан ни одним шаблоном» — отказ по собственному же решению.
        if "node_modules/" in f:
            continue
        # `.claude/backup/` — АРХИВ, и это решение владельца 2026-09-20 с предметом,
        # а не недосмотр. Шаблона в LIVE_WS у него нет НАМЕРЕННО: LIVE-утверждение
        # обязано сходиться с СЕГОДНЯШНИМ деревом, а текст архива называет
        # координаты, верные НА МОМЕНТ СНЯТИЯ. Требовать от истории соответствия
        # нынешнему дереву значило бы либо чинить историю, либо держать гейт
        # красным вечно. Две другие половины того же решения держатся механизмами:
        # `.claude/settings.json` → `claudeMdExcludes` не даёт архиву попасть в
        # автозагрузку, а `TRUTH_EXCLUDE` выше — в сверку истины.
        #
        # ЧЕМ ЭТО ИСКЛЮЧЕНИЕ САМОИСТЕКАЕТ: оно верно, пока архив не читают как
        # норму. Признак нарушения — ссылка на `.claude/backup/` из `skills:`
        # любого агента; её судит `scripts/rules-gate/check-02-nothing-lost.sh`
        # осью «объявленный архив, а не второй дом» (два предиката, оба обязательны).
        # Появится такая ссылка — красным станет тот гейт, а не этот, и правильно:
        # предмет там.
        if f.startswith(".claude/backup/"):
            continue
        if f.startswith(TREE_DOC_NAMESPACE) and f.endswith(".md") and "/" in f:
            by_dir.setdefault(f.rsplit("/", 1)[0], []).append(f)
    bad = []
    for d in sorted(by_dir):
        docs = by_dir[d]
        if any(_is_live(f, pats) for f in docs):
            continue
        bad.append(
            f"каталог оснастки '{d}/' несёт {len(docs)} документ(ов), и ни один не"
            f" опознан ни одним шаблоном LIVE воркспейса — например '{sorted(docs)[0]}'."
            " Координаты этих документов не проверяет НИКТО, и признака у этого нет:"
            " перепись честно доложит успех о корпусе, из которого вынули целый вид."
            " Либо завести шаблон рядом с прочими в LIVE_WS, либо — если каталог"
            " читать не надо — объявить это решением с предметом"
        )
    return bad
...
def _same_as_trunk(p: dict) -> bool:
    """Копия и ствол — ОДНА ревизия? Сравнением ревизий, а не нулём в счётчике.

    Вынесено из `tree_provenance` вместе с `_trunk_lag`: читателей у обоих стало
    два, а разбор того, почему «вровень» нельзя выводить из `behind == 0`, лежит
    в `_provenance` и здесь не пересказывается.
    """
    return bool(p.get("head_full")) and p.get("head_full") == p.get("trunk_rev_full")


def _trunk_lag(p: dict) -> str:
    """Расхождение выписанной копии со стволом — ОДНОЙ формулировкой, ЧИСЛОМ.

    Читателей у неё два: перепись вердикта (`tree_provenance`) и предпосылки хука
    (`premise_basis`). Формулировка одна, потому что «по какой ревизии судили и на
    сколько копия расходится» — утверждение о дереве, и два его изложения разошлись
    бы ровно так же, как разошлись два ОСНОВАНИЯ, ради починки которых эта функция
    и выделена: полоса предпосылок читала индекс копии, полоса вердикта — ствол.
    """
    behind, ahead = p.get("behind") or 0, p.get("ahead") or 0
    if not p.get("trunk_ref"):
        cands = p.get("candidates") or []
        return ("ствол НЕ НАЙДЕН среди кандидатов ["
                + (", ".join(cands) if cands else "кандидатов нет") + "]")
    ref, rev = p["trunk_ref"], p["trunk_rev"]
    if _same_as_trunk(p):
        return f"вровень со стволом {ref} {rev}"
    if behind and ahead:
        return f"РАСХОДИТСЯ со стволом {ref} {rev}: позади {behind}, впереди {ahead}"
    if behind:
        return f"ОТСТАЁТ от ствола {ref} {rev} на {behind}"
    if ahead:
        return f"опережает ствол {ref} {rev} на {ahead}"
    # Ревизии разные, а оба счётчика нулевые. Так не бывает при исправном git,
    # поэтому это НЕ повод сказать «вровень»: молчание здесь вернуло бы ровно тот
    # дефект, ради которого `_provenance` считает оба направления.
    return f"расхождение со стволом {ref} {rev} НЕ ИЗМЕРЕНО"
...
def tree_provenance(truth: "Truth") -> str:
    ...
    for tag, human in (("mono", "дерево продукта"), ("ws", "воркспейс")):
        ...
        if not p:
            continue
        s = f"{human} {p['head']}@{p['branch']} — " + _trunk_lag(p)
        same = _same_as_trunk(p)
        if not p.get("trunk_ref"):
            s += (": полоса ствола не построена, вердикт по нерезолвящимся "
                  "координатам НЕ ВЫНЕСЕН")
            # Предпосылки словаря едут на той же полосе и потому в этот прогон не
...
def coverage_line(c: dict) -> str:
    ...
    unres = c.get("unresolved_by_kind") or {}
    seen = c.get("coords_by_kind") or {}
    modelled, blind = [], []
    for kind in KINDS:
        model = COVERAGE_MODEL.get(kind)
        if model:
            # ЗНАМЕНАТЕЛЬ — не резолвившиеся, а не все координаты вида: детектор
            # границы спрашивается только у них. И форма, которую он умеет
            # опознать, называется ЗДЕСЬ ЖЕ — иначе «path 0/1» читается как «за
            # границей основания по путям пусто», хотя означает «среди не
            # резолвившихся не было имени предшествующего полирепо». Та же
            # дисциплина, что у невыраженной границы ниже; разница в том, что
            # там не искали вовсе, а здесь искали ОДНУ названную форму.
            modelled.append(f"{kind} {by.get(kind, 0)}/{unres.get(kind, 0)}"
                            f" из {seen.get(kind, 0)} распознанных (ищется: {model})")
        else:
            blind.append(f"{kind} {by.get(kind, 0)}/{unres.get(kind, 0)}")
    out = (f"вне покрытия основания {c.get('uncovered', 0)}; знаменатель — НЕ РЕЗОЛВИВШИЕСЯ "
           f"координаты, детектор границы спрашивается только у них: " + ", ".join(modelled)
           + " — ноль у вида означает «этой формы среди не резолвившихся не было», "
             "а НЕ «за границей основания пусто»")
    if blind:
...
# Длина примера. Вставка бывает до 200 символов (`INLINE`), и такая в строке
# переписи вытеснила бы саму перепись. Обрезка ВИДНА: пример без многоточия —
# целый, с многоточием — усечённый, и второе не выдаётся за первое.
SAMPLE_MAX = 48


def _cut(tok: str) -> str:
    return tok if len(tok) <= SAMPLE_MAX else tok[:SAMPLE_MAX] + "…"


def _samples(pair) -> str:
    """Концы порядка → «пример `X`» либо «примеры `X` … `Y`».

    Один член корзины даёт одно слово, а не два одинаковых: «примеры `,` … `,`»
    читалось бы как два разных, которых нет.
    """
    if not pair:
        return "примера нет"
    lo, hi = (pair if isinstance(pair, list) else [pair, pair])[:2]
    if lo == hi:
        return f"пример `{_cut(lo)}`"
    return f"примеры `{_cut(lo)}` … `{_cut(hi)}`"


def recognition_line(c: dict, docs_seen: int) -> str:
    """Граница РАСПОЗНАВАНИЯ: сколько вставок осмотрено и у скольких вид не определён.

    Ради чего строка заведена. Перепись печатала «вне покрытия основания 0 из
    рассмотренных: path 0/29», и знаменатель 29 читался как «в документе 29
    координат». Означал он «29 я распознал». Про остальные вставки хук не
    говорил НИЧЕГО, и его ноль читался как полнота — класс
    `kacho-workspace#762` внутри прибора, который этот класс ищет у других.
    Числа замера и его единица — в объявлении `WALK`, одним местом: второе
    изложение одной величины разошлось бы с первым молча.

    СТРОКА ВЫХОДИТ ВСЕГДА, в том числе когда нераспознанных ноль, — иначе она
    сама стала бы экземпляром того же класса: появляющаяся только при находках,
    она делала бы «ноль» неотличимым от «не считали». Три состояния печатаются
    РАЗНЫМИ словами: не измерено · измерено, нераспознанных нет · измерено,
    нераспознанных N с примером.
    """
    docs = int(c.get("walk_docs", 0))
    if not docs:
        # ДВА РАЗНЫХ НУЛЯ, и их нельзя слить. «Документов не осматривали» —
        # ШТАТНОЕ состояние (правка файла дерева, пустой конец хода), и тревога
        # на нём перестала бы читаться вовсе. «Документы осмотрены, тальи нет» —
        # настоящий отказ: разбор шёл по кэшу редакции без тальи.
        if not docs_seen:
            return ("граница РАСПОЗНАВАНИЯ не измерялась: документов этим "
                    "прогоном не осматривали")
        return ("граница РАСПОЗНАВАНИЯ НЕ ИЗМЕРЕНА: талья разбора не пришла ни по "
                f"одному из {docs_seen} осмотренных документов — «нераспознанных 0» "
                "здесь означало бы «не считали»")
    unknown = c.get("unknown") or {}
    n_unknown = sum(unknown.values())
    wd = c.get("withdrawn") or {}
    out = (f"граница РАСПОЗНАВАНИЯ (талья разбора по {docs} докум. из {docs_seen}): "
           f"вставок кода осмотрено {int(c.get('spans', 0))}, вид координаты определён "
           f"у {int(c.get('kinded', 0))}")
    if wd:
        out += (", отведено по названной причине " + str(sum(wd.values())) + " ("
                + ", ".join(f"{WITHDRAWN_REASONS.get(k, k)} {n}"
                            for k, n in sorted(wd.items())) + ")")
    else:
        out += ", отведено по названной причине 0"
    if n_unknown:
        sample = c.get("unknown_sample") or {}
        out += (f", вид НЕ ОПРЕДЕЛЁН у {n_unknown} ("
                + ", ".join(f"{UNKNOWN_EXITS.get(k, k)} {n}, {_samples(sample.get(k))}"
                            for k, n in sorted(unknown.items()))
                + ") — по ним вердикта НЕТ ни зелёного, ни красного")
    elif int(c.get("spans", 0)):
        out += (", вид НЕ ОПРЕДЕЛЁН у 0 — каждая осмотренная вставка либо распознана, "
                "либо отведена по названной причине")
    else:
        # ПУСТОЙ ОБХОД — НЕ ВЕРДИКТ. «Нераспознанных 0» над нулём вставок верно
        # и бессодержательно: оно читается как чистота, а означает «предмета не
        # было». Проза без вставок кода — большинство корпуса, и молчать об этом
        # значило бы выдавать её за проверенную.
        out += (", вид НЕ ОПРЕДЕЛЁН у 0 — но и вставок кода нет: распознавать было "
                "нечего, это не «всё распознано»")
    # Имя ручки — ВТОРОЙ носитель: оно ловится из текста, не из вставки, поэтому
    # в сумму вставок не входит и стоит ОТДЕЛЬНОЙ оговоркой в конце строки. У
    # него своя невыраженная граница: ищется ровно префикс `KACHO_`, имена
    # другой формы не ищутся ВОВСЕ — и без этой оговорки «координат РАСПОЗНАНО
    # 1» при «вид определён у 0» читалось бы как противоречие.
    out += (f" · имён ручек вне вставок {int(c.get('bare', 0))} — ищется только "
            "форма `KACHO_*`, имена другого вида (`PGHOST`, `HOME`) не ищутся вовсе")
    mism = walk_mismatch(c)
    if mism:
        # Предпосылка тальи отказала: у вставки перестал быть ровно один исход.
        # Молчаливое «ну и ладно» открыло бы новую слепую зону ровно там, где её
        # закрывали, поэтому расхождение называется числом и прямо в переписи.
        out += (f" · ТАЛЬЯ НЕ СОШЛАСЬ на {mism}: у вставки больше не ровно один "
                "исход — числа этой строки НЕДЕЙСТВИТЕЛЬНЫ, чинить разбор в extract()")
    return out
...
def census_line(idx: dict, truth: Truth, c: dict, warm: bool, ms: int,
    ...
    # Псевдонимы (символьные ссылки) названы ОТДЕЛЬНЫМ числом, а не слиты с
    # записками: основания разные — там объявленное состояние документа, здесь
    # второе имя одного тела, — и общий счётчик не дал бы прочесть ни одного из
    # них. Ноль не печатается: строка, выходящая всегда, перестаёт читаться.
    alias = idx.get("live_aliases") or {}
    # Расхождение корней печатается ПЕРВЫМ, до провенанса ревизий: ревизия чужого
    # дерева читается как ревизия этого, пока не сказано, что дерево чужое.
    parts = ([ROOT_NOTICE] if ROOT_NOTICE else []) + ([prov] if prov else []) + [
        f"осмотрено документов {c.get('docs', 0)} из {len(idx['docs'])} LIVE"
        + (" (" + ", ".join(f"{k} {v}" for k, v in sorted(by_tree.items())) + ")"
           if by_tree else "")
        + (" · вне LIVE псевдонимом (символьная ссылка — второе имя документа) "
           + str(sum(alias.values()))
        ...
        # «РАСПОЗНАНО», а не «рассмотрено». Слово несущее: знаменатель этой
        # переписи — не «сколько координат в документе» (этого не знает никто, и
        # хук в том числе), а «сколько распознал разбор». Прежняя формулировка
        # читалась первым способом, и её ноль вне покрытия читался как полнота.
...
def sweep() -> int:
    ...
    if bad:
        basis = premise_basis(mono)
        if basis:
            sys.stderr.write("[VOID] docfresh: " + basis + "\n")
        for b in bad:
...
def main() -> int:
    ...
    if bad:
        ...
        # КАКОЕ ДЕРЕВО ОТКАЗАЛО — раньше всего прочего. Отказ чаще всего и приходит
        # оттого, что корень взят из чужой копии: «дерево продукта не найдено» без
        # этой строки читается как утверждение о ЗДЕШНЕМ дереве. Перепись до сюда
        # не доходит — путь возвращает 2 выше неё, — поэтому факт назван и здесь.
        if ROOT_NOTICE:
            lines.append("║ docfresh: " + ROOT_NOTICE)
        # Ревизия, по которой судили, — ПЕРВОЙ строкой: отказ, не назвавший своего
        # основания, неотличим от отказа о продукте (см. `premise_basis`).
        basis = premise_basis(mono)
        if basis:
            lines.append("║ " + basis)
        for b in bad:
    ...
    # Псевдоним и здесь не документ (`is_alias`): правка ВТОРОГО ИМЕНИ записки не
    # вправе вернуть её в корпус в обход того же предиката — иначе находка
    # печаталась бы под именем ссылки, которого корпус не знает.
    if VAULT_NOTE.match(rel) and _is_live(rel, LIVE_WS) and not is_alias(fpath):

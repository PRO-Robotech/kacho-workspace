...
class Truth:
    ...
    def classify(self, kind: str, coord: str) -> str:
        ...
        Пятый исход заведён по той же причине, что третий и четвёртый, и это
        предпоследний шаг той же лестницы: «смотрел не туда» нельзя выдавать за
        «этого нет». Здесь смотреть НЕ УДАЛОСЬ ВОВСЕ — у одного из корней не
        построена полоса ствола (ссылки нет, `ls-tree` пуст). Тогда координата,
        не найденная в выписанной копии, не отрицается: отсутствие вердикта —
        отдельное состояние, а не отрицательный вердикт.
        """
...
def rel_of(path: str, ws: Path, mono: Path | None) -> str | None:
    ...
    if mono is not None and mono != ws:
        try:
            return "project/kacho/" + str(p.relative_to(mono.resolve()))
        except (ValueError, OSError):
            pass
    try:

...
LIVE_WS = [
    ...
    # Свод нормативных правил лежит В ДВУХ каталогах, и шаблона здесь ДВА, а не
    # один с альтернативой `(rules|rulebook)`. Различие не стилистическое: у
    # шаблона есть собственный страж самоистечения (`orphan_patterns`), и он
    # считает предметом ШАБЛОН. Слитая альтернатива опустела бы только вместе с
    # обоими каталогами, то есть унаследовала бы ровно ту слепоту, которая этот
    # дефект и породила: 2026-09-07 свод переехал `rules` → `rulebook`, в старом
    # каталоге осталось три файла, шаблон нашёл предмет — и 14 правил из 16
    # вышли из корпуса МОЛЧА (воркспейс 272 → 260, замер командой из README).
    re.compile(r"^\.claude/rules/[^/]+\.md$"),
    re.compile(r"^\.claude/rulebook/[^/]+\.md$"),
...
def _norm_path(tok: str, doc_dir: str = "") -> str:
    ...
    """
    # `@` — маркер импорта правил (`@.claude/rulebook/x.md`), а не часть пути.
    tok = tok.lstrip("@")

...
def preconditions(ws: Path, mono: Path | None) -> list[str]:
    ...
    if mono is not None:
        ...
        #
        # НЕРАЗРЕШИМЫЙ СТВОЛ — ТРЕТЬЯ КАТЕГОРИЯ, А НЕ НАХОДКА: перечень остаётся
        # ПУСТЫМ, и пустой перечень здесь означает «дерево не прочитано» (так же
        # читает его `orphan_patterns` и проверка `NOT_LIVE_MONO` ниже), поэтому
        # ложного залпа обвинений вдогонку отказу не бывает. Расхождение копии со
        # стволом числом называет перепись вердикта (`_provenance`), и второй раз
        # здесь оно не считается: два счёта одной величины разошлись бы молча.
        mono_ref, mono_rev, _mono_behind = _resolve_integration_ref(mono)
        if mono_ref is None:
            bad.append(
                "ствол дерева продукта не резолвится (искали: "
                + ", ".join(_trunk_candidates(mono) or ["кандидатов нет"])
                + "): перечень, по которому судятся словарь корневых сегментов и "
                "выписанные шаблоны, ПУСТ. Это «не выполнилось», а не «предмета нет» — "
                "создать условие (подтянуть ссылку слежения в дереве продукта), а не "
                "править словарь"
            )
            mono_files: list[str] = []
        else:
            mono_files = git(mono, "ls-tree", "-r", "--name-only", mono_ref)

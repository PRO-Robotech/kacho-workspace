...
# Признак дельты — только у события ГЛАВНОГО ПОТОКА (`Stop`). `PostToolUse` читает
# исполнитель, и в одной сессии их несколько: отпечаток не знает читателя, и
# «без изменений» дошло бы до полосы, не видевшей полного тела, — потерянная находка.
MAIN_THREAD = False
...
VOLATILE_OF_THIS_PROBE = (r"кэш (?:тёплый|холодный)",
                          r"прогнан \d+ раз")   # счётчик прогонов тревоги молчания
...
def main() -> int:
    ...
    global SESSION, MAIN_THREAD
    SESSION = str(event.get("session_id") or "")
    hook_event = event.get("hook_event_name") or ""
    MAIN_THREAD = hook_event == "Stop"
    ...
    if findings or stales or gaps:
        out, _ = signal_out(
            ...
            clear_when="python3 .claude/hooks/docfresh/docfresh.py --sweep даёт код 0 "
                       "и строку [PASS]",
            sha=sha)
        # На `PostToolUse` признак дельты не применяется (`MAIN_THREAD`): тело идёт
        # полностью каждой правке, код 2 доставляет его автору правки.
...
def stop_mode(idx: dict, truth: Truth, entries: list[dict], ws: Path,
    ...
    # Пуст предмет ВСЕХ осей, а не одной: исчезнувший путь без вердикта (`unjudged`)
    # при нуле осмотренных документов — предмет, и его счёт обязан дойти до печати.
    if not (cnt.get("docs") or touched or unaccounted or ngone):
        out, _ = signal_out(
            "docfresh-no-subject",
            f"docfresh: предмета не было: осмотрено документов 0 из {len(idx['docs'])} LIVE"
            f" · за ход затронуто 0, мимо Write/Edit 0, исчезло из дерева 0"

...
#
# `PostToolUse` по Bash — событие ТОГО, КТО ЗВАЛ: главному потоку этот хук не печатает
# никогда, потому что у диспетчера Bash'а нет. Читает его ИСПОЛНИТЕЛЬ, двинувший HEAD,
# и «перепроверь координату чтением файла» адресовано ему по существу. Текст общий с
# `rag-session-start.sh` и живёт в `rag-freshness.sh` — там же названо, какая строка
# чья (см. §«Кому адресован вывод»).
set -uo pipefail

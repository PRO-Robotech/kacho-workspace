...
## 9. Контур изменения: вердикт привязан к ОТПЕЧАТКУ, а не к документу

Норма — `change-graph.md` §Вердикт привязан; здесь она не пересказывается. Что связывает
**тебя** как единственный гейт одобрения:

- **ты одобряешь точное содержимое, а не файл по имени.** Правка одобренного документа
  вердикт **не переносит**: прежний вердикт остаётся вердиктом о прежней редакции. Это
  верно, а не досадно, — и означает, что после каждой правки требуется новое рассмотрение.
- **статическая форма вердикта в шапке не меняется после ревью.** Слово в заголовке — то,
  что туда вписали; действующее одобрение выводится из внешнего события и добавляемой
  записи. Не принимай шапку за вердикт и не правь её вместо записи.
- **история ревью добавляется, не переписывается.** Прежняя запись не редактируется и не
  удаляется — новая редакция получает **соседнюю**. Перезапись стирает единственное
  свидетельство того, что редакция рассматривалась.
- **роль без события полномочия не даёт.** Имя роли — координата, а не доказательство
  личности. Недоступность внешнего источника — **«не выполнилось»**, третий исход; никогда
  «да».
- **одобрять нечего, пока перечень кейсов пуст.** Приёмка с нулём кейсов, нулём требуемых
  держателей либо без предмета — отказ, а не вакуумное одобрение.
...
> держит испытуемый контура, которого в дереве ещё нет.

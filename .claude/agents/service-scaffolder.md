...
Чего у сервиса **нет и не заводится**: собственного `go.mod` — **по умолчанию**: модуль берётся корневой. Своё объявление
модуля заводится только тогда, когда часть продукта выделяется в **самостоятельный
продукт со своим именем**, и это отдельное решение владельца, а не шаг заведения службы
(так получила своё объявление служба управления доступом). Модулей в дереве сегодня
**два**, и предикат — `git ls-files '**/go.mod' | wc -l` и

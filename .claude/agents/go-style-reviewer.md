...
- `landing-reviewer` — после твоего «Одобрено»: твой вердикт о коде он не пересматривает,
  а использует, устанавливая ОБЛАСТЬ зелёного (что прогон реально покрыл). Коммит/мёрж и
  чтение отчёта прогона — его момент, не твой; «блокирует мердж» у тебя означает замечание
  автору, а не решение о посадке.

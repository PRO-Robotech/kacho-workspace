...
Канон lifecycle/gates — @.claude/rulebook/ai-tooling.md; правила тестов — @.claude/rulebook/testing.md;
форма API/Operation/error-коды — @.claude/rulebook/api-conventions.md.

> **Скил, владеющий этим моментом:** `gate-authoring` — конструкция проверки, СПОСОБНОЙ упасть: производитель входа и захват реального сообщения, инъекция настоящим входом с законным близнецом, отрицание только в паре с положительным, детерминизм входа и управляемые часы, фикстура не снисходительнее продукта. Плюс `verdict-and-landing` §«Три категории исхода» на подтверждении RED: «не выполнилось» — не красный.
>
...
(см. @.claude/rulebook/data-integrity.md). LRO в use-case/handler-тестах дожидаются
...
Норма — `change-graph.md`; наблюдаемое поведение и перечень кейсов — приёмка SDD-1. Здесь
не пересказываются: два места об одном предмете расходятся молча. Что связывает **тебя**:

- **дифф до реализации — твой, и он не содержит реализации.** Драйвер кейсов, библиотека
  разбора и фикстуры принадлежат тебе; испытуемый, его вызывающие, генератор производного,
  канонический `.claude`, конвейер и дерево продукта — **дифф реализации**, и до честного
  красного он запрещён. Оснастка, содержащая либо маскирующая реализацию, запрещена
  независимо от пути, по которому лежит.
- **красный обязан быть честным.** Отсутствие возможности у испытуемого — честный красный,
  ...
  **не он**; они реализацию не открывают. Различение и порядок проверок — скил
  `change-graph` §Что открывает реализацию.
- **порядок проверок в драйвере несущий**, а не стилистический: проба возможности
  испытуемого стоит **после** всей проверки фикстуры. Обратный порядок даёт сломанной
  фикстуре право выдать себя за отсутствующую возможность — то есть поломка сама себе
  выписывает разрешение писать код. Правя драйвер, порядок сохраняй.
- **у отрицательного кейса назван положительный близнец и ровно один изменённый факт.**
  Дельту **вычисляет драйвер**, объявление сверяется с вычисленным. Фикстура, меняющая два
  факта, вердикта держателя не даёт — и выглядит при этом обычным зелёным.
- **пустой предмет не даёт зелёного:** ноль кейсов, ноль требуемых держателей либо
  отсутствующий предмет — красное, а не вакуумный успех.

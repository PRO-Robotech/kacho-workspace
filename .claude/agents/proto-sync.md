...
Канон API — `@.claude/rulebook/api-conventions.md` (форма ресурса, naming, error-format,
update_mask, pagination). Размещение и порядок работы — `@.claude/rulebook/polyrepo.md`.
...
`string name`, `string description`, `map<string,string> labels`. См. `@.claude/rulebook/api-conventions.md`.
...
сервис — они регистрируются только на internal-listener (см. `@.claude/rulebook/security.md`).
...
- Кросс-репо порядок: proto — шаг 1 (см. `@.claude/rulebook/polyrepo.md`); ниже по графу CI

...
> числа. Записка описывает **топологию и рёбра**; нормативный источник — `.claude/rules/polyrepo.md`
> по топологии и `.claude/rules/polyrepo-runtime-edges.md` по рёбрам, и при расхождении верно
> оно, а не эта страница.
...
- **`registry → OCI-бэкенд`** — ребро к хранилищу артефактов
  (`services/registry/internal/clients/`). В перечне `polyrepo-runtime-edges.md` его нет: там
  перечислены рёбра **между доменами Kachō**, а это ребро к внешней системе.

Поимённый регламент каждого ребра (протокол, срок, поведение при отказе, история) —
`.claude/rules/polyrepo-runtime-edges.md` §«Runtime cross-domain edges» и категория `edges/`
этого vault.
...
- **`kacho-vpc-operator`** — не резолвится на GitHub (404), в дереве ноль файлов. Два ребра к
  нему держались «как контракт на случай появления» и **сняты** решением владельца 2026-08-09:
  `polyrepo-runtime-edges.md`, note «Здесь стояли два ребра оператора сети».
...
- `.claude/rules/polyrepo.md` — нормативная топология, build-граф, порядок работы
- `.claude/rules/polyrepo-runtime-edges.md` — нормативный перечень рёбер рантайма

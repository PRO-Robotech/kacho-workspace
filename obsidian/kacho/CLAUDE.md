...
- **Repo**: `kacho-vpc`, `kacho-rm`, `kacho-corelib`, `kacho-proto`, `kacho-apigw`, `kacho-compute`, `kacho-loadbalancer`, `kacho-deploy`, `kacho-vpc-implement`, `kacho-ui` (для каждого репо — **полное** имя, не `vpc`/`rm`/`apigw`).

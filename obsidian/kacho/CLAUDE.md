...
> [!important] Перед изменением vault — действующего адреса у канона СЕГОДНЯ нет
> Здесь стояла отсылка к корневому `CLAUDE.md` §«Obsidian vault — обязательный
> context-источник и trail». В модели с диспетчером такого раздела у корневого файла **нет**:
> он несёт модель загрузки, контракт возврата и топологию, а норм продукта и хранилища не
> несёт. Отсылка вела в никуда и снята 2026-09-22.
>
> Замена ей — норма хранилища под именем `vault`, закреплённая за агентом, который в хранилище
> пишет. **Её файла в этом дереве нет**: норма снята вместе с двенадцатью процессными файлами
> и восстанавливается полосой `tooling-dead-rule-coordinates` (координата, не живая ссылка) —
> одной из одиннадцати восстанавливаемых там под прежними именами. Полоса **не слита**, и до её
> посадки координата корпуса здесь не пишется: живого адреса за ней сегодня нет, а мёртвый
> отправил бы читателя туда, где ничего нет.
>
> Предикат снятия этой врезки: файл нормы `vault` резолвится в дереве — тогда его координата
> встаёт сюда тем же заходом. До тех пор действует то, что написано **ниже в этом файле**:
> оболочка записки, словарь состояний, категории и запреты.
>
...
- **Одна записка — один предмет**, самодостаточно, без дублирования содержимого соседа
  ...
  Канон — норма хранилища под именем `vault`; здесь он не пересказывается, чтобы два места об
  одном предмете не разошлись снова. Её координата в дереве сегодня не резолвится — почему,
  сказано во врезке выше.

...
>
> Нормы хранилища `vault` в корпусе `.claude/rules/` **нет**: она снята в архив
> `.claude/backup/vault.md` (#706), а архив — не норма. Вернуть её в корпус берётся полоса
> `tooling-dead-rule-coordinates` (координата, не живая ссылка); до посадки координата корпуса
> здесь не пишется — за ней нет файла.
>
> Предикат снятия этой врезки: файл нормы `vault` резолвится в `.claude/rules/` — тогда его
> координата встаёт сюда тем же заходом. До тех пор действует то, что написано **ниже в этом
> файле**: оболочка записки, словарь состояний, категории и запреты.
>
...
- **Одна записка — один предмет**, самодостаточно, без дублирования содержимого соседа
  ...
  одном предмете не разошлись снова. В корпусе её сегодня нет — почему, сказано во врезке выше.

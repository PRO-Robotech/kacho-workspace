...
**Видимость**: административный, но **НЕ** internal-only — с ADM-1 S1 (kacho#522).
Управление выставлено на ПУБЛИЧНОМ `AddressPoolService` под правом `system_admin` @
`cluster`: закрыто не место вызова, а вызывающий без права. Запрет 6 при этом не
смягчён — `Internal*`-сервис на внешний край не выставлен, переехал глагол. Инфра-полей
по перечню `security.md` ресурс не несёт, поэтому двух проекций ему не нужно: публичное
сообщение и есть полное.

> [!note] Здесь стояло «только `Internal*` (:9091), ban #6» — утверждение пережило предмет
> Оно было верно до 2026-08-17 и читалось как действующее ограничение: следующий
> контрибьютор, увидев его, заключил бы, что публиковать этот ресурс нельзя. Внутренний
> сервис при этом ЖИВ — окно расширения (S1→S3) держит оба глагола на одной паре
> (метод, путь): снаружи виден публичный, изнутри внутренний. Стадия S3 снимет внутренний,
> и тогда эта оговорка снимается вместе с ним.

**Контракт**: `proto/kacho/cloud/vpc/v1/address_pool_service.proto` (публичный) +
`proto/kacho/cloud/vpc/v1/internal_address_pool_service.proto` (внутренний; там же живёт
`message AddressPool` и все сообщения запросов — публичный сервис их импортирует, а не
переобъявляет)

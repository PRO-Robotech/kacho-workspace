...
> [!note] У полосы отзыва появился читатель НА КРАЕ — и шапка сервиса это отрицала
> Записка ниже описывает фазу, в которой чтение отзыва было внутренним. На линии
> `release/identity-3` (в стволе **нет** на 2026-08-24) клиент края экспонирует чтение, и оно
> провязано в решение слоя личности. Шапка метода на стороне сервиса продолжала утверждать,
> что вызывающего у края нет, — предмет и гейт, который держит это утверждение по дереву:
> [[KAC/issue-1156]].
>
> Поля и жизненный цикл ниже при этом **не перепроверялись** — их границей остаётся прежняя
> сверка, названная в `verified_against`.

...
tags:
  ...
  - kacho-iam
  - iam
verified_against: "раздел о потолке сверен с `release/quota-domain` (2026-08-17, PR #629); остальные поля построчно не пересматривались"
---
...
## Потолок на число аккаунтов — носитель ЛИЧНОСТЬ (2026-08-17)

До этого число аккаунтов не ограничивалось ничем: вид стоял в каталоге **исключением** —
«потолка нет, потому что нет носителя». Носитель нашёлся, и им оказалась **не строка
пользователя**.

**Считается по внешнему идентификатору входа.** Причина не стилистическая: строка
пользователя **есть членство** и размножится ровно тем переходом, который потолок обязан
пережить ([[iam-user]], отрыв идентичности от аккаунта). Потолок, привязанный к строке,
после того перехода начал бы считать членства вместо людей — и вырос бы молча.

Владелец величин здесь **впервые списывает сам**, поэтому догоняющего читателя ему не
нужно; гейт покрытия проверяет эту предпосылку отдельно и определяет владельца **замером**,
а не именем.

Корней аренды стало **три** (проект · аккаунт · личность), исключений в каталоге осталось
**одно**.

> [!warning] Аккаунт приглашённого владельца — не считается и НЕ отвергается
> У такого аккаунта внешнего идентификатора нет. Проверка на этапе разработки едва не
> начала отвергать его — то есть **менять то, что платформа принимает**, под видом счёта.
> В обзоре диффа это неотличимо от аккуратной проверки. Теперь случай пропускается, и
> пропуск **громкий**; обходом он не является, потому что самообслуживание выводит
> владельца из проверенного принципала.

...
> в монорепо ноль файлов по этому имени. Ни CRD, ни контроллера, ни generated-типов нет.
> Источник истины по подсети — [[vpc-subnet]] (`kacho_vpc.subnets`, prefix `sub`).
>
> **Перемерено 2026-08-12** (`4d26e330`): имя `KachoSubnet` в дереве продукта не
> встречается **ни разу** — ни в отслеживаемых файлах, ни в рабочем каталоге целиком
> (`git grep KachoSubnet` и `grep -rl` дают ноль). Прежняя редакция говорила, что имя
> живёт «в страницах документации vpc», и называла каталог, который с тех пор переехал:
> утверждение было неверно и в факте, и в координате. Замер без предиката стареет молча —
> здесь названы оба, чтобы следующий перемерил, а не поверил.

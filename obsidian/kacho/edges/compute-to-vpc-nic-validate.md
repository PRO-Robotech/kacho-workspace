...
tags:
  ...
  - kacho-vpc
  - ni
verified_against: "живость внутреннего провода и его провязка чартом перемерены на дереве продукта b0de60a05 (2026-08-09); таблицы RPC и отказов сверялись 96b2879a (2026-08-05) и построчно не пересматривались"
---
...
- **2026-08-08 (`kacho#10`, PR `#124`)** — внутренний провод **оживлён**: см. предупреждение
  ниже. Публичный провод (`SubnetService.Get`) всё это время был жив; мёртв был внутренний.

> [!warning] Внутренний провод был провязан в коде и НЕ выставлен чартом — три недели
> Группу клиентских кред внутреннего провода конфиг compute читал с середины июля, а чарт не
> производил её **ни разу** (предикат назван с ревизией, чтобы не устарел от закрывающего
> коммита: поиск имени группы по истории `services/compute/deploy/` до `6b1293713` — ноль
> коммитов). Дозвон шёл на слушатель, требующий проверенный клиентский сертификат, и
> обрывался на рукопожатии — то есть привязка отвечала `UNAVAILABLE` **честно**.
>
> **Опаснее было другое.** Профиль разработки обнулял адрес соседа, а это не выключает ребро,
> а подменяет его заглушкой. Заглушка на привязке отказывает честно, но **перечисление
> интерфейсов возвращает у неё пустой список без ошибки** — поэтому шаг высвобождения на
> удалении машины молча не снимал ничего. Пустой ответ неотличим от «интерфейсов нет», и
> именно эта половина заглушки была невидима: [[lessons/checks-with-form-but-no-substance]].
>
> **Закрыто двумя гейтами, оба декларативные** (это их граница, и её надо знать): чарт обязан
> производить **каждую** группу кред, которую читает конфиг и которой требует страж старта
> (разбор конфига и стража по AST, свободный проброс окружения производителем **не**
> считается); и ни один профиль umbrella не вправе обнулять адрес соседа — перечень адресов
> **выводится** из умолчаний чарта, а не выписывается, поэтому новый сосед попадает под
> проверку сам.
>
> **Чего в дереве нет:** e2e-кейсов compute на привязку интерфейса — **ноль** (перепись по
> `services/compute/tests/newman/cases/`). «Прогнано на живом стенде» — заявление коммита, и
> оно не подтверждается артефактом в дереве. Значит проводом ходили, но повторить это одной
> командой из репозитория нельзя. Разбор дня — [[KAC/debt-waves-2026-08-08]].

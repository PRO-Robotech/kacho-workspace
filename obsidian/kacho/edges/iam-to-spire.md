...
> [!warning] SPIRE в дереве нет — есть SPIFFE-имена, и это не одно и то же (сверено 2026-08-05)
> `spire` встречается в **8** файлах, и ни один из них не является клиентом SPIRE: это
> SPIFFE-**идентификаторы** в сертификатах (`pkg/grpcsrv/cert_identity.go`, списки законных
> отправителей, политики вызывающего в iam). Личности сервисов выпускает cert-manager с
> внутренним УЦ, а не агент SPIRE.
>
> Различать это важно: «у нас SPIFFE» верно, «у нас SPIRE» — нет, и вывод «значит есть
> ротация и аттестация рабочей нагрузки» из первого не следует.

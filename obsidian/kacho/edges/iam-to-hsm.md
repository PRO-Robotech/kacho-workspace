...
> Ни `pkcs11`, ни модуля подписи в аппаратном хранилище в монорепо нет (**ноль** вхождений),
> переменная требования аппаратного ключа не резолвится. Подписантом токенов сегодня
> является Hydra ([[iam-to-hydra-admin]]), а iam ключей не чеканит — он их зеркалит
> ([[registry-to-iam-jwks-fetch]]). Записка — намерение, и при его реализации первым вопросом
> будет не «как подключить модуль», а «кто здесь подписант».

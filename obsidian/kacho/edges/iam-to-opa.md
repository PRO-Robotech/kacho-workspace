...
> [!warning] Sidecar существует, но выключен по умолчанию и решения о доступе не принимает (сверено 2026-08-05)
> В отличие от соседей по разделу, у этой записки предмет частично есть: чарт iam несёт
> шаблоны sidecar'а и его политик, а значение включения по умолчанию — `false`
> (`deploy/helm/umbrella/values.yaml`). Чего нет — **клиента в коде iam**: решение о доступе
> принимает модель прав (проверка через OpenFGA), а не политика sidecar'а.
>
> Отсюда правило чтения: наличие шаблона в чарте не означает, что путь исполняется. Прежде
> чем закладываться на этот sidecar, надо ответить, что он решает сверх модели, — иначе
> появятся два места, принимающих одно решение (`security.md` §«Авторизация живёт в МОДЕЛИ»).

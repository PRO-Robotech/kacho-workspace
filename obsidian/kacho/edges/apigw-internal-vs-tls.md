...
| **internal** (cluster) | `api-gateway.kacho.svc.cluster.local:80` или alias | + `Internal*` RPCs (AddressPool, InternalNetwork, InternalCloud, InternalDiskType/Zone/Region для KAC-15) |
...
3. `/vpc/v1/addresses/{id}/addressPoolOverride` → internal.
4. `/vpc/v1/clouds/{cloud_id}/poolSelector` → internal.
5. Всё остальное `/vpc/v1/...` → public vpc (9090).

...
> [!warning] Исходящей доставки событий нет; отзыв сессий живёт ВНУТРИ (сверено 2026-08-05)
> Файл дренажа исходящих событий, названный в записке, не резолвится. Что есть на самом деле:
> **внутренний** контракт отзыва — `session_revocation.proto` /
> `session_revocations_service.proto`, репозитории отзывов сессий и токенов, и потребитель
> **внутри периметра** — край, который сверяется с отзывами (`gateway/internal/middleware`).
> Наружу (подписчику по протоколу событий безопасности) не уходит ничего.
>
> Разница не косметическая: «сессия отозвана» у нас означает «край перестанет её принимать»,
> а не «мы уведомили доверяющую сторону».

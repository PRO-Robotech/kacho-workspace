...
> [!warning] Это ЗАМЫСЕЛ дата-плейна, а не ребро: обеих сторон нет (проверено 2026-08-05)
> Ни вызывающего (репозиторий оператора не резолвится на GitHub, в монорепо ноль его
> файлов), ни встраиваемого модуля Cilium. Имя `cilium` встречается в дереве продукта в
> **5** файлах, и ни один из них не является этим модулем. Записка ценна как проектное
> решение о тенантинге (SRv6 VRF против политики), но проверять по ней стенд нечего, и
> ссылаться на неё как на «как это работает» нельзя. Смежное: [[vpc-operator-to-vpc-mtls]]
> (что от оператора живо на стороне платформы — SPIFFE-SAN и кластерный кортеж чтения).

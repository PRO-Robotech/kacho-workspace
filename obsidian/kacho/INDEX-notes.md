...
| [[resources/iam-token-family\|iam-token-family]] | в работе (test) |
...
| [[rpc/iam-access-key-service\|AccessKeyService (kaname)]] | в работе (test) |
...
| [[KAC/issue-1016\|#1016: эпик: единый поток изменений ресурсов для всех модулей]] | живо (done) |
| [[KAC/issue-1017\|#1017: контракт потока: оси подписки и курсор]] | живо (done) |
| [[KAC/issue-1018\|#1018: сервер потока живёт в фундаменте, а не в каждом сервисе]] | живо (done) |
...
| [[KAC/issue-1021\|#1021: консоль: снять поллинг списков там, где есть поток]] | в работе (in-progress) |
| [[KAC/issue-1022\|#1022: отзыв прав закрывает открытый поток]] | в работе (test) |
| [[KAC/issue-1023\|#1023: провязать поток изменений в остальных модулях]] | в работе (test) |
| [[KAC/issue-1024\|#1024: край сам открывает поток к iam — снять обратный вызов]] | в работе (test) |
| [[KAC/issue-1025\|#1025: серверный фильтр по меткам — завести после замера]] | в работе (planned) |
| [[KAC/issue-1043\|#1043: читатель снятого потока nlb жив и не зовётся ниоткуда]] | живо (done) |
| [[KAC/issue-1046\|#1046: мёртвая таблица курсоров compute приглашала отменить решение эпика]] | живо (done) |
| [[KAC/issue-1049\|#1049: снятая форма журнала осталась жить там, куда пробы не смотрят]] | в работе (test) |
| [[KAC/issue-1053\|#1053: чтение журнала ограничено сверху границей устоявшегося]] | в работе (test) |
| [[KAC/issue-1055\|#1055: радиус брали по имени файла, а свойство называлось иначе]] | в работе (test) |
| [[KAC/issue-106-kaname\|kaname#106: MRW-1 — группу пишущих кортежи объявляет манифест службы, а не SQL-свод]] | в работе (test) |
...
| [[KAC/issue-1106\|#1106: своя дверь авторизации у службы, у которой нет края]] | в работе (test) |
| [[KAC/issue-1109\|#1109: собственный REST-фронт службы — 103 привязки и ноль регистраций]] | в работе (test) |
| [[KAC/issue-1110\|#1110: генераторы каталога и маршрутов собираются вне монорепо]] | живо (done) |
| [[KAC/issue-1111\|#1111: веха M11 — разрез репозитория одним переключением]] | живо (active) |
...
| [[KAC/issue-1154\|#1154: новый домен не измерялся ни одним шардом — MR в ствол красен]] | живо (done) |
...
| [[KAC/issue-1231\|kacho#1231: удаление проекта сиротит ресурсы всех доменов — отказ при непустом проекте]] | в работе (in-progress) |
...
| [[KAC/issue-1238\|#1238: список адресов возврата замещается целиком]] | в работе (test) |
| [[KAC/issue-1239\|#1239: третья копия тела схемы личности мертва]] | в работе (test) |
...
| [[KAC/issue-1268\|#1268 Ф2 П2: проверяющий пароля следует за хранимым значением]] | в работе (in-progress) |
...
| [[KAC/issue-1274\|kacho#1274: экраны входа рисует своя консоль вместо чужого приложения]] | в работе (test) |
...
| [[KAC/issue-1348\|#1348: дублёр искажал тип значения поля с подсказками]] | живо (done) |
| [[KAC/issue-1349\|#1349: гейт покрытия не ловил подмену вида псевдонимом другого вида]] | живо (done) |
| [[KAC/issue-1350\|#1350: дублёр отдавал каскадный выбор плоским списком]] | живо (done) |
| [[KAC/issue-1351\|#1351: остаток стадии S4 отрыва идентичности — колонка и поля запросов живы]] | в работе (to-do) |
| [[KAC/issue-1352\|#1352: две полосы чтения выдач субъекта допускают разных распорядителей]] | в работе (to-do) |
...
| [[KAC/issue-1359\|#1359: рукописный перечень отстал от регистрации на три службы]] | в работе (test) |
...
| [[KAC/issue-1370\|#1370: тон отказа края отличим от отказа владельца]] | живо (done) |
| [[KAC/issue-1372\|#1372: допуск перечислял исход без производителя]] | живо (done) |
| [[KAC/issue-1373\|#1373: дельта величин читается по голому номеру — смена предела теряется навсегда]] | в работе (test) |
| [[KAC/issue-1374\|#1374: журнал смены субъекта читается по голому номеру — оповещение теряется навсегда]] | в работе (test) |
| [[KAC/issue-1375\|#1375: документ перечисляет снятую таблицу как опущенную для читаемости]] | в работе (test) |
| [[KAC/issue-1376\|#1376: счёт строк перед сносом таблицы возможен, но никем не делается]] | в работе (to-do) |
| [[KAC/issue-1378\|#1378: диаграмма схемы не рисует живые таблицы, и страница спорит сама с собой]] | в работе (in-progress) |
...
| [[KAC/issue-1381\|#1381: журнал nlb не несёт состояния — клиентский отбор остаётся без источника]] | в работе (in-progress) |
| [[KAC/issue-1382\|#1382: владелец не может назвать причину отсутствия состояния]] | в работе (test) |
| [[KAC/issue-1383\|#1383: две формы одного бинаря сведены к одной]] | в работе (test) |
| [[KAC/issue-1384\|#1384: гейт «пул помещается в базу» не считает соединения ВНЕ пула]] | в работе (test) |
| [[KAC/issue-1386\|#1386: край не отличает «журнал пуст» от «граница не устоялась»]] | в работе (to-do) |
| [[KAC/issue-1387\|#1387: гейт журнальных курсоров не видит закрытости на стороне писателя]] | в работе (test) |
| [[KAC/issue-1388\|#1388: подписка не включена ни одним профилем поставки]] | в работе (to-do) |
| [[KAC/issue-1389\|#1389: клиентская документация отрицает поставленную подписку]] | в работе (test) |
| [[KAC/issue-1390\|#1390: клиентской страницы подписки нет — вызов собирается из инженерных документов]] | в работе (test) |
| [[KAC/issue-1391\|#1391: предел потоков ниже, чем требует зернистость собственного фильтра]] | в работе (test) |
| [[KAC/issue-1392\|#1392: две таблицы владения на одном сайте называют владельца по-разному]] | в работе (test) |
| [[KAC/issue-1393\|#1393: ось видов объявлена, словаря нет, а вид пишется тремя способами]] | в работе (test) |
| [[KAC/issue-1394\|#1394: путь потока не маршрутизируется посредником — браузер получает страницу]] | в работе (test) |
| [[KAC/issue-1395\|#1395: перепрос отзыва сбрасывает весь кэш решений, зная имена субъектов]] | в работе (test) |
| [[KAC/issue-1396\|#1396: снять колонки доставки у журнала смены субъекта]] | в работе (to-do) |
| [[KAC/issue-1397\|#1397: общая форма подписки не принимает журнал смены субъекта]] | живо (done) |
| [[KAC/issue-1398\|#1398: канал пробуждения журнала смены субъекта остался без слушателя]] | в работе (test) |
| [[KAC/issue-1402\|#1402: гейт сторожит предел входа, который в поставке не рендерится]] | в работе (test) |
| [[KAC/issue-1405\|#1405: гейт честности утверждений красный на базе линии]] | живо (done) |
| [[KAC/issue-1410\|#1410: отзыв удостоверения не закрывает открытый поток]] | в работе (test) |
| [[KAC/issue-1412\|#1412: перенос читателя отзыва в фундамент отменяет обе починки #1022]] | в работе (test) |
| [[KAC/issue-1414\|#1414: у storage ресурсного журнала нет — подписку не к чему провязывать]] | в работе (test) |
| [[KAC/issue-1415\|#1415: у registry ресурсного журнала нет — очередь домена служит только правам]] | в работе (test) |
| [[KAC/issue-1416\|#1416: предикат «канал читает только фундамент» судит слово, а не оператор]] | в работе (test) |
| [[KAC/issue-1421\|#1421: дубль задачи о виде, которого у владельца нет]] | живо (done) |
| [[KAC/issue-1422\|#1422: отложенная проба зовёт вид предмета, которого у владельца нет]] | живо (done) |
| [[KAC/issue-1423\|#1423: отсутствие состояния объясняется неверной причиной]] | в работе (test) |
| [[KAC/issue-1424\|#1424: комментарий кадрировщика приписывает позиции чужую форму]] | в работе (test) |
| [[KAC/issue-1425\|#1425: распознаватель единственного производителя не знал склейки]] | живо (done) |
| [[KAC/issue-1426\|#1426: проба долга ждёт вида, которого журнал не пишет, и в четвёртом написании]] | в работе (test) |
| [[KAC/issue-1432\|#1432: гейт посадки судил наличие вызова, а не достижение отказа]] | живо (done) |
| [[KAC/issue-1435\|#1435: край не резолвит область из чужого ресурса — задача закрыта опровержением]] | живо (done) |
| [[KAC/issue-1436\|#1436: канал журнала намерений остался без слушателя — второй экземпляр класса]] | в работе (to-do) |
| [[KAC/issue-1442\|#1442: контракт советовал глагол, которого нет]] | в работе (test) |
| [[KAC/issue-1443\|#1443: ось достижения выхода была тождественно истинна на 5 вызовах из 6]] | живо (done) |
| [[KAC/issue-1445\|#1445: каталог проб, ждущих условия, не читает ни один разбор типов]] | в работе (to-do) |
| [[KAC/issue-1446\|#1446: фикстура потока берёт id из метаданных, не проверив исход операции]] | в работе (to-do) |
| [[KAC/issue-1447\|#1447: два написания одного домена: клиента отсылают к тому, которое не принимается]] | в работе (to-do) |
| [[KAC/issue-1448\|#1448: вводная страница описывает полирепо-топологию, которой нет]] | в работе (in-progress) |
| [[KAC/issue-1449\|#1449: регламент миграций объявляет применёнными девять при полусотне файлов]] | в работе (in-progress) |
| [[KAC/issue-1450\|#1450: полоса базового секрета — посылка опровергнута, остаток закрыт гейтом]] | живо (done) |
| [[KAC/issue-1451\|#1451: посадка compute обещает базе больше соединений, чем она принимает]] | в работе (in-progress) |
| [[KAC/issue-1452\|#1452: нагрузку журнала nlb пишут все мутации, а не читает никто]] | в работе (in-progress) |
| [[KAC/issue-1454\|#1454: проба чарта утверждает, что край знает владельца под другим именем]] | в работе (to-do) |
| [[KAC/issue-1460\|#1460: страница установки учит собирать образ из трёх сиблинг-репозиториев]] | в работе (to-do) |
| [[KAC/issue-1461\|#1461: у семи сервисов одной платформы четыре разных CLI миграторов]] | в работе (to-do) |
...
| [[KAC/issue-15-corelib\|corelib#15: гейт единственности словаря посадки — сторона потребителей держится обзором]] | в работе (test) |
...
| [[KAC/issue-1631\|#1631: R7 — клиентские поверхности сходятся между собой]] | живо (done) |
| [[KAC/issue-1632\|#1632: счётчики витрины читали 14 списков в минуту мимо потока]] | в работе (test) |
| [[KAC/issue-1633\|#1633: гейт владельца сверял с принимаемыми именами, а не с объявленными]] | в работе (test) |
| [[KAC/issue-1635\|#1635: карта «владелец → префикс вида» была второй точкой правки]] | в работе (test) |
| [[KAC/issue-1636\|#1636: мёртвая витрина в общем модуле, чей опрос читался как живой]] | в работе (test) |
| [[KAC/issue-1637\|#1637: у пробы наката миграций нет производителя]] | в работе (test) |
| [[KAC/issue-1650\|#1650: непокрытой была не одна форма вызова, а обе]] | в работе (test) |
| [[KAC/issue-1718\|#1718: R7-2 — находки, заведённые волнами R7]] | в работе (in-progress) |
| [[KAC/issue-1766\|#1766: пин на голый коммит резолвится — поэтому операции «поставить версию» никто не хватился]] | в работе (test) |
...
| [[KAC/issue-183-kaname\|kaname#183: полоса входа паролем судится сквозной пробой на посадке own]] | в работе (test) |
...
| [[KAC/issue-184-kaname\|kaname#184: копия каталога прав отстанет от края на строку Resolve — запись ведомости ожидания]] | в работе (test) |
| [[KAC/issue-1841\|#1841: свойство у гейта было, а держал его разовый прогон]] | в работе (test) |
| [[KAC/issue-1842\|#1842: сверялись с телом задачи, а не с приёмкой, которая его отменила]] | в работе (test) |
| [[KAC/issue-185-ws\|ws#185: решения о доступе идут одной дверью — и предмет задачи снят вместе с движком]] | живо (done) |
| [[KAC/issue-1852\|#1852: у оси приведения написаний не было производителя красного]] | в работе (test) |
| [[KAC/issue-1854\|#1854: обход гейта шире мотива, которым он объявлен]] | в работе (test) |
| [[KAC/issue-1856\|#1856: комментарий пережил норму, которую он пересказывал]] | в работе (test) |
| [[KAC/issue-1857\|#1857: две строки одной таблицы считали разным и назывались одинаково]] | в работе (test) |
| [[KAC/issue-1865\|#1865: наблюдение, называющее число вместо предмета]] | в работе (test) |
| [[KAC/issue-1873\|#1873: время правки роли — обещание контракта без единого производителя]] | в работе (test) |
| [[KAC/issue-188-kaname\|kaname#188: полоса входа — время исхода не называет класс хранимого значения]] | в работе (test) |
| [[KAC/issue-1889\|#1889: чистота текста отказа держалась тем, что никто не оборачивал]] | в работе (test) |
| [[KAC/issue-1890\|#1890: тождество текста объявлено прозой, а сверка сравнивала строки результата]] | в работе (test) |
| [[KAC/issue-1895\|#1895: гейт судил форму имени миграции и не судил отношение двух меток]] | в работе (test) |
| [[KAC/issue-1903\|#1903: полоса «наш дефект» существовала и не выбиралась ни разу]] | в работе (test) |
| [[KAC/issue-1905\|#1905: находка называла одну причину там, где выброшено три]] | в работе (test) |
| [[KAC/issue-1946\|#1946: страж паритета разрешал объявление края по имени файла]] | в работе (test) |
| [[KAC/issue-195-kaname\|kaname#195: Ф11 §9 п.3 и Ф3-45 — разный момент снятия сопоставления словаря поставщика на крае]] | в работе (in-progress) |
| [[KAC/issue-1961\|#1961: комментарий контракта сузил то, что домен разрешает]] | в работе (test) |
| [[KAC/issue-1964\|#1964: цена страницы фиксированного размера росла с популяцией]] | в работе (test) |
| [[KAC/issue-1995\|#1995: прибор нагрузки переводит имя типа сборкой — словарь назван границей]] | в работе (test) |
| [[KAC/issue-201\|Сцепка пользователя с аккаунтом: аккаунт по умолчанию неудаляем (#201)]] | в работе (in-progress) |
| [[KAC/issue-2025\|#2025: контракт обещал держателя, которого в дереве нет]] | в работе (test) |
| [[KAC/issue-2027\|#2027: перепись ролей, переживших своё объявление]] | живо (done) |
| [[KAC/issue-2035\|#2035: отчёты пакетной съёмки зовут дерево грязным из-за своих же соседей]] | живо (active) |
| [[KAC/issue-2036\|#2036: ось метки резолвит точечное имя порождённой таблицей, а не каталогом]] | в работе (test) |
| [[KAC/issue-2039\|#2039: отпечаток предмета замера ловил байты, а не поведение]] | в работе (test) |
| [[KAC/issue-2040\|#2040: отказ назвал координату, и координата была неверна]] | в работе (test) |
| [[KAC/issue-2041\|#2041: обход манифестов брал перечень у диска — вердикт становился свойством чужой рабочей копии]] | в работе (test) |
| [[KAC/issue-2042\|#2042: имя, которое живёт только в тексте о самом себе]] | в работе (test) |
| [[KAC/issue-2043\|#2043: одобренный текст датировал замер самоссылкой — правка идёт своим заходом]] | в работе (test) |
| [[KAC/issue-2044\|#2044: перечень нормы разошёлся с обходом второй раз — и оба раза тихо]] | в работе (test) |
| [[KAC/issue-2045\|#2045: «манифеста нет» сказано о манифесте, который есть]] | в работе (test) |
| [[KAC/issue-2046\|#2046: страница арендатора — та же клиентская поверхность, и она была вне обхода]] | в работе (test) |
| [[KAC/issue-2047\|#2047: комментарий описывал одну полосу авторизации, контракт рядом — другую]] | в работе (test) |
| [[KAC/issue-2048\|#2048: отказ называл отсутствие там, где предмет присутствует и занят]] | в работе (test) |
| [[KAC/issue-2049\|#2049: самопол судил легаси-скаляр, хотя набор субъектов лежал рядом загруженным]] | в работе (test) |
| [[KAC/issue-2050\|#2050: столбцы попыток создавали вид отсечки, которой не было]] | в работе (test) |
| [[KAC/issue-2052\|#2052: одна производная утверждения токена жила в двух слоях]] | в работе (test) |
| [[KAC/issue-2053\|#2053: столбец проиндексирован — это не то же, что «отбор пойдёт по индексу»]] | в работе (test) |
| [[KAC/issue-2054\|#2054: страница отдавала чужой курсор и платила за каждую строку]] | в работе (test) |
| [[KAC/issue-2055\|#2055: снятие собственного объекта было асимметрично его заведению]] | в работе (test) |
| [[KAC/issue-2056\|#2056: вход внутрь машины и управление ею требовали разного, и это не решалось]] | в работе (test) |
| [[KAC/issue-2057\|#2057: одно отношение, три перечисления — и приём уже применён рядом]] | в работе (test) |
| [[KAC/issue-2058\|#2058: список присваиваний вычислялся в рантайме — и это НЕ вопрос безопасности]] | в работе (test) |
| [[KAC/issue-2059\|#2059: посылка задачи была вычитана из комментария и наполовину ложна]] | в работе (test) |
| [[KAC/issue-2060\|#2060: мёртвая ветвь читалась следующим как действующий словарь]] | в работе (test) |
| [[KAC/issue-2062\|#2062: «скан не работал ни разу» и «наблюдения нет» выглядели одинаково]] | в работе (test) |
| [[KAC/issue-2063\|#2063: рабочий каталог не выбирает репозиторий, когда его уже выбрало окружение]] | в работе (test) |
| [[KAC/issue-2065\|#2065: единица счёта гейта уже текстового предиката — и это решение]] | в работе (test) |
| [[KAC/issue-2066\|#2066: необратимость выкатки обязана быть объявлена, а не получиться сама]] | в работе (test) |
| [[KAC/issue-2067\|#2067: комментарий пережил снятую блокировку и породил трёхмесячную задачу]] | в работе (test) |
| [[KAC/issue-2069\|#2069: очередей шесть, уборка у двух — класс чинили дважды по экземпляру]] | в работе (test) |
| [[KAC/issue-2071\|#2071: канон модели прав правили сегодня, шапку того же файла — нет]] | в работе (test) |
| [[KAC/issue-2072\|#2072: раздел приёмки объяснял отсутствие ключа, который с тех пор завели]] | в работе (test) |
| [[KAC/issue-2073\|#2073: величина в шапке без владельца — гейт сработал, находку никто не закрыл]] | в работе (test) |
| [[KAC/issue-2074\|#2074: собственный слушатель решал о доступе без надзорного звена]] | в работе (test) |
| [[KAC/issue-2075\|#2075: клауза сценария и запись замера — разные роды, второй править нельзя]] | в работе (test) |
| [[KAC/issue-2077\|#2077: звено объявлено и не построено — служба не принимала предъявленное на публичном слушателе]] | в работе (test) |
| [[KAC/issue-2078\|#2078: комментарий описывал режим, которого у развёрнутой службы не бывает]] | в работе (test) |
| [[KAC/issue-2079\|#2079: половину транспортных рёбер профилю модуля задавал зонтичный чарт]] | в работе (test) |
| [[KAC/issue-208\|issue-208 — ci: три накопителя без не-тестового читателя]] | в работе (test) |
| [[KAC/issue-2080\|#2080: подавление, написанное для инструмента, который его не читает]] | в работе (test) |
| [[KAC/issue-2083\|#2083: граница лицензий получила гейт, а предикат задачи ослеп на переименовании]] | в работе (test) |
| [[KAC/issue-2084\|#2084: шов «конвейер зовёт сверку копий каталога прав» не утверждался ничем]] | в работе (test) |
| [[KAC/issue-2086\|#2086: гейт обёрнутой сверки не видел блочную форму в оболочке]] | в работе (test) |
| [[KAC/issue-2087\|#2087: провод переименовывается весь — четыре поверхности сверх шести]] | в работе (test) |
| [[KAC/issue-2088\|#2088: страж старта отвергает базу прежней установки — переноса нет намеренно]] | в работе (test) |
| [[KAC/issue-2090\|#2090: умолчания чарта службы называли чужой релиз]] | в работе (test) |
| [[KAC/issue-2091\|#2091: три пробы посадки были неисполнимы в опубликованном артефакте]] | в работе (test) |
| [[KAC/issue-2092\|#2092: сканер безопасности никогда не читал модуль службы]] | в работе (test) |
| [[KAC/issue-2093\|#2093: закрыт ОПРОВЕРЖЕНИЕМ посылки — полоса «поле без читателя» пуста, слеп был обход]] | в работе (test) |
| [[KAC/issue-2094\|#2094: чарт объявлял умолчанием образ нашего стенда]] | в работе (test) |
...
| [[KAC/issue-21-corelib\|corelib#21: хук отправки фундамента и способ его провязки]] | в работе (test) |
...
| [[KAC/issue-2105\|#2105: две APPROVED-приёмки описывали исход переезда состояния неверно]] | в работе (test) |
| [[KAC/issue-2106\|#2106: свойство держалось сверкой, а не построением]] | в работе (test) |
| [[KAC/issue-211-identity-account-decoupling\|Отрыв аккаунта от пользователя — приёмка перед кодом (эпик #211)]] | в работе (test) |
| [[KAC/issue-2111\|#2111: под рабочим пространством резолв идёт в дерево, а конвейер собирает против пина]] | в работе (test) |
| [[KAC/issue-2113\|#2113: якорь переехал в Go и остался прежним в контракте, модели прав и манифестах]] | в работе (test) |
| [[KAC/issue-2115\|#2115: закрыт ОПРОВЕРЖЕНИЕМ посылки — у обоих полей читатель есть]] | в работе (test) |
| [[KAC/issue-2117\|#2117: модуль квотирования уходит из службы доступа — стадия S2, полоса контракта]] | в работе (in-progress) |
| [[KAC/issue-2121\|#2121: имя базы — четвёртая ось словаря, и переименование её не покрыло]] | в работе (test) |
| [[KAC/issue-2126\|#2126: условие готовности, проверяемое разовым перебором, не проверяется ничем]] | в работе (test) |
| [[KAC/issue-2127\|#2127: «отложено задачами» без задачи — клеймо токена осталось без владельца]] | в работе (test) |
| [[KAC/issue-2130\|#2130: комментарии называют пакет прежним именем (kacho.Reader)]] | живо (done) |
| [[KAC/issue-2133\|#2133: имя пакета контракта следует за продуктом — разрез один, окна нет]] | в работе (test) |
| [[KAC/issue-2135\|#2135: разбор поверхностей слова «квота» положен документом, две слепые зоны переписи закрыты]] | в работе (in-progress) |
| [[KAC/issue-2136\|#2136: имя образа в профиле не сверялось с источником имён]] | в работе (test) |
| [[KAC/issue-2137\|#2137: корень контрактов вне перечня входов не порождает ничего — успехом]] | в работе (test) |
| [[KAC/issue-2138\|#2138: популяция, суженная литералом приставки, молчит вместо того чтобы краснеть]] | в работе (test) |
| [[KAC/issue-2141\|#2141: боковой контейнер правил пережил своего потребителя]] | в работе (test) |
| [[KAC/issue-2144\|#2144: рецепт сборки модуля вне монорепо — и пометка [монорепо] как утверждение без держателя]] | в работе (test) |
| [[KAC/issue-2145\|#2145: гейты модуля выводили корень арифметикой пути — и выносили зелёный вердикт о чужом дереве]] | в работе (test) |
| [[KAC/issue-2146\|#2146: у наката своя дверь — страж посадки службы им не наследуется]] | в работе (test) |
| [[KAC/issue-2147\|#2147: самопроверки гейтов узнали второе имя продукта — и обе половины класса сразу]] | в работе (test) |
| [[KAC/issue-2149\|#2149: ведомость отстала от полосы, снявшей её предмет]] | в работе (test) |
| [[KAC/issue-2151\|#2151: ствол опубликованного продукта защищён — остаток снят настройкой, а не коммитом]] | живо (done) |
| [[KAC/issue-2154\|#2154: закрыта ОПРОВЕРЖЕНИЕМ посылки — канал к базе шифрован, а слеп был отбор популяции]] | в работе (test) |
| [[KAC/issue-2155\|#2155: доказательства падучести лежали в дереве и не звались ни одним прогоном]] | в работе (test) |
| [[KAC/issue-2158\|#2158: порождающий пинов узнал второе имя продукта — отбор спрашивается, а не выводится]] | в работе (test) |
| [[KAC/issue-2159\|#2159: резолв канона поднимался до корня файловой системы и находил чужой]] | в работе (test) |
| [[KAC/issue-2160\|#2160: третий исход назван прозой — и потому выглядел уже починенным]] | в работе (test) |
| [[KAC/issue-2161\|#2161: неверна была не раскладка лицензий, а ОДНА фраза об области]] | в работе (test) |
| [[KAC/issue-2162\|#2162: посадка стала ВЫРАЗИМОЙ — по ручке на слушателя, групповые сняты с отказом рендера]] | в работе (test) |
| [[KAC/issue-2165\|#2165: гейт поверхностей сбора не читал схему и был слеп к слушателю под TLS]] | в работе (test) |
| [[KAC/issue-2166\|#2166: девять сторожей вернулись, и ведомость снятого оказалась неполной]] | в работе (test) |
| [[KAC/issue-2167\|#2167: гейт был доказан инъекцией в обе стороны — и зелен на самом инциденте]] | в работе (test) |
| [[KAC/issue-2168\|#2168: форма записи каталога выводится у объявленного источника, а не выписывается приставкой]] | в работе (test) |
| [[KAC/issue-2169\|#2169: закрыта ОПРОВЕРЖЕНИЕМ посылки — адреса собственных фронтов доезжают]] | живо (done) |
| [[KAC/issue-2170\|#2170: восемь откаченных предметов возвращены — и задача НЕ утверждает, что их восемь]] | в работе (test) |
| [[KAC/issue-2172\|#2172: ведомость снятого пополнялась везением, а не проверкой]] | в работе (test) |
| [[KAC/issue-2173\|#2173: задокументированный вывод repohygiene называл снятую координату]] | в работе (test) |
| [[KAC/issue-2174\|#2174: у ведомости отката появился машинный предикат полноты — и он считает ИМЕНА]] | в работе (test) |
| [[KAC/issue-2176\|#2176: находка была ЛОЖНОЙ — гейт научен третьему виду значения, чарт не тронут]] | в работе (test) |
| [[KAC/issue-2177\|#2177: служба поднялась — предметов было ТРИ, и рендером не находится ни один]] | живо (done) |
| [[KAC/issue-2179\|#2179: две формы имени сверки TLS — расхождение, которого никто не решал]] | в работе (test) |
| [[KAC/issue-2180\|#2180: профиль безупречен, ручка задана — читателя нет в ЭТОЙ сборке]] | в работе (test) |
| [[KAC/issue-2182\|#2182: шапка pkg/authz откачена переездом контракта и снова называла мёртвые координаты]] | живо (done) |
| [[KAC/issue-2183\|#2183: седьмая точка наката — не недобор, а молчание]] | в работе (test) |
| [[KAC/issue-2185\|#2185: проверка формы имени сверки создавала вид проверки адресата]] | в работе (test) |
| [[KAC/issue-2186\|#2186: отказ транспорта сменился ответом СЛУЖБЫ — и это и есть вердикт]] | в работе (test) |
| [[KAC/issue-2187\|#2187: форму предохранителя скопировали, а её метку — нет]] | в работе (test) |
| [[KAC/issue-2188\|#2188: 34 упавших утверждения набора собственных фронтов — и два корня из двух разных семейств]] | в работе (test) |
| [[KAC/issue-2190\|#2190: судьба авторитета величин — решение, исход В (величину объявляет посадка)]] | в работе (test) |
| [[KAC/issue-2191\|#2191: собственный публичный фронт не различал предъявителя — читателя не поднимала ни одна посадка]] | в работе (test) |
| [[KAC/issue-2194\|#2194: самоотчёт печатал константу описания, а не состояние звена]] | в работе (test) |
| [[KAC/issue-2195\|#2195: пять утверждений собственного REST-фронта зеленели вакуумно]] | в работе (test) |
| [[KAC/issue-2196\|#2196: предохранитель называл предмет исключением — и запрос уходил]] | в работе (test) |
| [[KAC/issue-2197\|#2197: бюджет прогонов называл каталог nlb именем службы доступа]] | в работе (test) |
| [[KAC/issue-2198\|#2198: 103 — это КАНДИДАТЫ, и адъюдикация здесь дороже самой правки]] | в работе (test) |
| [[KAC/issue-2199\|#2199: комментарий контракта называл прежний репозиторий фундамента и уезжал в стаб]] | в работе (test) |
| [[KAC/issue-2201\|третий исход во всех восьми наборах сквозных проб, а не в одном]] | в работе (test) |
| [[KAC/issue-2202\|#2202: читатель имён обещал отказ, которого не бывает]] | в работе (test) |
| [[KAC/issue-2203\|#2203: приёмки службы называют путь монорепо — предмет опровергнут, пять из шести суть свидетельство]] | живо (done) |
| [[KAC/issue-2206\|#2206: гейт адресации набора ЗОВЁТСЯ — посылка задачи опровергнута замером]] | в работе (test) |
| [[KAC/issue-2207\|#2207: перечень, выписанный рядом с растущим составом, стареет молча]] | в работе (test) |
| [[KAC/issue-2208\|#2208: шапка резолвера адреса называла несуществующий вложенный ключ]] | в работе (test) |
| [[KAC/issue-2211\|#2211: подъём к go.mod не спрашивал, чей он модуль — в трёх местах]] | в работе (test) |
| [[KAC/issue-2212\|#2212: координата пережила переезд домена — и потому молчала, а не краснела]] | в работе (test) |
| [[KAC/issue-2214-verdict-names-a-revision\|Вердикт приёмки называет ревизию, а не файл (#2214)]] | живо (done) |
| [[KAC/issue-2214\|#2214: вердикт приёмки называет ревизию, а не файл — принят третий исход]] | в работе (test) |
| [[KAC/issue-2216\|#2216: объявленное отсутствие домена величин поднимало процесс и глушило мутации]] | в работе (test) |
| [[KAC/issue-2224\|#2224: гейт называл свою слепую зону честно — и она была вчетверо больше судимой]] | в работе (test) |
| [[KAC/issue-2225\|#2225: половина решения держится гейтом, вторая — ничем, и это сказано вслух]] | в работе (test) |
| [[KAC/issue-2226\|#2226: аккуратность автора — не держатель числа]] | в работе (test) |
| [[KAC/issue-2232\|#2232: три дерева называют три версии платформы — и каждое по отдельности собирается]] | в работе (test) |
| [[KAC/issue-2233\|#2233: страж старта был, а механизма сработать у него не было]] | в работе (test) |
| [[KAC/issue-2234\|#2234: ноль по предикату соседа означал другую форму записи, а не чистоту]] | в работе (test) |
| [[KAC/issue-2235\|#2235: имя, совпавшее в двух местах, перестало быть адресом — и испортило два замера]] | в работе (test) |
| [[KAC/issue-2236\|#2236: отказ цели назвал арендатору обходной путь, который у него не работает]] | в работе (test) |
| [[KAC/issue-2237\|#2237: проба сравнивала числа из разных моментов DOM — и обвиняла исправный продукт]] | в работе (test) |
| [[KAC/issue-2239\|#2239: область в ведомости объявлена координатой монорепо — один файл едет в обе посадки и верен в одной]] | в работе (test) |
| [[KAC/issue-2241\|#2241: словарь исходов объявлял четыре кода, а исполнитель пользовался двумя]] | в работе (test) |
| [[KAC/issue-2243\|#2243: уборка веток продукта уничтожит разрешимость пина службы — острота перемерена, запрет остаётся]] | в работе (to-do) |
| [[KAC/issue-2245\|#2245: имя двоичного файла — витрина, потому что его набирает оператор]] | в работе (test) |
| [[KAC/issue-2247\|#2247: умолчание снято у процесса и осталось у чарта]] | в работе (test) |
| [[KAC/issue-2250\|#2250: третий исход был ПРОИЗВЕДЁН и потерян по дороге к конвейеру]] | в работе (test) |
| [[KAC/issue-2254\|#2254: корень обхода литералом — проба вышла за пределы всякого репозитория]] | в работе (test) |
| [[KAC/issue-2260\|#2260: объём осмотренного доказывает полноту только относительно объявленной поверхности]] | в работе (test) |
| [[KAC/issue-2275\|#2275: ложно не только число в заголовке раздела, но и ВЫВОД из него]] | в работе (test) |
| [[KAC/issue-2305\|#2305: приёмка называла схему kacho_iam — дерево производит kaname]] | живо (done) |
| [[KAC/issue-2308\|#2308: §1.2 приёмки сегментов правила — числа без ревизии у блока]] | в работе (test) |
...
| [[KAC/issue-2415\|#2415: разрез kaname пересобирается от свежего ствола — перезаписью, не снимком]] | в работе (in-progress) |
| [[KAC/issue-242-kaname\|kaname#242: коллекция iam-membership-mine — запись ведомости производителя и захват тела]] | живо (done) |
| [[KAC/issue-242\|#242: docfresh — «не нашёл» неотличимо от «не искал»]] | в работе (test) |
| [[KAC/issue-2420\|#2420: ось якоря координаты судила один пакет — вердикт сужен до касаний дерева]] | в работе (test) |
| [[KAC/issue-2424\|#2424: два раздела приёмки требовали противоположного — закрыта опровержением посылки]] | живо (done) |
...
| [[KAC/issue-25-corelib\|corelib#25: правило git судят хук коммита, страж отправки и проверка запроса]] | в работе (test) |
...
| [[KAC/issue-2691\|kacho#2691: гейт достижимости пола «2» судит по посадке, а не по настройке поставщика]] | в работе (test) |
...
| [[KAC/issue-2733\|kacho#2733: ручки и проксирование консоли к чужим службам личности — снята первая часть]] | в работе (test) |
| [[KAC/issue-2735\|kacho#2735: посадка own во всех профилях, чужие службы личности выключены]] | в работе (test) |
| [[KAC/issue-2777\|kacho#2777: чужая консоль личности разворачивается и на посадке own]] | в работе (test) |
...
| [[KAC/issue-2780\|kacho#2780: церемонию входа консоли не проверяет ни одна браузерная проба]] | в работе (test) |
...
| [[KAC/issue-2796\|kacho#2796: волна-2 identity-own — сборки, влитые в ветку волны]] | в работе (test) |
...
| [[KAC/issue-2825\|kacho#2825: отправка одних удалений ветки не гонит проверки содержимого]] | в работе (test) |
| [[KAC/issue-2828\|kacho#2828: самопроверки deploy/scripts не зависят от репозитория, объемлющего TMPDIR]] | в работе (test) |
| [[KAC/issue-2831\|kacho#2831: код 2 от grep под set -e читается отказом, а не «условие не создано»]] | в работе (test) |
| [[KAC/issue-2832\|kacho#2832: доказательство герметичности выпуска несёт личность коммита]] | в работе (test) |
| [[KAC/issue-2833\|kacho#2833: счёт утверждений пробы берётся из неё, а не выписывается]] | в работе (test) |
| [[KAC/issue-2847\|kacho#2847: секция H самопроверки gosec не пишет в общий /tmp машины]] | в работе (test) |
| [[KAC/issue-285\|#285: версию генератора контракта выбирал PATH, а не дерево]] | в работе (test) |
| [[KAC/issue-2851\|kacho#2851: сервер пробы F1_46 считает прибытия только на своём адресе]] | в работе (test) |
| [[KAC/issue-2852\|kacho#2852: у каждого прогона ci-local свой каталог журналов]] | в работе (test) |
| [[KAC/issue-2855\|kacho#2855: сборка 1 волны 2796 — #2807 и #2833 одной веткой]] | в работе (test) |
| [[KAC/issue-2857\|kacho#2857: сборка 2 волны 2796 — own, F8, #2691 и оснастка одной веткой]] | в работе (test) |
| [[KAC/issue-2863\|kacho#2863: image-size 2.0.4 в lock-файлах сайтов документации]] | в работе (test) |
...
| [[KAC/issue-2873\|kacho#2873: край читает значение посадки external, снятое в фундаменте]] | в работе (to-do) |
| [[KAC/issue-2874\|kacho#2874: остаток ручек и проксирования чужих служб личности в консоли]] | в работе (to-do) |
| [[KAC/issue-2875\|kacho#2875: зонт не переносится в другое пространство имён]] | в работе (to-do) |
| [[KAC/issue-2876\|kacho#2876: security.txt объявляет Expires моментом рендера]] | в работе (to-do) |
...
| [[KAC/issue-2880\|kacho#2880: одновременные прогоны ci-local одной копии делят кэш и журналы]] | в работе (to-do) |
...
| [[KAC/issue-29-corelib\|corelib#29: волна-4 — словарь посадки без external, влита в ветку эпика]] | в работе (test) |
...
| [[KAC/issue-30-corelib\|corelib#30: значение посадки external снято со словаря фундамента]] | в работе (test) |
...
| [[KAC/issue-31-corelib\|corelib#31: конвейер гонится на запрос в ветку-номер]] | в работе (test) |
| [[KAC/issue-313-kaname\|kaname#313: своя запись кода авторизации и семейств токенов]] | в работе (test) |
| [[KAC/issue-314-kaname\|kaname#314: у жизненного цикла ключа подписи есть вызывающий]] | в работе (test) |
| [[KAC/issue-316-kaname\|kaname#316: обмен кода называет изоляцию и классифицирует ноль строк]] | в работе (test) |
| [[KAC/issue-317-kaname\|kaname#317: способ аутентификации клиента выражен схемой]] | в работе (test) |
| [[KAC/issue-319-kaname\|kaname#319: отзыв семейства и отсечка субъекта решаются одним читателем]] | в работе (test) |
| [[KAC/issue-32-corelib\|corelib#32: волна-2 — порты движка под службу доступа]] | в работе (test) |
| [[KAC/issue-320-kaname\|kaname#320: единственность поверхности церемонии держит гейт]] | в работе (test) |
| [[KAC/issue-321-kaname\|kaname#321: срок ключа подписи доезжает до стража незаданным]] | в работе (test) |
| [[KAC/issue-322-kaname\|kaname#322: своя посадка — клиент церемонии и разрыв сессии своим кодом]] | в работе (test) |
| [[KAC/issue-324-kaname\|kaname#324: хук отправки и цели его провязки]] | в работе (test) |
| [[KAC/issue-325-kaname\|kaname#325: личный адрес правообладателя объявлен в ведомости вклада]] | в работе (test) |
| [[KAC/issue-329\|#329: отказ хука читается как «не выполнилось», а не как расхождение]] | в работе (test) |
| [[KAC/issue-33-corelib\|corelib#33: причина отзыва гранта — из закрытого словаря]] | в работе (test) |
| [[KAC/issue-334-kaname\|kaname#334: своя причина снятия сессии при административном выходе]] | в работе (test) |
| [[KAC/issue-337-kaname\|kaname#337: посадка own без своего контура выдачи не поднимается]] | в работе (test) |
| [[KAC/issue-34-corelib\|corelib#34: токен доступа выпускает порт службы AccessTokenIssuer]] | в работе (test) |
| [[KAC/issue-340-kaname\|kaname#340: запись принудительного выхода кладётся после снятия и несёт исход]] | в работе (test) |
| [[KAC/issue-341-kaname\|kaname#341: ветвь default двери отсечки судится внутрипакетной пробой]] | в работе (test) |
| [[KAC/issue-349-kaname\|kaname#349: хук отправки службы доступа и переходник, который отказывает]] | в работе (in-progress) |
| [[KAC/issue-35-corelib\|corelib#35: ID гранта чеканит крючок службы Config.NewGrantID]] | в работе (test) |
...
| [[KAC/issue-358-kaname\|kaname#358: волна-2 identity-own — сборки, влитые в ветку волны]] | в работе (test) |
| [[KAC/issue-36-corelib\|corelib#36: сессия, acr и auth_time — поля гранта и сеанса]] | в работе (test) |
| [[KAC/issue-363-kaname\|kaname#363: двухрежимный переключатель посадки own/external не снят]] | в работе (to-do) |
| [[KAC/issue-364-quota-console\|Витрина квот в консоли: арендатор видит предел, занято и источник (#364)]] | в работе (in-progress) |
| [[KAC/issue-365-kaname\|kaname#365: волна-1 identity-own — стенд службы на own, влита в ветку эпика]] | в работе (test) |
| [[KAC/issue-369-kaname\|kaname#369: пробы выдачи внахлёст и ключа живости семейства]] | в работе (test) |
| [[KAC/issue-37-corelib\|corelib#37: секрет клиента сверяет порт службы, не хешер движка]] | в работе (test) |
| [[KAC/issue-370-kaname\|kaname#370: освобождения ведомости называют свои вклады и действующий механизм]] | в работе (test) |
| [[KAC/issue-373-kaname\|kaname#373: записи ведомости вклада о подписи и атрибуции — по действующим решениям]] | в работе (test) |
| [[KAC/issue-373-list-scope-honesty\|#373: ручка списка называет область, в которой судит]] | в работе (test) |
| [[KAC/issue-374-kaname\|kaname#374: род писателя семейства назван без слова Token]] | в работе (test) |
...
| [[KAC/issue-379-kaname\|kaname#379: полосы выдачи токена клиента сверяются с отзывом одним правилом]] | в работе (test) |
| [[KAC/issue-38-corelib\|corelib#38: адреса authorize и token названы в Config]] | в работе (test) |
| [[KAC/issue-382-ws\|ws#382: переход standalone-iam — условие для работы создано, волна П7 сведена]] | в работе (in-progress) |
| [[KAC/issue-39-corelib\|corelib#39: сроки кода и refresh судятся потолками tokenpolicy]] | в работе (test) |
| [[KAC/issue-393-kaname\|kaname#393: генератор набора newman: повтор окна прав только на ожидаемом успехе]] | в работе (test) |
| [[KAC/issue-394-kaname\|kaname#394: конвейер идёт на запросе в ветки волн и эпика]] | в работе (test) |
| [[KAC/issue-395-kaname\|kaname#395: отказ CHECK на значении службы отвечает INTERNAL]] | в работе (test) |
| [[KAC/issue-396-kaname\|kaname#396: адаптеры портов токена доступа исполняют контракт порта]] | в работе (test) |
| [[KAC/issue-397-kaname\|kaname#397: сцена конкуренции ForceLogout строится наблюдением, а не выдержкой]] | в работе (test) |
| [[KAC/issue-399-kaname\|kaname#399: чистильщик отчёта newman судит каждую часть документа]] | в работе (test) |
| [[KAC/issue-40-corelib\|corelib#40: привязка PKCE — поля записи кода авторизации]] | в работе (test) |
| [[KAC/issue-401-kaname\|kaname#401: коллекции newman гонит шаг конвейера]] | в работе (test) |
| [[KAC/issue-402-kaname\|kaname#402: записи ревью приведены к опубликованному событию]] | в работе (test) |
...
| [[KAC/issue-408-kaname\|kaname#408: сборка 1 волны 358 — шесть задач под один прогон]] | в работе (test) |
| [[KAC/issue-41-corelib\|corelib#41: артефакт снятого клиента отвечается как негодный]] | в работе (test) |
| [[KAC/issue-410-kaname\|kaname#410: адаптеры церемонии на фундаменте corelib v1.10.0-rc.2]] | в работе (test) |
| [[KAC/issue-411-quota-unity\|Единство квот: что едино, что не будет, и где записаны решения]] | в работе (in-progress) |
| [[KAC/issue-412-kaname\|kaname#412: сборка 2 волны 358 — шесть задач под один прогон]] | в работе (test) |
| [[KAC/issue-412\|#412: чтение квот арендатором заводится у всех пяти списывающих доменов]] | в работе (test) |
| [[KAC/issue-414-kaname\|kaname#414: пять записей ревью ветки 358 ссылаются на событие санкции]] | в работе (test) |
| [[KAC/issue-415-kaname\|kaname#415: коллекции предмета платформы без производителя в конвейере]] | в работе (to-do) |
| [[KAC/issue-418-kaname\|kaname#418: сборка 3 волны 358 — три задачи под один прогон]] | в работе (test) |
...
| [[KAC/issue-42-corelib\|corelib#42: интроспекция отвергает способ, которого точка не принимает]] | в работе (test) |
| [[KAC/issue-420-kaname\|kaname#420: сборка 4 волны 358 — четыре задачи под один прогон]] | в работе (test) |
...
| [[KAC/issue-428-kaname\|kaname#428: ветвь предупреждения о выдаче ключей не сведена со стражем #337]] | в работе (to-do) |
| [[KAC/issue-428\|issue-428 — ui: подмена antd брала дублёр динамическим импортом]] | в работе (test) |
| [[KAC/issue-429-kaname\|kaname#429: голова ветки эпика без образа — связку линии не выкатить]] | в работе (to-do) |
| [[KAC/issue-429-ws\|ws#429: посадка слияния может приезжать не из дерева]] | в работе (test) |
| [[KAC/issue-43-corelib\|corelib#43: откат единицы работы не наследует срок операции]] | в работе (test) |
...
| [[KAC/issue-44-corelib\|corelib#44: поверхность токена личности без обработчика снята]] | в работе (test) |
...
| [[KAC/issue-45-corelib\|corelib#45: мелочи ревью церемонии: заголовки проб, алфавиты, тексты отказов]] | в работе (test) |
...
| [[KAC/issue-47-corelib\|corelib#47: выданная область — scope-token по RFC 6749 §3.3]] | в работе (test) |
...
| [[KAC/issue-486\|issue-486 — третий собственный отказ ядра не был доказан ничем]] | живо (done) |
...
| [[KAC/issue-49-corelib\|corelib#49: ранжирование уровня аутентификации — в пакете без транспорта]] | в работе (test) |
...
| [[KAC/issue-491-ws\|ws#491: долгий прогон молчал, и «идёт» было неотличимо от «повис»]] | живо (done) |
| [[KAC/issue-492-ws\|ws#492: фикстура вердикта артефакта заведена, правила у неё ещё нет]] | живо (done) |
| [[KAC/issue-493\|issue-493 — перечень ожиданий у инъекций ядра выписан именами]] | живо (done) |
| [[KAC/issue-494-ws\|ws#494: диагностика семейства драйвера впервые получила исполняемый кейс]] | живо (done) |
...
| [[KAC/issue-50-corelib\|corelib#50: определение гонки в шапках проб — как у детектора]] | в работе (to-do) |
...
| [[KAC/issue-51-corelib\|corelib#51: срез настроек движка с запасом ёмкости — общий массив]] | в работе (to-do) |
...
| [[KAC/issue-53-corelib\|corelib#53: держатель выреза телеметрии по путям движка]] | в работе (to-do) |
...
| [[KAC/issue-54-corelib\|corelib#54: записи пост-дифф ревью #20 лежат в дереве продукта]] | в работе (to-do) |
...
| [[KAC/issue-55-corelib\|corelib#55: отказ отзыва без причины не заносится в ведомость]] | в работе (to-do) |
...
| [[KAC/issue-56-corelib\|corelib#56: у каждого поля Config, что судит New, есть читатель]] | в работе (test) |
...
| [[KAC/issue-57-corelib\|corelib#57: отказ адреса называет правило схемы, хоста и пути]] | в работе (test) |
...
| [[KAC/issue-58-corelib\|corelib#58: повтор ключа YAML — исход 2 гейта целевых веток]] | в работе (test) |
...
| [[KAC/issue-59-corelib\|corelib#59: прочтение success() по цепочке needs подтверждено запуском]] | в работе (test) |
| [[KAC/issue-60-corelib\|corelib#60: текст отказа при вырезании предъявленного]] | в работе (to-do) |
...
| [[KAC/issue-61-corelib\|corelib#61: комментарий Form не называет code из перечня записи]] | в работе (to-do) |
| [[KAC/issue-618-ws\|ws#618: приёмка KAN-AUTHN-1 — предикат §7 п.4 видит корень чарта продукта]] | в работе (test) |
| [[KAC/issue-63-corelib\|corelib#63: сборка волны 32 — сопряжение #36 с #37 и #39]] | в работе (test) |
| [[KAC/issue-636\|#636: локатор строки формы попадал в объемлющий блок]] | в работе (test) |
| [[KAC/issue-64-corelib\|corelib#64: ACRRank и ACRSatisfies возвращены устаревшими псевдонимами]] | в работе (test) |
...
| [[KAC/issue-68-corelib\|corelib#68: сборка 1 волны 32 — полосы 25, 45, 41, 42]] | в работе (test) |
...
| [[KAC/issue-70-corelib\|corelib#70: серверное слияние судится формой площадки]] | в работе (test) |
...
| [[KAC/issue-773-ws\|ws#773: приёмка F8 — свои экраны церемоний личности в консоли]] | в работе (test) |
...
| [[KAC/issue-805-ws\|ws#805: волна-2 оснастки — приёмки F8 и KAN-AUTHN-1, флоу v2, записи ревью]] | в работе (test) |
| [[KAC/issue-808\|#808: три стенда с боевой посадкой наследуют бюджет памяти баз от профиля разработки]] | в работе (in-progress) |
| [[KAC/issue-810-ws\|ws#810: удаление ветки не гонит наборы воркспейса по рабочей копии]] | в работе (test) |
| [[KAC/issue-810\|issue-810 — iam: накопленные выдачи на удалённых проектах снимаются разовой уборкой]] | в работе (test) |
| [[KAC/issue-811-ws\|ws#811: отправка воркспейса судит дерево отправляемой вершины]] | в работе (test) |
...
| [[KAC/issue-813-ws\|ws#813: перепись веток — режим кандидатов, судит только названные ветки]] | в работе (test) |
| [[KAC/issue-814\|#814: поток жизненного цикла nlb снят — у него не было ни одного потребителя]] | в работе (test) |
| [[KAC/issue-815-ws\|ws#815: самопроверка crossrepo-gate снимает KACHO_HOME_*]] | в работе (test) |
| [[KAC/issue-818-ws\|ws#818: дайджест записей ревью — в форме git diff --full-index]] | в работе (test) |
| [[KAC/issue-823-ws\|ws#823: записи пост-дифф ревью полосы kaname #340]] | в работе (test) |
| [[KAC/issue-825-ws\|ws#825: записи пост-дифф ревью полос corelib волны-2]] | в работе (test) |
| [[KAC/issue-826-ws\|ws#826: правила: край проверяет newman чёрным ящиком]] | в работе (test) |
| [[KAC/issue-827-ws\|ws#827: записи пост-дифф ревью полосы kaname #305]] | в работе (test) |
| [[KAC/issue-829-ws\|ws#829: правила: вердикт по прогонам, план опыта, сборка волны]] | в работе (test) |
| [[KAC/issue-830-ws\|ws#830: записи ревью полос kacho волны-2]] | в работе (test) |
| [[KAC/issue-832-ws\|ws#832: флоу v2: код первым, агрегаты, память не выше 45 ГБ]] | в работе (test) |
| [[KAC/issue-833-ws\|ws#833: trail сборок волны kacho#2796 — задачи со статусом test]] | в работе (test) |
| [[KAC/issue-835-ws\|ws#835: сборка 1 волны 805 — оснастка и записи ревью]] | в работе (test) |
| [[KAC/issue-839-ws\|ws#839: ветка docfresh сведена в волну коммитом слияния]] | в работе (test) |
| [[KAC/issue-846-ws\|ws#846: trail сборки 2 волны kacho#2796 — остаток #833]] | в работе (in-progress) |
| [[KAC/issue-850-ws\|ws#850: values.prod требуют подъёма, конвейер поднимает dev-prod]] | в работе (to-do) |
...
| [[lessons/one-language-predicate-undercounts-a-bilingual-corpus\|Одноязычный предикат недобирает по двуязычному корпусу молча — и недостача ложится туда, где предмет объясняли словами]] | живо (active) |
...
| [[lessons/proof-runner-crashed-and-silence-looked-like-cost\|Доказательство падучести перестало запускаться, и молчание читалось как цена]] | живо (stable) |
...
| [[lessons/required-check-without-a-producer-blocks-merge-forever\|Требование контекста, которого никто не производит, блокирует слияние навсегда — и красного при этом нет]] | живо (active) |

...
|---|---|---|
| `kacho-iam` | Account/Project/User/SA/Group/Role/AccessBinding + authz/reviews/GDPR/JIT | [[_l1-kacho-iam]] |
| `kacho-vpc` | Network/Subnet/SG/RouteTable/Address/Gateway/PE/NIC | [[_l1-kacho-vpc]] |
| `kacho-compute` | Instance/Disk/Image/Snapshot + Geography | [[_l1-kacho-compute]] |
| `kacho-api-gateway` | edge: gRPC-proxy + REST mux | [[_l1-kacho-api-gateway]] |
...
doc-коммента → FAIL). См. [[corelib-archgraph]].

## Статус раскатки

L1/L2 + L3/L4 — заведены для `kacho-iam`, `kacho-vpc`, `kacho-compute`,
`kacho-api-gateway`. **C1 completeness PASS** и **C4 doc-coverage PASS** у всех
четырёх (≈3240 функций/переменных получили doc-комментарии). Остаётся: триаж
C2-findings (мёртвый код), заполнение `source_sha` (C3), CI-job `arch-vault`.

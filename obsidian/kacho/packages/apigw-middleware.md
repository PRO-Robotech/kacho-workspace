...
tags:
  ...
  - kacho-apigw
  - middleware
verified_against: "каталог пакета есть в дереве продукта b4edc5d5 (2026-08-05); полные имена обоих внутренних методов соседа перемерены на f445aaaa554 (2026-09-22) по каталогу прав края и по proto службы доступа; остальной текст построчно не пересматривался"
---
...
## Полоса отзыва спрашивается НА ПРЕДЪЯВЛЕНИИ

Отсечка сессии судится здесь, на пути запроса: посредник спрашивает соседа по внутреннему
слушателю на **каждом** предъявлении, а не опирается на состояние, розданное при выдаче. Ровно
это и есть предмет урока
[[lessons/revocation-that-binds-at-issue-not-at-presentation]].

Глаголы соседа: [[rpc/iam-internal-session-revocations-service]] (`IsRevoked`,
`SessionCutoffOf`) и [[rpc/iam-internal-human-session-service]] (`Resolve`). Переходник,
которым посредник до них доходит, — [[packages/apigw-clients]]; **один** переходник на трёх
соседей по одному соединению, и довод этого решения записан там.

Отсечка сравнивается **включительно**, и обе стороны сравнения едут по проводу неусечёнными —
подробность живёт у метода, а не здесь.

## History

- 2026-09-22 — добавлен раздел о полосе отзыва на предъявлении со ссылками на оба внутренних
  метода соседа. Повод: возврат полосы края назвал `gateway/internal/middleware` затронутым
  пакетом и оба метода — затронутыми RPC.

...
> [!note] Здесь стояло «рядом живёт второй модуль, `…/terraform`» — предмета у этого нет
> Провайдер Terraform заводился отдельным Go-модулем под-фазой TF-1 и **сведён обратно в
> общий** 2026-08-12 (замер довода об изоляции его не подтвердил). На дереве `bfaf239b`
> `go.mod` ровно один, поэтому фраза о втором модуле описывала неделю жизни дерева и
> пережила её. Что осталось верным: код провайдера в бинари и образы сервисов **не
> попадает** — граф импортов сервиса не содержит ни одной строки `hashicorp`. Устройство и
> цена сведения — [[packages/kacho-terraform-provider]].

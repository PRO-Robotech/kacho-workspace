...
verified_against: "число гейтов репо-гигиены и пин фундамента перемерены на дереве продукта f445aaaa554 (2026-09-22) — 652 файла проб и `corelib v1.8.0` в `go.mod:119`; пины инструментов перемерены на b0de60a05 (2026-08-09) и с тех пор не пересматривались; раскладка каталогов, гочи и правило исключения секретов — 1653387b (2026-08-06); замеры времени CI и числа пакетов построчно не пересматривались"
---

# kacho — монорепа (замещает polyrepo)

`github.com/PRO-Robotech/kacho` — **один Go-модуль на всю платформу**, и это буквально:
`go.mod` в дереве один. Создана 2026-07-15/16. Старые `kacho-*` репозитории **не тронуты** и
остаются архивом; история в монорепу не переносилась (осознанное решение).

Проверяется предикатом, а не перечитыванием (ревизия `5df7da76`, `origin/main`):
...
Ответ, отличный от `1 / 0`, означает, что топология изменилась и этот абзац описывает
снятое — вместе с [[packages/kacho-terraform-provider]] §«Топология» и
`.claude/rules/polyrepo.md` §«Build-граф». Три места об одном предмете: правятся одним
заходом либо не правятся вовсе.

Провайдер Terraform неделю жил обособленно и **сведён обратно** 2026-08-12 — почему довод об
изоляции не пережил замера и чего стоило обособление, разобрано у соседа:
[[packages/kacho-terraform-provider]]. Здесь это не пересказывается.
...
Перечень выше — примеры, а не состав: гейтов существенно больше, и выписывать их сюда значило
бы завести второй список, который разойдётся с деревом. Число берётся предикатом, а не отсюда:
`ls internal/repohygiene/*_test.go | wc -l` → **56** на `b0de60a05` (2026-08-09) и **652** на
`f445aaaa554` (2026-09-22). Единица счёта одна и та же — файл пробы в корне каталога; выросло
само дерево, а не предикат. Два свойства, обязательные для **каждого** из них,
...
## Чем каталог гейтов связан с фундаментом

Правило «git зовётся через единую точку, а не голым вызовом» опирается на пакет фундамента,
который платформа берёт **пином**, а не копией: ребро — [[edges/kacho-to-corelib-module-pin]].
На 2026-09-22 пин `v1.8.0`, и он **не правился**.

## History

- 2026-09-22 — число гейтов репо-гигиены перемерено тем же предикатом на сегодняшнем дереве
  (56 → 652, обе ревизии названы); заведена ссылка на ребро пина фундамента. Повод: возврат
  полосы назвал `internal/repohygiene` и ребро `kacho → corelib` (пакет `gitenv`) затронутыми.
  Отдельной записки на `internal/repohygiene` **не заводится**: её предмет изложен здесь, и
  второй дом разошёлся бы с первым молча.

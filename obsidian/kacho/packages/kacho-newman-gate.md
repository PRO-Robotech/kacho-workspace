...
verified_against: "координата гейта перемерена по стволу продукта 2026-09-09 после переезда слоя в корневой каталог (фаза A приёмки удаления службы); числа падений и разбор корней — история прогонов 2026-07, по дереву не переподтверждались"
---

# newman: гейт и почему он краснеет

Вердикт по e2e выносит **не exit-код newman**, а `tests/newman/scripts/assert-suites-green.sh`.
Скрипт ОДИН и применяется к каждому набору **с его cwd** — он глобит `collections/` +
`out/` относительно pwd. Так же было в polyrepo; при переезде в монорепу вызов потерялся,
и гейтом стал голый exit newman → краснели кейсы RED-by-design.

> [!note] Скрипт переехал из каталога службы доступа в корневой — 2026-09-08
> Он жил в `services/iam/`, и это было верно, пока служба оставалась частью монорепо.
> Вынос её отдельным продуктом сделал такое размещение неисполнимым: **98 коллекций из
> 98** потеряли бы вердикт, третью категорию и перепись исполненности вместе с каталогом
> — то есть их потеряли бы ВСЕ наборы, а не один. Приёмка удаления называет это пунктом
> фазы A: работой, которую после удаления сделать уже нельзя.
>
> Довод размещения — от вызывающего, а не от вкуса: ни один не адресует слой «из своего
> каталога», восемь шагов конвейера берут его от корня рабочей области, два прогонщика —

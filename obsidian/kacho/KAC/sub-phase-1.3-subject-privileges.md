...
- **kacho-ui #88** (table/cascader polish): (A) теги ролей — **полное имя** (`displayRender` + хелпер `roleTagName`: wildcard→verb, custom→name, system→`m.res.verb`); (B) **«Свои роли» — flat-список** в каскадере (system = 3-уровневое дерево, custom = top-level без children; `roleIdToPath` учитывает `is_system`); (C) **таблица привилегий зеркалит LIST `/iam/access-bindings`** (колонки из `REGISTRY["access-bindings"]` минус «Субъект») + колонка «Привязка» с `<Link>` на конкретный binding; (D) колонка **«Кто выдал»** (`granted_by_user_id`, IamRefLink users/email) в access-bindings-списке И таблице привилегий — proto field 11 уже был, UI-only (+ поле в TS-интерфейс). Деплой: fe3455 rev20 (ui `main-33279ffd`).

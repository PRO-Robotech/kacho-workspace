...
> [!note] UI: вкладка, кнопка page-header CTA, встроенное создание, multi-role
> Финал: «Привилегии» — detail-**вкладка** (`extraTabs`) на User/SA/Group; кнопка «Добавить привилегии» — page-header CTA (`useHeaderRight`); создание привязки **встроено в зону-3** (mainOverride child-create, субъект залочен); роль — мультивыбор тегами → N биндингов за раз. Таблица — `ResourceTable` (паритет с access-bindings).

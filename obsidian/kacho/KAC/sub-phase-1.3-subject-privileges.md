...
- **kacho-ui #86**: (a) дропдаун ролей **сгруппирован по модулям** (OptGroup по `Role.name.split(".")[0]`: VPC/IAM/COMPUTE/NLB/…, wildcard-роли → «Глобальные»; поиск сквозь группы). (b) **Reconcile-режим** (только `lockedSubject`/привилегии): форма предзагружает текущие роли субъекта на выбранном ресурсе (по умолчанию home-account) и при сохранении **актуализирует** — added→create, removed→revoke (DELETE `/iam/v1/accessBindings/{binding_id}`). Подсказка «Добавить N · Отозвать M». SAFETY: revoke-карта `roleId→binding_id` строится **только из DIRECT-привязок ровно текущего (resource_type,resource_id)** → group-derived и другие ресурсы не трогаются; partial-failure-safe. Standalone `/iam/access-bindings/create` — additive-only (без reconcile). Деплой: fe3455 rev18 (ui `main-11d56697`).

> [!note] UI privileges — финальное состояние
> «Привилегии» — detail-**вкладка** (`extraTabs`) на User/SA/Group; кнопка «Добавить привилегии» — page-header CTA (`useHeaderRight`); создание/правка привязок **встроено в зону-3** (mainOverride child-create, субъект залочен); роль — **мультивыбор тегами, сгруппированный по модулям**; форма работает как **reconcile** (предзагрузка текущих DIRECT-ролей ресурса → add+revoke за одно сохранение, safety-scoped). Таблица — `ResourceTable` (паритет с access-bindings).

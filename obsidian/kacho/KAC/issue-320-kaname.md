---
title: "kaname#320: единственность поверхности церемонии держит гейт"
aliases:
  - issue-320-kaname
ticket_id: 320
category: kac
status: done
type: fix
repos:
  - kaname
areas:
  - internal/check
  - cmd/kaname
  - internal/handler/clienttokenhttp
prs:
  - https://github.com/PRO-Robotech/kaname/pull/411
issue_url: https://github.com/PRO-Robotech/kaname/issues/320
opened: 2026-09-20
tags:
  - kac
  - kacho-iam
  - fix
verified_against: "PRO-Robotech/kaname: голова задачи e4048dc37d9fe9b4fcd640c9d326dc57015901ba (= origin/320) — предок головы сборки 2f45e7c8aa0 и origin/358, не предок origin/357 и origin/main (`git merge-base --is-ancestor`), 2026-09-24. Предикат снятия мной не перемерялся; закрытие — `gh issue view 320 -R PRO-Robotech/kaname --json state,closedAt` — CLOSED 2026-09-26T09:38:17Z; голова `e4048dc37d9` — предок origin/357 @ fc9f5aff19c, не предок origin/main @ cbbac984b7b (`git merge-base --is-ancestor`, перемерено мной 2026-09-26)"
---

# kaname#320: единственность поверхности церемонии держит гейт

**Состояние на момент записи**: `test` — 2026-09-24. Задача **открыта**, метка `status:test`.
Влита в ветку волны [[KAC/issue-358-kaname|#358]] сборкой 1 и в `main` **не** доехала.
Тогдашняя норма держала задачу волны открытой до посадки эпика в ствол; её отменил каскад
закрытия 2026-09-26.

**Состояние на 2026-09-26**: `done`. Закрыта вместе со своей волной
[[KAC/issue-358-kaname|#358]]: запрос волны #422 влит в ветку эпика `357` коммитом слияния
`fc9f5aff19c`, и по каскаду закрытия (`git-issues.md#gi-close-cascade`, решение владельца
2026-09-26) задачи волны закрыты тем же заходом. Закрывающая запись с коммитом работы и исходом
предиката — комментарий в задаче. Посадка эпика #357 в `main` — предмет эпика, а не задачи.

## Что и зачем

Проба монтажа держала свойство «церемония живёт ровно на одной поверхности» перечнем
слушателей и путей, **выписанным** на момент написания: слушатель, заведённый позже, в
перечень не попадал, и проба оставалась зелёной. Задача заводит держатель на класс — гейт,
**выводящий** места регистрации из исходника композиционного корня разбором, печатающий
объём осмотренного и падающий на пустом обходе. Находка `LA1-SEC-M5` разбора поверхности
безопасности под-фазы LINE-A-1.

## Путь в волну

| что | координата |
|---|---|
| голова первого сведения | `074b2fd3e51763902a3118f8a7e1b56a8653dd92` → слияние `6e7fc2443721f94dc232761d002d25356ca970b0` |
| голова повторного сведения | `e4048dc37d9fe9b4fcd640c9d326dc57015901ba` → слияние `351a853b117e19e89a75d8312678461481e855ef`; ветка `320` (координата, не живая ссылка) на origin с этой головой |
| запрос сборки | PR #411 `408` → `358`, голова `2f45e7c8aa0ebb6a1ec25ff26523cc70e205bb03`, коммит слияния `d22123ba0957625fa791649bd72b6fffd694cfa0` |

Повторное сведение несёт обоснование чтения исходника для сканера и суд над монтажом
обработчика под чужим путём; таблица тела PR #411 называет голову первого сведения.

## DoD (из тела задачи)

- [x] ветка задачи влита в ветку волны `358` коммитом слияния — через сборку 1;
- [x] вердикт головы сборки: 20 заданий, 17 `success`, 3 `skipped`, 0 упавших;
- [x] гейт судит узел разбора, рядом проба предпосылки, инъекция второй регистрацией с
      законным близнецом, гейт позван конвейером — снято по закрывающей записи в задаче, 2026-09-26: гейт выводит места
      регистрации из композиционного корня, печатает объём осмотренного и падает на пустом
      обходе; принято посадкой сборки 1 #408, запрос #411; мной не перемерялось;
- [x] задача закрыта вместе с волной #358: её запрос #422 влит в ветку эпика `357`,
      голова задачи `e4048dc37d9` — предок origin/`357` и не предок origin/`main` (перемерено
      мной 2026-09-26). Посадка в `main` — DoD эпика #357, а не задачи.

## Затронутые сущности vault

Поля «затронуто в vault» нет; узкие записки этой записью не менялись.

- [[KAC/issue-358-kaname]] — волна и сборка 1.

#kac #kacho-iam #fix

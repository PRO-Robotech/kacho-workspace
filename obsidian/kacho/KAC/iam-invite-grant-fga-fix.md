...
>
> **done.** Все три причины закрыты в дереве: эмиссия на якорь уровня живёт в `services/iam/internal/apps/kacho/api/access_binding/`, активация приглашения и выдача каждому пользователю личного аккаунта — в `internal_upsert.go` / `invite.go`, а предикат «владеет нулём аккаунтов» подпёрт интеграционными пробами на счёт по владельцу и на гонку при первичной настройке.

**Состояние на момент записи**: done · было при заведении: **in-progress** (code-complete on branch `fix-invite-grant-fga`, awaiting role-reviews + merge). Acceptance `docs/specs/iam-anchor-grant-and-invite-activation-fga-acceptance.md` (✅ APPROVED round 2). Single-repo **kacho-iam** (no proto/gateway/migration). Strict TDD RED→GREEN done.

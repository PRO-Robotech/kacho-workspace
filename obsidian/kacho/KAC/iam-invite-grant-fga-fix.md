...
**Status**: done · было при заведении: **in-progress** (code-complete on branch `fix-invite-grant-fga`, awaiting role-reviews + merge). Acceptance `docs/specs/iam-anchor-grant-and-invite-activation-fga-acceptance.md` (✅ APPROVED round 2). Single-repo **kacho-iam** (no proto/gateway/migration). Strict TDD RED→GREEN done.

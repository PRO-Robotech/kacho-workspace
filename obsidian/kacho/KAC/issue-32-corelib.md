...
tags:
  ...
  - epic
  - kacho-corelib
verified_against: "PRO-Robotech/corelib: запрос волны PR #62 32 → 26 влит коммитом слияния 227ed2b77b4, 2026-09-24T20:36:02Z (`gh pr view 62`); восемнадцать голов задач из его тела и коммит 33e11da6840d — предки 227ed2b77b4 и не предки origin/main 34bc8104a83 (`git merge-base --is-ancestor`), 2026-09-26. Ветка 53 @ 1677303b6c86 — не предок 227ed2b77b4; веток 50, 51, 54, 55, 60, 61 на origin нет (`git ls-remote --heads origin`)"
---

# corelib#32: волна-2 — порты движка под службу доступа

**Состояние на момент записи**: `test` — 2026-09-26. Волна **влита** в ветку эпика `26`
(координата, не живая ссылка) запросом PR #62, коммит слияния `227ed2b77b4`; в `main` **не**
доехала: закроет её посадка эпика #26 в ствол. Трекер волны открыт с меткой `status:test`,
и семь его sub-issue остаются открытыми без работы — см. «Не вошли». Родитель — эпик #26.
...
запросе — #21 (`issue-21` @ `dd8df2c1e167`), не sub-issue этой волны.
...
переведены в другую волну. На 2026-09-26 все семь остаются sub-issue #32 — запрос влит, а
перевода нет.

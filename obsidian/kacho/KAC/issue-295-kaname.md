...
ветку эпика `357` (координата, не живая ссылка); в `main` **не** доехала. Тогдашняя норма
держала задачу волны открытой до посадки эпика в ствол; её отменил каскад закрытия 2026-09-26.
...
- [x] предикат снятия (устойчивость под нагрузкой либо детерминированная координата) — снят
      по закрывающей записи в задаче, 2026-09-26: оба держателя из заказа по приёмке Ф3 ред. 15 в дереве origin/`357`; мной не
      перемерялся;

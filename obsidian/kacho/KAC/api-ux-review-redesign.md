...
>
> **reference.** Записка — **отчёт панели критиков**, а не единица работы: у неё нет исполнителя и нет условия закрытия. Выводы, которые из неё следовали, уехали в план редизайна и в правила. Оставлена как справочный разбор.
...
## Линза «эргономика ошибок» (перезапуск после блокировки классификатором)

**Вердикт:** валидация (`BadRequest.FieldViolations`, 59 точек) и отказ по правам (`ErrorInfo` +
типизированный `PreconditionFailure` + RFC 9470 challenge) — заметно выше отраслевой нормы.
Остальное — голая проза: **885 `status.Error(f)` против 7 `WithDetails`**.

1. **HIGH.** Канон объявляет нормативную таблицу `reason`-токенов как машиночитаемый дискриминатор
   линий — в коде они существуют **только в комментариях**, на провод не доезжают. Regression-требование
   канона («assert reason-token И code») выполнить сейчас невозможно.
2. **HIGH.** Платформа eventually-consistent и требует клиентского retry, но **не помечает**, какой
   отказ временный (`RetryInfo` — 0 вхождений). Знание передано только собственным тестам (бюджеты
   в них разные: 15/25/40/50), не внешним клиентам.
3. **HIGH (security).** Hide-existence: карта байт-идентичности покрывает **8 типов**, остальные
   **13** падают в fallback `<fga_type> not found` (`compute_disk not found`, `registry_registry not
   found`). Настоящий промах бэкенда даёт `"Disk dsk-abc not found"` ⇒ тексты различимы ⇒
   **existence-oracle**, запрещённый security.md п.6, при том что docstring файла объявляет его
   закрытым. Плюс на 404 нет `x-request-id` — добросовестный пользователь не может даже сослаться
   на запрос в поддержке.
4. **MEDIUM.** `"Folder with id %s not found"` в compute — сущности `Folder` в API нет. Помечено как
   осознанное, но **обоснование — паритет с чужим облаком**, что нарушает core-правило #2 ⇒ это долг
   с устаревшим обоснованием, а не объяснимое отличие.
5. **MEDIUM.** Дрейф регистра в текстах: `accountId` (Project) vs `account_id` (Group) — провод
   camelCase, snake-вариант не совпадает ни с чем; 4 формулировки одного класса «неизвестное поле маски».
6. **MEDIUM.** `Operation.metadata` несёт pre-allocated id и при `error` — в proto-комментарии, который
   и есть документация внешнего клиента, предупреждения нет.

## Линза «IAM глазами администратора» (перезапуск)

**Вердикт:** модель мощная и честная (`target` REQUIRED без sentinel, структурные гейты синхронны и
actionable, роль показывает эффективные verb'ы). Плата — три параллельных словаря и три разных имени
одной координаты на соседних RPC.

1. **НАИВЫСШАЯ.** Неизвестный `(module,resource)`-токен в правиле роли **не отвергается, а молча
   пропускается**: `Role.Create` → 200, `AccessBinding.Create` → 200 (гейт сравнивает токен с той же
   опечаткой), реконсайлер эмитит **ноль** tuple'ов, коллега получает 403 навсегда. Признака пустого
   гранта нет ни в роли, ни в биндинге, ни в Operation. Вероятность высокая: каноническое написание
   непоследовательно между модулями (`compute.instance` ед.ч., `storage.volumes` мн.ч.). Фикс — одна
   проверка против уже существующей таблицы `authzmap.objectTypes`.
2. **HIGH.** Одна координата — три имени: `scope_type/scope_id` на Create, `resource_type/resource_id`
   на read-RPC, плюс enum `scope` и `scope_group`. При этом документ прямо говорит, что слово
   «resource» отдано `target`'у ⇒ legacy-имена означают противоположное обещанному.
3. **HIGH.** Отложенная материализация гранта **не наблюдаема из API**: `Operation.done` — durability,
   `status.PENDING` про другое, `Retry-After` на транзиентном 403 нет. Админ сказал «готово», коллега
   видит 403, отличить «едет» от «выдал не то» нельзя. Собственный план это уже требует.
4. **HIGH.** «Что может Ann» **слепо к доступу через группу** (`Derivation.GROUP` зарезервирован, v1 не
   эмитит) — ровно там, где модель советует масштабироваться. Механика раскрытия usersets уже есть.
5. **MEDIUM.** Пять способов прочитать привязки; «канонический» `List` — единственный **без**
   `includeRevoked`, тогда как у legacy-семьи он есть ⇒ ради главного свойства мягкого отзыва
   (retention) надо уйти с рекомендованного пути.
6. **MEDIUM.** Step-up нигде не объявлен заранее: `required_acr_min` живёт только в proto-аннотациях,
   каталог прав его не публикует ⇒ автоматизация под сервис-аккаунтом (они освобождены) зелёная,
   а человек ловит challenge на первом же вызове.
7. **LOW-MEDIUM.** `users:invite` молча ставит `allInScope`, тогда как `AccessBinding.Create`
   намеренно требует явный opt-in — асимметрия least-privilege-двери.

...
category: docs
status: in-progress
verified_against: "kacho-workspace@56935d4d (origin/main, 2026-09-03): координаты и числа сняты `git ls-tree -r origin/main`; предикат каждого стоит рядом с ним. Тексты приёмки и модуля правил построчно не пересматривались"
related_kac:
  - "[[KAC/issue-504-ws]]"
  - "[[KAC/issue-485]]"
  - "[[KAC/issue-486]]"
...
- **пересверки разбора на отпечаток замысла в дереве НЕТ**, и это остаток, а не упущение
  записки: такая запись требует внешнего проверенного события и не может быть написана
  инженером. Предикат: `ls docs/specs/reviews/*class-exposure*/revalidation/ 2>/dev/null`
  → путь не существует;
...
- **напоминание в момент правки** — хук `change-graph-reminder.sh` (`PostToolUse`), восьмой
  из провязанных в `.claude/settings.json`.

## Чего контур НЕ судит

Машина не объявляет, что понимает смысл прозы: она проверяет наличие, полномочие, привязку к
предмету и вердикт **зарегистрированного** держателя. Смысловые конфликты, дублирование
смысла и открытые решения замысла судит человек. Это граница по построению, а не остаток
работ, — и путать её с недоделкой не надо: см. [[lessons/checks-with-form-but-no-substance]].

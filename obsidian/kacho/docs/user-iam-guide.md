...
> [!warning] Это черновик доков эпохи производственного IAM, а не описание сегодняшнего продукта
> Записка заведена под эпик [[KAC/KAC-127]] и описывает возможности, значительной
> части которых в дереве **нет**: подсистемы внешнего провижининга учётных записей,
> моста к внешнему SAML-провайдеру, организаций как уровня иерархии и конвейера
> событий безопасности сняты — их таблицы дропнуты отдельными миграциями
> (`services/iam/internal/migrations/`), контрактов в `proto/kacho/cloud/iam/v1/`
> у них нет. Иерархия сегодня двухуровневая: **аккаунт → проект**.
>
> Страница сохранена как **история намерения** (`status: legacy`): по ней видно,
> какой продукт проектировался, и на неё ссылаются записки журнала. Как инструкцию
> пользователю читать её нельзя, и публичного адреса, обещанного в шапке, не
> существует. Сверено с деревом продукта `b4edc5d5`, 2026-08-05.

...
- **ListBySubject — self-only для ВСЕХ типов субъекта** (KAC-123, ужесточено позже).
  Метод перечисляет привилегии названного субъекта, поэтому разглашение здесь — это
  разглашение чужой картины доступа. Каталог держит RPC `<exempt>`, значит гейт целиком
  на стороне handler'а: аноним отсекается первым (`RequireAuthenticated`), `user` и
  `service_account` обязаны быть **тем самым** субъектом, `group` — состоять в ней,
  неизвестный `subject_type` — отказ. Корзины «все остальные» нет: ветка по умолчанию
  **запрещает**, а не пропускает.
  > [!warning] Здесь была запись про исключение для машинных принципалов
  > Прежняя редакция описывала отдельную полосу «служебная учётка → мимо self-проверки»
  > с обоснованием «admin-tooling». Такой полосы в коде нет, и заводить её заново нельзя:
  > исключение, ключуемое на **тип** принципала, а не на выданное право, — ровно тот класс,
  > который `security.md` запрещает (машина получает доступ **грантом**, а не тем, что она
  > машина). Запись сохранена, чтобы следующий не «восстановил» её как утраченную.

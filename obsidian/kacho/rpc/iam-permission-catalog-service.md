...
> [!warning] Набор глаголов переезжает К РЕСУРСУ — поле пересечения перестаёт быть словарём
> Значения `closedVerbs[]` ниже описывают состояние ДО линии `release/identity-3` и уже не
> сходятся с ней: набор глаголов — атрибут **типа**, а пересечение сужается по мере того, как
> наборы типов расходятся (это его объявленное поведение, а не поломка). Источником
> выпадающего списка редактора ролей становится собственный набор ресурса
> (`CatalogResource.verbs`).
>
> На 2026-08-24 правка лежит в открытом PR #1196 и в накопительную ветку линии **не влита** —
> поэтому запись оставлена как есть, а не переписана: описывать намерение как факт нельзя.
> Предмет, числа и предикат — [[KAC/issue-1128]]; продолжение по глаголу удаления —
> [[KAC/issue-1189]].

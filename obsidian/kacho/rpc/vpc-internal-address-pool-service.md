...
**Public/Internal**: cluster-internal — **но ресурс больше не internal-only**.

> [!important] У этого сервиса с 2026-08-17 есть ПУБЛИЧНЫЙ близнец (ADM-1 S1, kacho#522)
> `AddressPoolService` (`proto/kacho/cloud/vpc/v1/address_pool_service.proto`, `kacho-vpc:9090`)
> несёт те же одиннадцать глаголов на ТЕХ ЖЕ канонических путях, под правом
> `system_admin` @ `cluster`. Отличий ровно три: мутации отвечают `Operation`
> (запрет 9), форма идентификатора проверяется первым оператором, а промах прямого
> чтения отвечает контрактным тоном с машинным признаком полосы.
>
> **Оба сервиса пишут ОДНУ таблицу через ОДНИ И ТЕ ЖЕ use-case'ы** — composition root
> передаёт в оба транспорта одни указатели, поэтому разойтись валидацией или
> умолчаниями им нечем. Снаружи виден публичный (обе ветки диспетчера края на внешнем
> слушателе ведут в публичный мультиплексор), изнутри — этот.
>
> Окно расширения закрывает стадия S3: она снимает ЭТОТ сервис. До тех пор записка
> описывает живой предмет; после — подлежит снятию вместе с ним.

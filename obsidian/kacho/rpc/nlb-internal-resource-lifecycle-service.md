...
> [!important] ПОДПИСКА У nlb СНОВА ЕСТЬ — но не эта, а ОБЩАЯ (2026-08-28, [[issue-1019]])
> Сервис служит `InternalSubscriptionService.Subscribe` — форму, объявленную ОДИН раз на
> всю платформу и реализованную в фундаменте (`pkg/subscription`). nlb приносит ей только
> объявление своего журнала; ни своего сервера, ни своего контракта у него больше нет и не
> заводится.
>
> Это НЕ возвращение снятого, и различие несущее: снималась вторая форма об одном
> предмете, а не подписка как таковая. Срок жизни потока у носителя снова объявлен
> ВЕЛИЧИНОЙ — изъятие «серверных стримов у сервиса нет» стало бы ложью о дереве в тот же
> момент, когда глагол зарегистрирован.
>
> **Цена названа:** события nlb доезжают БЕЗ состояния предмета — нагрузка его журнала
> намеренно минимальна, а контракт разрешает читать непустую нагрузку как ПОЛНОЕ
> состояние. Подписчик узнаёт, ЧТО перечитать, и читает предмет отдельным вызовом.
> Обогащение журнала заведено своим предметом.

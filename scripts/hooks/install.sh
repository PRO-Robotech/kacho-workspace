...
# Зовётся целями `scripts/hooks/install.sh install` / `scripts/hooks/install.sh check`; руками — тоже можно.
#
...
#   · цена: новый отслеживаемый хук требует повторного `scripts/hooks/install.sh install`.
#     Цена наблюдаемая, а не молчаливая, — её называет `scripts/hooks/install.sh check`.
#
...
if [ -n "$configured" ]; then
    ...
    if [ ! -d "$abs" ]; then
        die "ОТКАЗ: core.hooksPath = «$configured» — каталога по этому пути НЕТ." \
            ...
            "" \
            "  git config --unset core.hooksPath   # затем: scripts/hooks/install.sh install"
    fi
    ...
    if [ "${#blinded[@]}" -gt 0 ]; then
        ...
        echo "  git config --unset core.hooksPath && scripts/hooks/install.sh install" >&2
    fi
...
stub_for() {
    cat <<STUB
#!/usr/bin/env bash
# СГЕНЕРИРОВАН \`scripts/hooks/install.sh install\` — правится НЕ здесь, а в scripts/hooks/$1.
# $marker
set -uo pipefail
...
check)
    ...
    if [ "${#missing[@]}" -gt 0 ]; then
        ...
        echo "  scripts/hooks/install.sh install" >&2
        exit 1
    fi
    exit 0
    ;;
notice)
    ...
    # утверждение печатает `scripts/hooks/install.sh check`, а не каждый прогон тестов.
    if [ "${#missing[@]}" -gt 0 ] || [ "${#foreign[@]}" -gt 0 ]; then
        echo "ВНИМАНИЕ: хуки git не провязаны (${#missing[@]} не провязано, ${#foreign[@]} занято чужим)." >&2
        echo "  Отправка ветки НЕ будет проверена локально — «scripts/hooks/install.sh install» это чинит." >&2
    fi
    exit 0
    ;;
install) ;;
...
echo "проверить в любой момент: scripts/hooks/install.sh check"

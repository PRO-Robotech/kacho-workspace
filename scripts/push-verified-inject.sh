...
echo
echo "[CENSUS] push-verified-inject: проб исполнено $probes, провалов $failed; фикстур собрано 7, ложный ноль воспроизведён тремя путями (ноль команды, отказ транспорта, код съеден посредником)"
if [ "$probes" -eq 0 ]; then

...
#
# Утверждения (их число печатает последняя строка прогона, счётом вызовов say):
#   A. влитая локальная ветка → хук говорит и называет её репозиторий;
#   B. чистое дерево → stdout ПУСТ (не «почти пуст», а пуст);
#   C. ветка, занятая рабочей копией, в счёт не идёт — звать о ней бессмысленно,
#      снять её нельзя;
...
#   G. ветка без своего коммита (голова на первой линии ствола), чья ссылка
#      двигалась в окне свежести, в счёт не идёт: влито нечего, перепись её не
#      снимет (ws#847, круг 2); законный близнец — `landed` со своим коммитом —
#      в счёте (C);
#   H. та же ветка, но только локально и вне окна (журнал ссылки в прошлом), —
#      в счёте: исключение истекает (ws#847, круг 3);
#   I. законный близнец H: та же ветка есть на origin — молчание.
#
...
git checkout -q main
# Занятая ветка — со СВОИМ влитым коммитом: иначе её исключала бы из счёта
# первая линия ствола (G), и C не доказывал бы исключение по занятости.
git checkout -qb occupied main
echo занятое > occupied.txt && git add occupied.txt && git commit -qm "работа занятой ветки"
git checkout -q main && git merge -q --no-ff -m "влить occupied" occupied && git push -q origin main
...
# --- G: ветка без своего коммита — хук молчит ----------------------------------
# Отдельный репозиторий, где кроме неё веток нет: молчание здесь отличимо от
# молчания в B только её присутствием. Ветка отведена от ствола, а ствол ушёл
# вперёд — голова на первой линии, но не на вершине, как у волны, чей эпик
# принял соседнюю волну.
mk_repo "$TMP/g"
git -C "$TMP/g/ws" branch wave-no-own-commit main
echo дальше > "$TMP/g/ws/next.txt"
git -C "$TMP/g/ws" add next.txt && git -C "$TMP/g/ws" commit -qm "ствол ушёл вперёд"
git -C "$TMP/g/ws" push -q origin main
OUT_G=$(CLAUDE_PROJECT_DIR="$TMP/g/ws" bash "$HOOK" 2>&1)
if [ -z "$OUT_G" ]; then
  say "✅ G" "ветка без своего коммита (голова на первой линии ствола) влитой не названа"
else
  say "❌ G" "ветка без своего коммита посчитана влитой: $OUT_G"; fail=1
fi

# --- H/I: исключение без своего коммита ИСТЕКАЕТ (ws#847, круг 3) ---------------
# Силуэт G, но ссылка не двигалась давно: журнал ссылки датирован прошлым
# (GIT_COMMITTER_DATE датирует и запись журнала). H — только локально → хук
# ОБЯЗАН её посчитать: иначе ветку, которой нет на origin и которая никому не
# нужна, исключение держало бы вечно. I — законный близнец: та же ветка ЕСТЬ на
# origin (живая волна) → молчание. H и G разнятся одним журналом ссылки, H и I —
# одним присутствием на origin.
mk_repo "$TMP/h"
GIT_COMMITTER_DATE='2026-09-01T00:00:00Z' git -C "$TMP/h/ws" branch stale-no-own-commit main
echo дальше > "$TMP/h/ws/next.txt"
git -C "$TMP/h/ws" add next.txt && git -C "$TMP/h/ws" commit -qm "ствол ушёл вперёд"
git -C "$TMP/h/ws" push -q origin main
OUT_H=$(CLAUDE_PROJECT_DIR="$TMP/h/ws" bash "$HOOK" 2>&1)
if grep -qE 'не менее 1 локальных веток' <<<"$OUT_H"; then
  say "✅ H" "ветка без своего коммита, только локально и вне окна, посчитана — исключение истекло"
else
  say "❌ H" "ветка без своего коммита, только локально и вне окна, не посчитана: ${OUT_H:-<пусто>}"; fail=1
fi
git -C "$TMP/h/ws" push -q origin stale-no-own-commit
OUT_I=$(CLAUDE_PROJECT_DIR="$TMP/h/ws" bash "$HOOK" 2>&1)
if [ -z "$OUT_I" ]; then
  say "✅ I" "та же ветка на origin (живая волна) в счёт не идёт — законный близнец H"
else
  say "❌ I" "ветка без своего коммита на origin посчитана влитой: $OUT_I"; fail=1
fi
...
echo
if [ "$fail" -eq 0 ]; then
  echo "branches-clean-inject: утверждений ${N_SAID}, все выполнены — хук способен заговорить И смолчать"
else
  echo "branches-clean-inject: утверждений ${N_SAID}, есть невыполненные" >&2
fi
exit "$fail"

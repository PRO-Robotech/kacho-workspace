...
while IFS= read -r line; do
    case "$line" in
        # Заголовок раздела ЗАКРЫВАЕТ последнюю запись предыдущего раздела. Без
        # этого буфер последней записи каждого раздела дотягивался бы до первого
        # `### N.N` следующего, захватывая его преамбулу: запись, у которой часть
        # снята, зеленела бы от строки, лежащей ЗА её границей.
        '## §'[0-9]*)

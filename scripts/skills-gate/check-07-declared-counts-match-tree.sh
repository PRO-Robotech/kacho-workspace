...
#
# ПРИВЯЗКИ ПРАВИЛ В СЧЁТ СКИЛОВ НЕ ИДУТ, И ЗДЕСЬ ЭТО ГОВОРИТ НЕ ГЕЙТ, А ЦИТАТА.
# Каталог `.claude/skills/rule-<имя>/`, чей `SKILL.md` — символьная ссылка в
# `.claude/rules/`, есть адрес правила для предзагрузки, а не экспертиза
# (`_lib.sh` §ПРИВЯЗКА ПРАВИЛА), и предикат скилов в `ai-tooling.md` их
# отбрасывает. Гейт исполняет ЦИТАТУ и не подменяет её своим предикатом: подмена
# означала бы, что он сверяет объявленное число с собственным представлением о
# дереве, а не с тем, что объявлено исполнять, — то есть проверяет себя. Что
# отброшенное отброшено ПО СВОЙСТВУ, а не только по имени, держит `check-03`: у
# него ось на каталог `rule-*`, чей `SKILL.md` не ссылка.
#

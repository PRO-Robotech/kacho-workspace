...
#
# У НОРМЫ ОДИН ДОМ — `.claude/rules/`. Домов было два (ядро `.claude/rules/` плюс
# свод `.claude/rulebook/`); второй снят решением владельца 2026-09-13, и перечень
# поиска сузился до одного каталога.
#
# ПРИВЯЗКИ ПРАВИЛ ИЗ ОБХОДА ИСКЛЮЧЕНЫ — ЯВНО. Каталог `.claude/skills/rule-<имя>/`,
# чей `SKILL.md` — символьная ссылка в `.claude/rules/`, не скил, а адрес правила
# (`_lib.sh` §ПРИВЯЗКА ПРАВИЛА). Обход по нему читал бы ТЕКСТ НОРМЫ и судил его
# меркой скила: правило ссылается на соседнее правило своей формой записи
# («§«Заголовок»» в кавычках-ёлочках), и на этом дереве так набралось 52 находки,
# ни одна из которых не была о скилах. Предикат исключения — «SKILL.md является
# символьной ссылкой», не имя каталога: имя подделывается `mkdir`, свойство файла
# — нет. Ссылки на нормы внутри самих правил — предмет `rules-gate`, не этого
# набора; здесь они не проверяются ВТОРОЙ раз, а не остаются непроверенными.
#
# ПЕРЕПИСЬ ПЕЧАТАЕТ ТРИ ВЕЛИЧИНЫ, И ЭТО ПЕРЕЖИЛО ОБА ПЕРЕЕЗДА. Когда дом был один
# в первый раз, отсутствие файла молча пропускалось через `continue`, а перепись
...
# сколько ссылок найдено и сколько из них ПРОВЕРЕНО, — а ссылки на документ вне
# корпуса норм (сервисный `CLAUDE.md`) называются третьим числом, а не
# растворяются в первом. Возврат к одному каталогу этой слепоты не возвращает:
# держит её разделение величин, а не число домов.
#
...
# И не должно быть решётки: обход склеивает строки пробелом, поэтому упоминание
# файла в прозе, за которым идёт СВОЙ заголовок вида «## §1. …», выглядело адресом
# в чужой файл. Замер: одно ложное срабатывание (`change-graph` — упоминание
# `git-issues.md` в прозе плюс собственный «## §1.» тремя строками ниже); граница
# раздела не бывает серединой адреса.
NAMED='`[A-Za-z0-9_-]+[.]md`[^§`#]{0,40}§'

mapfile -t files < <(skills_gate_expertise_files "$WS")
mapfile -t bindings < <(skills_gate_binding_files "$WS")

if [ ${#files[@]} -eq 0 ]; then
    skills_gate_void "$NAME" \
        "в индексе git нет ни одного .claude/skills/*/SKILL.md, который не был бы привязкой правила (привязок ${#bindings[@]})"
    exit 2
fi
...
# Путь к норме по её имени. Пусто — документ вне корпуса норм (сервисный
# CLAUDE.md), и это ТРЕТИЙ исход, а не «проверено».
rule_path() {
    local rf="$1"
    if [ -f "$WS/.claude/rules/$rf" ]; then printf '%s' "$WS/.claude/rules/$rf"; fi
}
...
echo "перепись: прочитано SKILL.md — ${#files[@]}, привязок правил вне счёта — ${#bindings[@]}; ссылок по имени раздела — $named_refs, из них ПРОВЕРЕНО $checked_refs (норма найдена в .claude/rules/), вне корпуса норм $offcorpus_refs; ссылок номером строки — $numbered_refs"
...
skills_gate_pass "$NAME" "осмотрено ${#files[@]} файлов (привязок правил вне счёта ${#bindings[@]}), $named_refs ссылок по имени раздела, из них проверено $checked_refs — все резолвятся; вне корпуса норм $offcorpus_refs; номерных 0"
exit 0

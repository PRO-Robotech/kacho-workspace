...
# M, N, O, P, Q, R, S, T, U, V, W, X, Y, Y2, Z, R2, AA, AB, AC, AD, AE, EW1–EW4 — полный
...
# Четвёртая колонка — база PR: так ветка эпика становится стволом (ws#821).
printf '901\t105\tMERGED\t900\n' >> "$PRFILE"
...
if awk '/^── ВЛИТЫ/,/^── НА ORIGIN/' <<<"$OUT" | grep -q '^   901 .*относительно origin/900'; then
...
if awk '/ТОЛЬКО ЛОКАЛЬНО/,/^── ЖИВЫЕ/' <<<"$OUT" | grep -q '^   902 '; then
...
if awk '/ТОЛЬКО ЛОКАЛЬНО/,/^── ЖИВЫЕ/' <<<"$OUT" | grep -q '^   903 ' &&
   ! grep -E '^branch-audit: стволов в сверке' <<<"$OUT" | grep -q 'origin/903'; then
...
if grep -E '^branch-audit: стволов в сверке' <<<"$OUT" | grep -q 'origin/900 (эпик/волна'; then

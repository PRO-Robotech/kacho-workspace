...
#              ожиданием. Поглощено, если найдены ВСЕ добавленные и не уцелела
#              НИ ОДНА снятая. Обе половины обязательны: без второй ветка, чья
#              работа — снятие, была бы объявлена поглощённой молча.
#
...
#
# ОТСЮДА — ДВА РАЗНЫХ ВОПРОСА И ДВА РАЗНЫХ ПРЕДИКАТА. До 2026-08-19 на оба
# отвечал один: «байты не совпали». Он годится для вопроса «безопасно ли
...
ba_norm() { sed 's/^[[:space:]]*//;s/[[:space:]]*$//' | grep -v '^$' | LC_ALL=C sort -u; }
...
census_file() { # $1=ref $2=base $3=путь → 0, если содержимое найдено в стволе
  ...
  for tr in "${TRUNKS[@]}"; do
    ...
    if [ "$miss" -eq 0 ] && [ "$stay" -eq 0 ]; then
      CENSUS_MISS=0; CENSUS_STAY=0; CENSUS_TARGET="$target"; return 0
    fi
...
classify_files() { # $1 = ref → заполняет UNABS_PROVEN и UNABS_UNDET
  ...
  while read -r f; do
    ...
    if census_file "$ref" "$base" "$f"; then
      census_rescued=$((census_rescued + 1))
      continue
    fi
...
census_asked=0; census_rescued=0
...
while read -r b; do
  ...
  if [ "$ancestor" = 1 ]; then
    ...
    if [ -n "$src" ]; then
      ...
      # а там, где и он молчит, — перепись (6в) и признак касания пути.
      sixth_checked=$((sixth_checked + 1))
      classify_files "$b"
      if [ "${#UNABS_PROVEN[@]}" -eq 0 ] && [ "${#UNABS_UNDET[@]}" -eq 0 ]; then
        how="ПОГЛОЩЕНА ПОФАЙЛОВО (шестой признак и перепись; пятый дал конфликт)"
...
if [ "${#undet_work[@]}" -ne 0 ]; then
  emit "ПОГЛОЩЕНИЕ НЕ УСТАНОВЛЕНО — ствол правил те же файлы; ветка может быть ранним черновиком" \
       "${undet_work[@]}"
fi
...
echo "branch-audit: осмотрено локальных ${examined_local}, на origin ${examined_remote}," \
     ...
     "перепись строк спрошена по ${census_asked} файл(ам) и сняла ${census_rescued};" \

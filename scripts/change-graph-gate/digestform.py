...
переписи. Дата вершины — дата коммиттера, поставленная тем, кто коммитил;
подделку даты гейт не ловит.
...
перечня не полным sha), пустой обход или слепой распознаватель; 2 — ревизия,
граница, коммит правила или строка перечня вершин в клоне не разрешаются.
"""
...
def main(argv=None):
    ...
    say("CENSUS", "записей %d · команд дайджеста %d: с --full-index %d, "
        ...
        "не сошлись %d, унаследовано с них %d, строк не полным sha %d" % (
    ...
    return 1 if findings or blind or knows or idle or malformed else 0

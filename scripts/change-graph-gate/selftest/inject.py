...
INJECTIONS = [
    ...
    (
        ...
        ["D1 мира по пути нет -> собственный отказ, stdout пуст",
         ...
         # Утверждение секции F' той же природы: оно тоже требует СОБСТВЕННОГО
         # отказа, поэтому подмена отказа вердиктом роняет и его. Перечень
         # ожидаемого обязан быть полным — инъекция, покрасневшая шире
         # объявленного, не доказывает, что покраснело именно проверяемое.
    ...
    (
        ...
        # То же свойство, спрошенное с другой стороны: отзыв, запрошенный не
        # владельцем, неприменим ни одному правилу семейства, и без этой ветви
        # он получил бы GREEN — то есть неавторизованный отзыв был бы объявлен
        # законным. Перечень ожидаемого обязан называть оба утверждения.
        ["D7 ни одно правило не применимо -> собственный отказ, а НЕ vacuous GREEN",
         "F-WITHDRAW-1 отзыв не от владельца НЕ судится и не объявляется "
         "зелёным"],
    ),

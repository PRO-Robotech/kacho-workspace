...
INJECTIONS = [
    ...
    (
        ...
        ["D1 мира по пути нет -> собственный отказ, stdout пуст",
         ...
         # Обе полосы, опирающиеся на зарегистрированный эталон, спрашивают то
         # же различение на СВОЁМ входе: эталона нет -> вердикта нет. Инъекция
         # ломает различение целиком, поэтому краснеют и они — и это
         # доказывает, что утверждения не вакуумны.

...
INJECTIONS = [
    ...
    (
        # Третья ветвь ядра, до этой инъекции не проверенная НИЧЕМ: её снятие
        # роняло ноль утверждений (перемерено), то есть отказ мог быть мёртв, а
        # заметить это было неоткуда — собственный отказ печатает на stdout ноль
        # строк, поэтому его отсутствие неотличимо от его молчания (#486).
        #

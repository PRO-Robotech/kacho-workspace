...
EDIT_LATE_LOOP_ANNOUNCE = [
    ...
    # Якорь — ОДНА строка прогона, а не пара «прогон + замер времени». Пара
    # перестала быть смежной, когда между ними встало раскрытие маркеров, и
    # подстановка молча перестала находиться: инъекция сообщала «образец не
    # найден», то есть утверждение P3 осталось без доказательства падучести.
    # Чем короче якорь, тем меньше у него поводов пережить свой предмет; здесь
    # он к тому же единственный в файле (отступ 12 — только тело цикла инъекций,
    # у контрольного прогона другое имя переменной).
    (

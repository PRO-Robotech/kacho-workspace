...
def sut_path():
    ...
    (selfcheck) и служит обеим сторонам различения выше: им предъявляют и
    присутствующий SUT (`selfcheck/fake_sut.py`), и ОТСУТСТВУЮЩИЙ — путь,
    которого нет на диске. Второе — не уловка, а единственный способ построить
    состояние ABSENT после того, как production SUT написан: искать его среди
    кейсов бессмысленно, необъявленных семейств в дереве ноль (#485).

    Матрица из 196 кейсов переменную не выставляет никогда, и это отдельно
    проверяется пробой harness'а.
    """

...
# не наблюдение за испытуемым; у остальных 182 — наблюдение.
#
# **Что это не толкование, а единственное чтение, — измерено дважды** (первый
# замер 2026-09-02, ПЕРЕМЕРЕНО 2026-09-18 после снятия семейства `ADAPTER`;
# оба предиката несут контроль в обратную сторону):
#
...
#      185 — ровно эти (ждут `CG_TRACE_ID_ORPHAN` / `CG_TRACE_ID_MISSING`,
#      предмет `cg.evid` и `cg.trace`); у остальных 182 диагностика
#      производима. Наблюдением такая колонка быть не может ни при каком мире.
#      Знаменатель — число РЯДОВ ПРИЁМКИ, а не фикстур на диске; он расходился
#      с числом фикстур, пока двум рядам (`SDD-1-AUTH-09`, `SDD-1-DRIVER-04`)
#      фикстур не хватало, и это расхождение читалось как утверждение о
#      приёмке. Сегодня обе величины дают 185. Знаменатель здесь ВЫПИСАН, а не
#      выведен, и цена названа: он стареет молча. Держит его предикат ниже —
#      гони его, а не читай число. Копия ЭТОЙ ПАРЫ («три из N, у остальных M»)
#      жила в `cglib/families/trace.py` и снята тем же изменением: два места об
#      одном предмете уже разошлись — там остался прежний знаменатель, когда
#      здешний съехал вместе с перечнем кейсов приёмки;
#   2. код возврата выводится из категории (`cglib/outcome.py`), и приёмка
#      держит это соответствие в **932 тройках из 936**. Четыре исключения —
#      `GREEN · … · exit 10` и `RED · … · exit 0` — стоят только в рядах
#      DRIVER-01 и DRIVER-03 (по два вхождения, §13 и §14).
#
#      ЕДИНИЦА СЧЁТА НАЗВАНА, потому что редакция @`b48cda87` называла «987 из
#      991», и эта пара НЕ воспроизводится предикатом ниже: на ревизии `b3f75bfc`,
#      где она ещё стояла, предикат даёт 997 из 1001. Единицы различались — какая,
#      установить нечем: у прежнего числа предиката не было вовсе. Здесь единица
#      такая: ВХОЖДЕНИЕ тройки `КАТЕГОРИЯ · ДИАГНОСТИКА · exit N` в текст
#      приёмки, где угодно — в ряду, в таблице, в прозе.
#
...
#
# Предикат ВТОРОГО замера — он же и есть объявленная единица счёта:
#
#     python3 - <<'PY'
#     import io, re, sys
#     sys.path.insert(0, "scripts/change-graph-gate")
#     from cglib import outcome
#     from caselib import spec
#     text = io.open(spec.acceptance_path(), encoding="utf-8").read()
#     triple = re.compile(
#         r"\b(GREEN|RED|NOT_EXECUTED)\s*·\s*([A-Z0-9_]+)\s*·\s*exit\s+(\d+)")
#     hits = triple.findall(text)
#     off = [h for h in hits if outcome.SUBJECT_EXIT_CODES[h[0]] != int(h[2])]
#     print("троек", len(hits), "· сходится", len(hits) - len(off),
#           "· расходится", len(off), off)
#     PY
#
# Расхождений больше четырёх — приёмка завела пятое исключение и его надо
# объяснить; меньше — исключение пережило свой предмет, и стаб подлежит сужению.
#

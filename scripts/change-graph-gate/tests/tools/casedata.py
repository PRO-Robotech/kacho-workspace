# -*- coding: utf-8 -*-
"""Авторская часть fixtures: базовые миры и one-fact дельты.
...
DERIVED = {
    ...
    # Verdict артефакта заводится ИМЕННО здесь, а не в базовом мире AUTH-01, и
    # это выбор, а не удобство. Испытуемый отказывается судить мир, в котором
    # есть факт, не прочитанный ни одним применимым правилом
    # (`CG_SELF_WORLD_FACT_UNREAD`, exit 40 — вердикт НЕ выносится). Правила о
    # вердикте в семействе cg.auth сегодня нет, поэтому лист в базовом мире
    # погасил бы ВОСЕМЬ соседних кейсов, включая положительный twin: замер —
    # `run_matrix.py final` даёт 9 поломок harness против одной. Один
    # непрочитанный факт остаётся ровно у того кейса, чьего правила нет.
    "SDD-1-AUTH-09": (
        "add", "artifact.verdict", "CHANGES_REQUESTED",
        "artifact объявил verdict, расходящийся с сохранённым verdict события",
    ),
    # --- Truth ownership ---------------------------------------------------
    "SDD-1-TRUTH-02": (
    ...
    # Предмет DRIVER-04 — не компаратор driver'а, а сам испытуемый: мир ему
    # передаётся, тройка наблюдается (§14, абзац о трёх birth fixtures).
    # Поэтому twin здесь TRACE-01, а не EVID-02, и sut_stub кейсу не положен.
    # В тройке меняется ТОЛЬКО код возврата: категория RED и диагностика
    # остаются прежними, и единственное, что перестало быть верным, — «код
    # возврата следует из категории».
    "SDD-1-DRIVER-04": (
        "change", "driver_birth.actual_triple", "RED · CG_TRACE_ID_ORPHAN · exit 0",
        "код возврата записанной birth triple перестал следовать из её категории",
    ),

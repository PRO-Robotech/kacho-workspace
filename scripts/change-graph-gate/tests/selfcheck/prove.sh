...
# и не будет. Замер на этом дереве (перемерен 2026-09-18 после снятия семейства
# `ADAPTER` вместе с его предметом) — семейств в testdata 32, объявлено 32,
...
#
# Источников состава ДВА, и оба законны — каждый в своём контексте:
#   * РАБОЧАЯ КОПИЯ — индекс ревизии. Перечисление диска несёт неотслеживаемое и
#     молча меняет объём осмотренного между машинами; причина прежняя и не
#     отменяется.
#   * ПЕСОЧНИЦА inject.sh — диск. Индекса там нет by construction: `cp -r`
#     копирует дерево, а не репозиторий. И перечисление диска там безопасно by
#     construction же: всё, что в песочнице лежит, положил туда сам harness
#     намеренно — неотслеживаемого там нет.
# Слепота ко второму контексту делала пробу беспредметной на КАЖДОЙ инъекции,
# включая контрольную, то есть страж переставал падать на своём предмете,
# оставаясь на вид рабочим.
#
# Поэтому источник не угадывается по контексту, а УСТАНАВЛИВАЕТСЯ ответом git, и
# перепись его НАЗЫВАЕТ: без имени источника «ноль прочитанного» снова стало бы
# неотличимо от «ноль находок» — теперь уже в двух контекстах вместо одного.
# Форма счёта у обоих источников ОДНА (второй сегмент пути, `sort -u`), поэтому
# они не могут разойтись в трактовке «что считается fixture».
fixture_composition() {
  awk -F/ 'NF>1 {print $2}' | sort -u | wc -l
}
FIXTURE_SOURCE=""
FIXTURE_DIRS=0
if [ ! -d "$TESTS_DIR/testdata" ]; then
  : # источника нет вовсе — ни индексу, ни диску нечего прочитать
elif FIXTURE_INDEX="$(git -C "$TESTS_DIR" ls-files testdata 2>/dev/null)" \
     && [ -n "$FIXTURE_INDEX" ]; then
  FIXTURE_SOURCE="индекс"
  FIXTURE_DIRS="$(printf '%s\n' "$FIXTURE_INDEX" | fixture_composition)"
else
  # Индекс о ЭТИХ файлах не знает (не репозиторий либо дерево не отслеживается) —
  # значит состава он дать не может, и диск остаётся единственным правдивым
  # источником. Имя источника уезжает в перепись, а не замалчивается.
  FIXTURE_SOURCE="диск"
  FIXTURE_DIRS="$( ( cd "$TESTS_DIR" && find testdata -type f ) | fixture_composition)"
fi
# «Источника нет вовсе» и «источник прочитан, fixtures ноль» — РАЗНЫЕ исходы:
# первое означает, что пробу не провести, второе — что она беспредметна. Свести
# их в один вердикт значило бы объявить непрочитанное осмотренным.
if [ -z "$FIXTURE_SOURCE" ]; then
  FAILED=$((FAILED + 1))
  echo "  FAIL H2 каталога fixtures нет вовсе — источника состава не существует, пробу не провести"
elif [ "$FIXTURE_DIRS" = "0" ]; then
  FAILED=$((FAILED + 1))
  echo "  FAIL H2 fixtures не найдены вовсе (источник: $FIXTURE_SOURCE) — проба беспредметна"
elif [ "$FAKES" = "0" ]; then
  PASSED=$((PASSED + 1))
  echo "  OK   H2 ни одна из $FIXTURE_DIRS fixtures (источник: $FIXTURE_SOURCE) не ссылается на подделку"
else
  FAILED=$((FAILED + 1))
  echo "  FAIL H2 подделка упомянута в $FAKES fixtures из $FIXTURE_DIRS (источник: $FIXTURE_SOURCE)"
fi

...
def audit_canonical_inputs():
    ...
    construction, и весь набор остаётся зелёным. Так и вышло 2026-09-08: дом
    нормы раздвоился, рядом с `.claude/rules/` завёлся `.claude/rulebook/`,
    манифест новый вход объявил, а набор продолжал судить его НЕканоническим.
    Ни одна проверка обеих сторон не покраснела. Раздвоение отозвано 2026-09-13
    — второго дома в дереве нет, — но проверка предметом этого случая не
    ограничена: она судит РАВЕНСТВО двух перечней, каким бы ни был их состав.

...
#
# СХОДИМОСТЬ С ДЕРЕВОМ БОЛЬШЕ НЕ ПРОВЕРЯЕТСЯ, И ЭТО НЕ УПУЩЕНИЕ (2026-09-18).
# Её держал `check-04` этого набора, сверявший перечень ниже с манифестом
# владения в дереве. Проекция оснастки для чужих агентских сред снята решением
# владельца целиком — манифеста нет, — и проверка снята ВМЕСТЕ со своим
# предметом: оставленная, она отвечала бы «сверять не с чем» вечно, то есть
# стала бы послаблением, которое не истечёт никогда.
#
# Перечень ниже при этом остаётся и менять его НЕ НАДО: он — МОДЕЛЬ ЗАКОННОГО
# МИРА кейсов приёмки SDD-1, а не утверждение о дереве. Семейство судит мир
# фикстуры by construction, и тринадцать кейсов `SDD-1-ADAPTER-*` объявлены
# APPROVED-приёмкой; их правка идёт своим кругом с записью ревью, а не попутно.

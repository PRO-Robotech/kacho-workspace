...
предикат чтением дерева нельзя: полный набор кейсов приёмки содержит ВСЕ её
идентификаторы, а пакет одного вызывающего не покрывает и не обязан покрывать их
все. Числа здесь нет намеренно: довод от него не зависит, а копия величины
устарела бы молча — владелец знаменателя один, `tests/caselib/fixture.py`.

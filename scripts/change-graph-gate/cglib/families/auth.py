...
бессмысленными, к тому, что они сравнивают:**
    ...
    8. вердикт совпал              событие о другом исходе

Последние три — сравнения ОДНОГО рода, и порядок между ними не выбран, а взят у
§4 дословно: «event body/subject digests и verdict совпадают». Друг друга они не
обессмысливают, поэтому объявленный порядок здесь ничего не решает, кроме того,
какая диагностика станет вердиктом на мире, расходящемся сразу по нескольким, —
и потому он обязан быть виден, а не получаться из порядка написания.
...
**Третье сравнение §4 — `auth.verdict-mirrors-event`, и оно заведено КРУГОМ
РЕЦЕНЗИИ, а не решением реализации** (задача воркспейса #492). §4 объявляет пару
«артефакт ↔ событие» сходящейся по ТРЁМ величинам: «event body/subject digests И
VERDICT совпадают». Две сравнивались с самого начала (`auth.body-digest`,
`auth.subject-digest`), третья — нет, и пропуск себя не выдавал: вердикт стоял в
`EVENT_CITATION_COORDINATES`, где он проверяется на непустоту У СОБЫТИЯ, а со
стороны это читается как «вердикт учтён».

Написать правило раньше было НЕЧЕМ, и это не отговорка: диагностики расхождения
вердикта приёмка не объявляла, а §13 описывал артефакт без вердикта — «...
body/subject digests и timestamp». Завести свою диагностику значило бы написать
контракт реализацией. Круг рецензии волны 2 разрешил расхождение исходом «§13
догоняет §4»: Given `SDD-1-AUTH-01` теперь называет вердикт среди содержимого
артефакта, приёмка объявила кейс `SDD-1-AUTH-09` и диагностику
`CG_REVIEW_VERDICT_MISMATCH` — и правило написано ТЕМ ЖЕ изменением, ровно как
предписывал прежний предикат снятия, стоявший здесь. **Обе половины приёмки
сегодня говорят одно**, и предикат тому — `grep -n 'body/subject digests'` по
приёмке: §4 и Given положительного кейса вердикт называют оба.

**Вердикт остаётся и в `EVENT_CITATION_COORDINATES` — это НЕ два места об одном
предмете, и различие проверяемо.** Два правила судят разные утверждения о разных
сторонах пары:

    auth.event-citable          СОБЫТИЕ назвало свой вердикт
                                (непустота координаты у события)
    auth.verdict-mirrors-event  АРТЕФАКТ отражает вердикт события
                                (равенство пары)

Ни одно не выводится из другого: событие с пустым вердиктом непроцитируемо
независимо от того, что о нём говорит артефакт, а артефакт, приписавший событию
чужой исход, расходится с ним при совершенно годной цитате. Снять вердикт из
координат цитирования значило бы заплатить за третье сравнение отказом от
проверки, которая ему предшествует.

**Остаток переехал из приёмки в ФИКСТУРЫ, и это надо назвать числом, а не
умолчанием.** Приёмка требует вердикта от артефакта КАЖДОГО post-cutover мира, а
координату `artifact.verdict` объявляет ОДИН мир из девяти — тот, ради которого
она и заведена. На остальных восьми артефакт вердикта не несёт вовсе, хотя их
собственный Given его называет. Правило применимо ко ВСЕМ девяти и на восьми
МОЛЧИТ: вердикт, которого артефакт не заявляет, сравнивать не с чем, а
диагностики «артефакт без вердикта» приёмка не объявляет — завести свою значило
бы написать контракт реализацией (тот же довод, каким выше отвергнуто правило
«артефакт без роли»). Расхождение фикстур с их Given — предмет полосы фикстур, а
не этого модуля; правило от него не зависит и его не маскирует.

Молчание записано в ПРЕДИКАТЕ, а не в применимости, и это решение: применимость,
зависящая от присутствия судимой координаты, сняла бы правило молча — перепись
показывала бы «применимо 7», и восемь миров остались бы без третьего сравнения
так, что по выводу этого не увидеть. Сегодня перепись говорит «применимо 8» на
всех девяти, и молчание отличимо от отсутствия.

**Предикат остатка — КОМАНДОЙ, чтобы его не проверял каждый по-своему:**
...
Числа сравнялись — фикстуры догнали свой Given, и этот абзац снимается вместе с
остатком.
...
#
# `verdict` здесь — утверждение о СОБЫТИИ («оно назвало свой исход»), а не
# сверка его с артефактом: сверку ведёт `auth.verdict-mirrors-event`, и одно
# другого не заменяет. Разбор, почему это не два места об одном предмете, — в
# шапке модуля.
...
def _verdict_diverges(world):
    """§4: артефакт отражает ВЕРДИКТ события, а не только его отпечатки.

    Третье из трёх сравнений §4 («event body/subject digests и verdict
    совпадают»). Событие может нести неизменные тело и предмет и при этом быть
    процитировано с чужим исходом — тогда перед нами не запись события, а
    самостоятельное утверждение о полномочии, и §4 запрещает именно это.

    Артефакт, вердикта НЕ ЗАЯВЛЯЮЩИЙ, находкой этого правила не является:
    сравнивать не с чем, а диагностики «артефакт без вердикта» приёмка не
    объявляет — завести свою значило бы написать контракт реализацией (тот же
    исход, каким выше разрешён артефакт без роли). Поэтому присутствие
    спрашивается через `has`: отсутствие координаты здесь — законное состояние
    мира, а не собственный отказ испытуемого.

    Молчание стоит ЗДЕСЬ, а не в применимости правила, намеренно: применимость,
    зависящая от присутствия судимой координаты, сняла бы правило молча — оно
    исчезало бы из переписи ровно на тех мирах, где сравнение не состоялось.
    """
    if not world.has(ARTIFACT_VERDICT):
        return False
...
RULES = [
    ...
    Rule(
        rule_id="auth.verdict-mirrors-event",
        diagnostic="CG_REVIEW_VERDICT_MISMATCH",
        category=outcome.CATEGORY_RED,
        subject_keys=(EPOCH, "artifact", "event"),
        requires=(EPOCH, "artifact", EVENT_VERDICT),
        applicability=_judges_post_cutover_world,
        predicate=_verdict_diverges,
        why="§4 объявляет пару «артефакт ↔ событие» сходящейся по ТРЁМ "
            "величинам, и вердикт — третья: событие с неизменными телом и "
            "предметом всё ещё может быть процитировано с чужим исходом. "
            "Применимость держится на контейнере артефакта и вердикте "
            "события, а НЕ на самой судимой координате — иначе правило "
            "снимало бы себя молча там, где артефакт вердикта не несёт",
    ),
]

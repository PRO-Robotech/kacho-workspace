...
# Ось 1 обезврежена: условие «поле пусто ИЛИ равно прочерку» превращено в «и то и
# другое разом», то есть не выполнимо никогда. Страж перестаёт звать на сведённой
# волне, оставаясь исправным во всём остальном.
b="$(mksandbox)"
if mr_patch "$b/$HK_REL" \
    's/\[ -z "\$monitor" \] \|\| \[ "\$monitor" = "—" \]/[ -z "\$monitor" ] \&\& [ "\$monitor" = "—" ]/' \
    "условие открытой строки ведомости"; then
    run 1 "$b" "инъекция: страж молчит на сведённой волне — краснеет" \
        check-10-wave-monitor-fires-on-its-triggers.sh
fi

# Законный близнец ТОЙ ЖЕ оси: то же условие, записанное иначе. Без него гейт
# закреплял бы ФОРМУ записи, а не поведение, и запрещал бы автору любую другую.
b="$(mksandbox)"
if mr_patch "$b/$HK_REL" \
    's/\[ -z "\$monitor" \] \|\| \[ "\$monitor" = "—" \]/! [ -n "\$monitor" ] \|\| [ "\$monitor" = "—" ]/' \

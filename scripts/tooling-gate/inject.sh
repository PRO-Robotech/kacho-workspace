...
# ось A: норма ядра о критериях снята
b="$(mksandbox)"
if mr_patch "$b/$CORE_REL" 's/ban18-criteria/ban18-questions/g' \
    "норма ban18-criteria переименована в ядре"; then
    run 1 "$b" "инъекция: норма критериев исчезла из ядра — краснеет" "$C11"
fi
...
b="$(mksandbox)"
if mr_patch "$b/$CORE_REL" 's/ban18-workaround-is-not-a-breach/ban18-workaround-any-way/g' \
    "норма «обход не есть нарушение» переименована в ядре"; then
    run 1 "$b" "инъекция: норма «обход среди БЕЗОПАСНЫХ способов» исчезла из ядра — краснеет" "$C11"
fi

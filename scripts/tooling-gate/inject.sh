...
#
# Знаменатель обхода оси 2 — ревизия ствола из ПОСЛЕДНЕЙ строки ведомости.
...
# ЗНАМЕНАТЕЛЬ ВЫВЕДЕН ИЗ СТРАЖА, А НЕ ВЫПИСАН — и вот чем это доказывается.
# Страж получает ТРЕТЬЮ метку («н/д» наравне с пустым и «—»), то есть заводит
# шесть новых клеток состояния последней строки, ни одна из которых не подана
# фикстурой. Гейт обязан покраснеть САМ, без правки: перечень меток он снимает
# с исходника стража. Выписанный перечень на этой инъекции остался бы зелёным.
b="$(mksandbox)"
if mr_patch "$b/$HK_REL" \

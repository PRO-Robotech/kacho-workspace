...
echo "== check-10: выписанный предикат на форме, где два движка расходятся =="
...
if [ -x "$CC_BIN" ]; then
    ...
    if [ -z "$ug_out" ] && [ -n "$gnu_out" ]; then
        ...
        echo "  ПРОВАЛ предпосылка check-10 — движки БОЛЬШЕ НЕ расходятся" >&2
        echo "         обёртка=[$ug_out] GNU=[$gnu_out]; запрет потерял предмет и обязан быть снят вместе с проверкой" >&2
        failed=$((failed + 1))
    fi
else
    echo "  VOID предпосылка check-10 — второго движка нет ($CC_BIN), расхождение здесь не проверяемо"
fi

C10_REL=".claude/hooks/README-inject-probe.md"

b="$(mksandbox)"; run 0 "$b" "чистое дерево — молчит" check-10-handrun-predicate-survives-both-greps.sh
...
run 1 "$b" "инъекция: захватывающая скобка с якорем — краснеет" check-10-handrun-predicate-survives-both-greps.sh
...
run 1 "$b" "инъекция: НЕЗАХВАТЫВАЮЩАЯ скобка с якорем — тоже краснеет" check-10-handrun-predicate-survives-both-greps.sh
...
run 0 "$b" "близнец: та же форма на исполняемой строке скрипта — молчит" check-10-handrun-predicate-survives-both-greps.sh
...
run 0 "$b" "близнец: та же граница просмотром назад — молчит" check-10-handrun-predicate-survives-both-greps.sh
...
run 0 "$b" "близнец: та же форма без вызова grep на строке — молчит" check-10-handrun-predicate-survives-both-greps.sh

b="$(mksandbox .claude)"
rm -rf "$b/scripts" "$b/CLAUDE.md"
run 2 "$b" "предпосылка: файлов области нет — VOID, а не успех" check-10-handrun-predicate-survives-both-greps.sh

...
echo "== check-11: перепись каскада читает одно отношение из двух =="
CC_REL="scripts/cascade-census.sh"
C11=check-11-cascade-census-reads-both-relations.sh
...
if mr_patch "$b/$CC_REL" 's/next unless \//next; next unless \//' "перечень тела не читается"; then
    run 1 "$b" "инъекция: читается только sub-issue (kacho#2794 проходит) — краснеет" "$C11"
fi
b="$(mksandbox)"
if mr_patch "$b/$CC_REL" 's/void "дочерних ноль/true || void "дочерних ноль/' "пустой уровень зелёный"; then
    run 1 "$b" "инъекция: дочерних ноль выдано за «открытых ноль» — краснеет" "$C11"
fi
b="$(mksandbox)"
if mr_patch "$b/$CC_REL" 's/\[ "\$state" = closed \] && \[ "\$open_n" -gt 0 \]/false/' \
    "закрытый уровень с открытыми не находка"; then
    run 1 "$b" "инъекция: волна закрыта при открытой задаче, находки нет — краснеет" "$C11"
fi
b="$(mksandbox)"
if mr_patch "$b/$CC_REL" 's/gh api --paginate /gh api /' "sub-issue без страниц"; then
    run 1 "$b" "инъекция: sub-issue читаются одной страницей — краснеет" "$C11"
fi
b="$(mksandbox)"
if mr_patch "$b/$CC_REL" 's/next if \$fence;/1;/' "блок кода читается перечнем"; then
    run 1 "$b" "инъекция: пример в блоке кода принят за дочерний — краснеет" "$C11"
fi
b="$(mksandbox)"
if mr_patch "$b/$CC_REL" 's/m\{\^#\(\\d\+\)\\b\}/m{#(\\d+)\\b}/' "ссылка дальше по строке — дочерний"; then
    run 1 "$b" "инъекция: упоминание дальше по строке принято за дочерний — краснеет" "$C11"
fi
...
if mr_patch "$b/$CC_REL" 's/elif \[ "\$WANT_CLOSED" = 1 \] && \[ "\$open_n" -gt 0 \]/elif false/' \
    ...
    run 1 "$b" "инъекция: --children-closed принят и не судит открытых — краснеет" "$C11"
fi
...
if mr_patch "$b/$CC_REL" 's/\[-\*\+\]\\s\+\\\[/[+*-]\\s+\\[/' "маркеры списка другой записью"; then
    run 0 "$b" "близнец: те же маркеры списка другой записью — молчит" "$C11"
fi

b="$(mksandbox scripts/cascade-census.sh)"
run 2 "$b" "предпосылка: инструмента нет — VOID, а не успех" "$C11"

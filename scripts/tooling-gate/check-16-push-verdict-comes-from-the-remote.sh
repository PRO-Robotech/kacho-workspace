#!/usr/bin/env bash
# check-16 — ВЕРДИКТ ОБ ОТПРАВКЕ БЕРЁТСЯ У СЕРВЕРА, А НЕ У КОДА ВОЗВРАТА.
#
...
# Имя выводится из файла, а не выписывается литералом: номер проверки занимают
# параллельные полосы, и переименование не должно оставлять старое самоимя (ws#749).

...
# `Unknown skill: rule-vault` — вызвано 2026-09-21 из сессии агента, агент УЗНАЁТ.
# У ЭТОГО ФАКТА ОДИН СВИДЕТЕЛЬ и он НЕ ВОСПРОИЗВОДИМ ИЗ ОБОЛОЧКИ: инструмент
# `Skill` доступен только из сессии, и стенд приёмки повторить замер не может. На вердикт
# этого гейта факт не влияет — гейт судит ДЕРЕВО, а не поведение среды, и все его
# оси воспроизводятся инъекцией. Но если факт понадобится КАК ДОВОД, его надо
# переснять тому, кто читает: один свидетель — это «со слов автора», а не замер.

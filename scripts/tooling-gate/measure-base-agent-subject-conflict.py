...
ВПИСАН В НАБОР 2026-09-21 через `check-16-base-and-agent-agree-on-subject.sh` (заведён номером 12, перенумерован 2026-09-22: номер 12 носит проверка соседней полосы).
До этого дня прибор давал на дереве 1 находку — `acceptance-author`, — и она не чинилась,
пока не было решено, какая из двух сторон права: основания, на которое ссылалась база,
в дереве не нашлось. Решение диспетчера 2026-09-21 предмет закрыло (§12 базы, строка
с датой, автором и основанием), тело `acceptance-author` приведено ПОСЛЕ этого, и держатель
встал тем же изменением. Инъекция — `prove-base-agent-subject-conflict.sh`.
"""

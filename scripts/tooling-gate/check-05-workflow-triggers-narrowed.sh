...
# релизная ветка» и `.claude/backup/multi-agent-flow-shared-tree.md` §8 обещают: конвейер тратится
...
if triggers == 0:
    # АВТОЗАПУСКА НЕТ — состояние ОБЪЯВЛЕННОЕ (решение владельца 2026-09-20 «на любом
    # бранче конвейер не активен, но описан в ямл»), а не потерянный предмет: задания
    # описаны и исполнимы вручную (`workflow_dispatch`), просто ни одно событие дерева
    # их не поднимает. Отказ здесь стал бы ВЕЧНЫМ, а вечный отказ — это проверка,
    # которую снимут не глядя, вместе с остальными её осями.
    #
    # Опасная сторона под контролем ровно та же, что и раньше: контекст, который не
    # начался, не зеленеет и не краснеет — он остаётся «ожидается». Значит объявление
    # «автозапуска нет» законно ТОЛЬКО пока защита ветки обязательных контекстов не
    # требует; иначе слияние заблокировано навсегда, и это находка, а не вкусовщина.
    if required:
        for ctx in required:
            print(f"[FAIL] {name} — автозапуска нет ни на одной ветке, а защита ветки "
                  f"требует контекст «{ctx}»: он не начнётся, останется «ожидается» и "
                  f"заблокирует слияние навсегда", file=sys.stderr)
        sys.exit(1)
    print(f"[PASS] {name} — автозапуска нет ни на одной ветке (объявлено решением), "
          f"обязательных контекстов защита не требует; защита ветки — {protection_state}")
    sys.exit(0)

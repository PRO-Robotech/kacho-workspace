...
# браузере.
#
# РЕЖИМ ПОСАДКИ ЭПИКА. Эпик до посадки в `main` открыт, а его волны к ней
# закрыты (`git-issues.md#gi-cascade-epic-main`). Без ключа открытый уровень с
# открытым дочерним законно даёт 0 (проба H), поэтому предикат посадки без
# ключа не краснел никогда: у ws#771 открытых волн три, код 0 (возврат
# wave-reviewer к ws#845, круг 2). Ключ `--children-closed` судится той же
...
# L — законный близнец режима посадки: уровень открыт, все дочерние закрыты в
# обоих отношениях.
L="$(mkcase L)"
...
probe "$L" 900 0 "без ключа: открытый уровень, дочерние закрыты — вердикт с total" \
    "ИТОГ: дочерних 2" "открытых 0"

FLAGS=(--children-closed)
probe "$H" 600 1 "--children-closed: уровень открыт, дочерний открыт — находка, открытый назван" \
    "режим:      --children-closed" "открытые:   $R#1" "уровень закрывать рано"

probe "$L" 900 0 "--children-closed: уровень открыт, все дочерние закрыты — законный близнец молчит" \
    "режим:      --children-closed" "ИТОГ: дочерних 2" "открытых 0"

probe "$D" 200 1 "--children-closed: уровень закрыт, дочерний открыт — прежняя находка не потеряна" \
    "уровень закрыт" "открытые:   $R#2"

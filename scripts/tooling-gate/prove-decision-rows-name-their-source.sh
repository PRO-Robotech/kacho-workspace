...
# ── ось E: ВЕДОМОСТЬ НЕ РАСШИРЯЕТСЯ ОДНОЙ БУКВОЙ ────────────────────────────
# Прежняя редакция сверяла ПРЕФИКСОМ: запись в один знак — префикс чего угодно,
# и гасила любую новую строку без источника. Заявленное «краснеет в обе стороны»
# держалось тогда длиной записей, а не предикатом.
d="$(sandbox "Старое чтение → новое чтение без всякого источника.")"; ledger "$d" "С"
assert 1 "$(run "$d")" "ДЕФЕКТ: однобуквенная запись ведомости НЕ гасит новую строку"; rm -rf "$d"

d="$(sandbox "Старое чтение → новое чтение без всякого источника.")"
ledger "$d" "Старое чтение"
assert 0 "$(run "$d")" "БЛИЗНЕЦ: ТОЧНЫЙ ключ той же строки — молчит"; rm -rf "$d"

# ── ось F: ВТОРАЯ ЗАКОННАЯ ФОРМА КООРДИНАТЫ ────────────────────────────────
# Канонический полный путь — такой же источник, как голое имя. Прежняя редакция
# знала одну форму в нижнем регистре и давала ЛОЖНОЕ КРАСНОЕ на законной записи.
d="$(sandbox "Старое чтение → новое чтение (\`.claude/rules/testing.md\` §«Раздел»).")"; ledger "$d" ""
assert 0 "$(run "$d")" "БЛИЗНЕЦ: координата ПОЛНЫМ ПУТЁМ — источник, молчит"; rm -rf "$d"

d="$(sandbox "Старое чтение → новое чтение (\`CLAUDE.md\` §«Раздел»).")"; ledger "$d" ""
assert 0 "$(run "$d")" "БЛИЗНЕЦ: координата С ЗАГЛАВНЫМИ — источник, молчит"; rm -rf "$d"

#!/usr/bin/env bash
# ДОКАЗЫВАЕТ: check-13-decision-rows-name-their-source.sh (зовётся из inject.sh; строку читает его перепись доказанности)
...
# ── ось G: ДАТА ОБЯЗАНА СУЩЕСТВОВАТЬ, а не иметь форму ─────────────────────
# Прежняя редакция судила образец `20\d\d-\d\d-\d\d`, и несуществующий день
# проходил источником: 13-й месяц и 45-е число.
d="$(sandbox "Старое чтение → новое чтение, решение владельца 2026-13-45.")"; ledger "$d" ""
assert 1 "$(run "$d")" "ДЕФЕКТ: несуществующая дата источником НЕ считается"; rm -rf "$d"

d="$(sandbox "Старое чтение → новое чтение, решение владельца 2026-02-29.")"; ledger "$d" ""
assert 1 "$(run "$d")" "ДЕФЕКТ: 29 февраля невисокосного года — тоже не дата"; rm -rf "$d"

d="$(sandbox "Старое чтение → новое чтение, решение владельца 2024-02-29.")"; ledger "$d" ""
assert 0 "$(run "$d")" "БЛИЗНЕЦ: 29 февраля ВИСОКОСНОГО года — законно, молчит"; rm -rf "$d"

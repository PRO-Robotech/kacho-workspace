...
Строка тела читается ОДНИМ читателем, снимающим врезку (`unquote`), — и разбор
таблицы, и разбор объявления сценария идут через него. Их было два, и они
разошлись: таблица врезку снимала, объявление сценария — нет, поэтому P6
проверялся над РАЗНЫМИ множествами строк одного документа. Класс — слепота
распознавателя к законной форме записи: форма, о которой он не знает, даёт не
красное и не зелёное, а молчание, неотличимое от отсутствия предмета
(`testing.md` §«Гейт на класс», п. 7). Здесь она молчала в одну сторону
(сценарий во врезке без строки не находился) и лгала в другую (его строка
объявлялась осиротевшей). Перепись печатает объём самого распознавателя —
объявлений прочитано и сколько из них во врезке, — иначе расширение
распознавателя нечем отличить от холостого.
...
def unquote(line):
    """Строка тела без обёртки врезки: `> **X — …**` читается как `**X — …**`.

    ЕДИНСТВЕННЫЙ снимающий врезку читатель на весь файл, и заведён он именно
    поэтому. Читателей было два, и они разошлись: разбор таблицы врезку снимал,
    разбор объявления сценария — нет. Документ, объявивший сценарий во врезке,
    для одного читателя существовал, для другого нет. Расхождение молчит в одну
    сторону (сценарий без строки не находится вовсе) и ЛЖЁТ в другую (его строка
    объявляется осиротевшей — «сценария с таким идентификатором в документе
    нет», при том что он в документе есть). Ни один из двух исходов не отличим
    от исправной работы по виду вывода.

    Вложенную врезку (`> > …`) читатель НЕ разворачивает, и это измерено, а не
    забыто: строк такого вида в корпусе 208, а объявлений сценария и строк
    таблицы среди них ноль (187 и 13475 — обе величины не меняются от разворота).
    Расширение, не двигающее перепись, холостое, и его не заводят «на всякий
    случай»: оно добавляет непроверенное поведение, за которое никто не отвечает.
    """
    body = line.strip()
    if body.startswith(">"):
...
def scenario_of(line):
    """Идентификатор сценария, объявленного этой строкой, либо None."""
    m = SCENARIO.match(unquote(line))
...
def audit(root, repo, index, rel, findings, stats):
    ...
    # Перепись распознавателя ведётся ДО отбора носителей: она про то, сколько
    # объявлений ПРОЧИТАНО, а не сколько документов дошло до сверки. Считай
    # после отбора — и «объявлений ноль» у документа без таблицы стало бы
    # неотличимо от «объявлений он не несёт», то есть от слепоты самого
    # распознавателя. Ровно это различие проверка и обязана делать видимым.
    scenarios = set()
    for line in text.split("\n"):
        cid = scenario_of(line)
        if cid is None:
            continue
        scenarios.add(cid)
        stats["decls"] += 1
        if line.lstrip().startswith(">"):
            stats["decls_quoted"] += 1
    tabs = tables(text)
    if not tabs:
        return False
    listed = set()
...
def main():
    ...
    stats = {"tables": 0, "rows": 0, "paths": 0, "checks": 0,
             "decls": 0, "decls_quoted": 0}
    carriers = [rel for rel in docs if audit(root, repo, index, rel, findings, stats)]

    # Две величины распознавателя печатаются рядом с величинами сверки: «находок
    # ноль» обязано быть отличимо от «прочитано ноль», а расширение распознавателя
    # обязано двигать ПЕРВУЮ из них — иначе оно холостое и подлежит снятию.
    _lib.census(
        "%s: приёмок осмотрено %d; несут таблицу состояния %d (%s); таблиц %d, строк %d; "
        "объявлений сценариев распознано %d, из них во врезке %d; "
        ...
        % (NAME, len(docs), len(carriers),
           ...
           stats["decls"], stats["decls_quoted"], head or "неизвестно",

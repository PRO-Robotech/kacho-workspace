...
# ОБЕ СТОРОНЫ ОДНОЙ СЛЕПОТЫ: сценарий, объявленный ВО ВРЕЗКЕ. Разбор таблицы в
# этой же проверке врезку снимает (`split_row` делает `lstrip(">")`), а разбор
# объявления сценария — не снимал: два читателя одного документа расходились в
# том, что считать строкой тела. Расхождение молчаливо в одну сторону и лживо в
# другую, поэтому проб две, и каждая меняет ровно один факт против близнеца.

b="$(mksandbox docs/specs)"; mkdir -p "$b/docs/specs"
{
    spec "| XC-99-01 | держится | \`$REAL_FILE\` :: \`$REAL_TEST\` |"
    printf '\n> **XC-99-03 — сценарий, объявленный во врезке**\n> - **Then** наблюдаемо\n'
} > "$b/$SPEC"
git -C "$b" add -A -f >/dev/null 2>&1
run 1 "$b" "инъекция: сценарий во врезке без строки таблицы — краснеет и называет код" \
    check-03-holding-claim-resolves.py "XC-99-03"

# Близнец отличается ОДНИМ фактом: у того же сценария есть своя строка. Он обязан
# молчать — но молчание засчитывается только вместе с ПЕРЕПИСЬЮ: без неё «молчит,
# потому что прочтён» неотличимо от «молчит, потому что не прочтён», а до починки
# было верно именно второе. Поэтому проба требует от вывода числа врезочных
# объявлений, а не только кода 0.
b="$(mksandbox docs/specs)"; mkdir -p "$b/docs/specs"
{
    spec "| XC-99-01 | держится | \`$REAL_FILE\` :: \`$REAL_TEST\` |"
    printf '| XC-99-03 | держится | `%s` :: `%s` |\n' "$REAL_FILE" "$REAL_TEST"
    printf '\n> **XC-99-03 — сценарий, объявленный во врезке**\n> - **Then** наблюдаемо\n'
} > "$b/$SPEC"
git -C "$b" add -A -f >/dev/null 2>&1
run 0 "$b" "близнец: тот же сценарий во врезке со своей строкой — молчит, и перепись его ПРОЧЛА" \
    check-03-holding-claim-resolves.py "во врезке 1"

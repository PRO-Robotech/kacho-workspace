...
воркспейс и дерево продукта, — резолюция ПУТИ дерева продукта (`monorepo`),
резолюция его СТВОЛА (`trunk`, `provenance`, `trunk_files`, `trunk_show`) и
резолюция ДОМА координаты по идентичности (`repo_identity`, `home_tree`,
`home_env`). Держать их копиями у каждой проверки значило бы завести два места об
одном предмете; они разошлись бы молча и именно там, где обе отвечают «вердикт
прочитан».

Дом — не то же, что дерево продукта, и появился он не для удобства: предмет пробы
переезжает вместе со своим продуктом, и координата, названная верно, перестаёт
резолвиться в дереве, откуда предмет уехал (`e2e-flow.md` §7а). Чтение при этом
остаётся стволовым: у дома спрашивают его `origin/main` теми же `trunk_files` и
`trunk_show`, что у продукта, — второй полосы чтения не заводится.
...
# ревизии, названные согласованностью (`multi-agent-flow-shared-tree.md` §8).
#
...
# ── ДОМ КООРДИНАТЫ: РЕПОЗИТОРИЙ ПРЕДМЕТА, ОПОЗНАВАЕМЫЙ ИДЕНТИЧНОСТЬЮ ──────────
#
# Предмет пробы живёт в репозитории СВОЕГО продукта, а не в том дереве, через чей
# край проба ходит (`e2e-flow.md` §7а). Служба доступа вынесена из монорепо
# отдельным репозиторием, и координаты её проб уехали вместе с ней: в стволе
# продукта их больше нет, а сами пробы живы. Поэтому координата вправе назвать
# свой дом приставкой `owner/name:`, а резолвится дом по ИДЕНТИЧНОСТИ рабочей
# копии — `owner/name` в `origin`, — а не по имени каталога.
#
# ПОЧЕМУ ПО ИДЕНТИЧНОСТИ, А НЕ ПО ИМЕНИ КАТАЛОГА. Имя каталога — произвольная
# метка: рядом лежат `w-seed-own`, `base-main`, `kacho-mr547b`, и ни одно из них
# не говорит, чей это клон. Опознание по метке было бы адресацией по мутабельному
# имени — ровно то, что core #15 запрещает ресурсам; `repos.sh` применяет то же
# правило к оснастке и по той же причине (§«Опознание идёт по origin»).
#
# ПОРЯДОК КАНДИДАТОВ ЗАКРЫТЫЙ, И ОБХОДА КАТАЛОГОВ В НЁМ НЕТ. Поиск по соседям
# отвергнут замером дерева, а не вкусом: рядом с воркспейсом лежат десятки
# припаркованных рабочих копий одного и того же репозитория, и вердикт стал бы
# функцией того, какая из них попалась первой, — тот же класс, из-за которого весь
# набор судит СТВОЛ, а не индекс копии. Кандидатов три: переменная
# `KACHO_HOME_<ИМЯ>` · уже резолвнутое дерево продукта, если его идентичность
# совпала с названной · `project/<имя>` от корня воркспейса — место, куда клонирует
# `bootstrap.sh` и куда выкладывает дерево конвейер.
#
# ДОМ НЕ РЕЗОЛВИТСЯ — ЭТО ТРЕТЬЯ КАТЕГОРИЯ У ЧИТАТЕЛЯ, а не находка и не проход:
# «копии рядом нет» есть несозданное условие, «координаты в доме нет» — дефект
# документа. Здесь возвращается ПРИЧИНА текстом, перечисляющая каждого
# отвергнутого кандидата и способ создать условие; категорию объявляет читатель
# своим текстом — «проверять нечего» у каждой проверки своё.
HOME_ENV_PREFIX = "KACHO_HOME_"


def home_env(identity):
    """Имя переменной, которой дом называют явно: `PRO-Robotech/kaname` → `KACHO_HOME_KANAME`."""
    name = identity.rsplit("/", 1)[-1]
    return HOME_ENV_PREFIX + re.sub(r"[^A-Za-z0-9]", "_", name).upper()


def repo_identity(repo):
    """«owner/name» рабочей копии по её `origin`, либо None.

    Нормализация та же, что у `repos.sh`: снимается `.git` и хвостовой слэш,
    берутся два последних сегмента пути. Форма URL роли не играет — https, ssh
    (scp-подобная) и file:// дают один и тот же ответ.

    Фильтра имён здесь НЕТ, в отличие от `repos.sh`: тот ОТКРЫВАЕТ каталог и
    решает, репозиторий ли это продукта, а здесь имя названо документом, и вопрос
    обратный — та ли это копия, которую назвали.
    """
    url = _repo_git(repo, ["remote", "get-url", "origin"])
    if not url:
        return None
    path = url.strip()
...
def home_tree(root, identity, product=None):
    """(путь, причина) — дерево названного дома. Путь None означает: дома нет.

    Причина перечисляет КАЖДОГО отвергнутого кандидата и то, чем он отвергнут.
    Без перечня «дома нет» читалось бы как «искали одно место»: переменная,
    указывающая мимо, и отсутствие клона — разные починки, и одна из них состоит
    в правке того, что уже объявлено.
    """
    env = home_env(identity)
    name = identity.rsplit("/", 1)[-1]
    candidates = []
    if os.environ.get(env):
        candidates.append((os.environ[env], "$" + env))
    if product:
        candidates.append((product, "дерево продукта"))
    candidates.append((os.path.join(root, "project", name), "project/%s" % name))

    tried = []
    for path, whence in candidates:
        if not os.path.exists(os.path.join(path, ".git")):
            tried.append("%s (%s) — рабочей копии там нет" % (path, whence))
            continue
        got = repo_identity(path)
        if got is None:
            tried.append("%s (%s) — у копии нет origin, идентичность непроверяема"
                         % (path, whence))
            continue
        if got.lower() != identity.lower():
            tried.append("%s (%s) — это копия %s, а не %s" % (path, whence, got, identity))
            continue
        return path, None
    return None, ("копия дома %s рядом не найдена [%s]. Условие создаётся так: "
                  "git clone https://github.com/%s.git <путь> и %s=<путь>, "
                  "либо клон в project/%s"
                  % (identity, "; ".join(tried), identity, env, name))

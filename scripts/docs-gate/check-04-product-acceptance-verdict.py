...
def main():
    ...
    if not docs:
        # ГДЕ ПРЕДМЕТ ТЕПЕРЬ — НАЗЫВАЕТСЯ ЗДЕСЬ, иначе третья категория читается как
        # «приёмок у продукта не бывает». Замер 2026-09-13: формы «каталог
        # `acceptance/`» в дереве продукта 0, в репозитории службы
        # `PRO-Robotech/kaname` — 30; вынес их #2598 вместе со службой. Их вердикт
        # судит гейт ДОМА (kaname#11), и это верно по `e2e-flow.md` §7а: дом
        # проверки — репозиторий её предмета. Здесь читать действительно нечего, и
        # это не дефект — но читатель обязан узнать, куда идти.
        _lib.void(NAME, "в дереве продукта (%s) приёмок не найдено ни одной из "
                        "двух форм — читать нечего. Приёмки компонента живут в "
                        "репозитории компонента (формы «каталог `acceptance/`» "
                        "здесь 0, в PRO-Robotech/kaname 30 — вынесены #2598), и их "
                        "вердикт судит гейт дома" % where)
        return 2

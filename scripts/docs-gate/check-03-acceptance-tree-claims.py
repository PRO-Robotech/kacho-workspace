...
пишут, а не там, где оно потом краснеет. База — СТВОЛ дерева продукта, и
резолвит его ОДИН на оба набора `scripts/lib/product_trunk.py`: выписанная пара
`origin/main`/`main` была здесь вторым кодеком об одном предмете и не знала, что у
общей копии продукта удалённых ДВА, причём у второго история НЕсвязная (ws#621).
Ствол не разрешается — ось объявляется НЕосмотренной переписью, а не считается
пройденной.
...
# СТВОЛА дерева продукта, и резолв у неё ОДИН на оба набора (`_lib.trunk` →
# `scripts/lib/product_trunk.py`). Своя пара имён жила здесь до ws#621 и судила по
# первому попавшемуся `origin/main`, не спрашивая о РОДСТВЕ: у общей копии
# продукта рядом с `origin` стоит удалённый на архив прежнего полирепо, чья
# история с деревом не пересекается вовсе.
...
def main():
    ...
    if base is None:
        judged = ("достижимость ревизий НЕ судилась — ствол дерева продукта не разрешён "
                  "(искали: %s)" % (", ".join(t.candidates) or "нечего"))
    elif unjudged:
    ...
    _lib.census(
        ...
        % (NAME, DOC, len(rows), len(CLAIMS), _lib.where(repo, t),

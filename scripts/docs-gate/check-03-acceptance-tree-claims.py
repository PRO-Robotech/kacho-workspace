...
пишут, а не там, где оно потом краснеет. База — СТВОЛ дерева продукта, и резолв у
него ОДИН на всех читателей (`_lib.trunk`): выписанная пара `origin/main`/`main`
была здесь вторым кодеком об одном предмете, а локальная ветвь в общей копии
продукта отстаёт от ветки слежения и потому объявляла бы недостижимым ровно
свежее. Ствол не разрешается — ось объявляется НЕосмотренной переписью, а не
...
# Ревизия, относительно которой судится ДОСТИЖИМОСТЬ закрепления, — вершина
# СТВОЛА дерева продукта, и резолв у неё ОДИН на всех читателей (`_lib.trunk`,
# `_lib.TRUNK_REF`). Своя пара имён `origin/main`/`main` жила здесь до ws#621: она
# принимала локальную ветвь за ствол, а в общей копии продукта та отстаёт от ветки
# слежения — то есть ось судилась по ревизии, которая свежую работу не содержит.
...
# Путь дерева продукта берётся из `_lib` — по одному резолву на набор. Своя копия
# жила здесь и объявляла, что ищет «так же, как vault-gate», расходясь с ним на
# форме `.git` рабочей копии: расхождение делало проверку беспредметной у всякого,
# кто работает из рабочей копии, — то есть у всех.
...
def main():
    ...
    # `rev_full` намеренно, а не короткая форма: `merge-base --is-ancestor` ниже
    # сверяет ревизии, и укороченная строка сюда попадать не должна.
    ref, base = prov["ref"], prov["rev_full"]
    findings = []
    seen = []
    lines = []
    unjudged = 0
    ...
    if base is None:
        judged = ("достижимость ревизий НЕ судилась — ствол дерева продукта (%s) "
                  "не резолвится в этом клоне" % _lib.TRUNK_REF)
    elif unjudged:
    ...
    _lib.census(
        ...
        "ревизий в закреплениях %d; %s"
        % (NAME, DOC, len(rows), len(CLAIMS), _lib.provenance_line(repo, prov),

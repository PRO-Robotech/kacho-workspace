...
# СИНТЕТИЧЕСКОЕ ИМЯ ФИКСТУРЫ НЕ ВЫПИСЫВАЕТСЯ ПОЛНЫМ АДРЕСОМ КОРПУСА.
# Пробы кладут в песочницу файлы, которых в дереве нет и быть не должно:
# `probe-monotone.md` — не правило и никогда им не было, это ВХОД опыта, живущий
# внутри `mktemp -d` ровно до `trap … EXIT`. Но написанный целиком путь
# `.claude/rules/<имя>.md` есть РАБОЧИЙ АДРЕС: по нему пойдёт и читающий, и
# проверка набора, судящая адрес ТАМ, ГДЕ ОН НАПИСАН. Выписанный литералом, он
# неотличим от ссылки на снятое правило — и объявить его долгом значило бы
# закрепить ложное имя навсегда: фикстура не резолвится никогда, а долг обязан
# только сокращаться. Поэтому координата СОБИРАЕТСЯ: в тексте прибора адреса
# нет, в песочнице файл лежит ровно там, где проба его ждёт.
# Настоящие адреса — `.claude/rules/MANIFEST.md` в probe_01 и probe_03 — здесь
# остаются литералами: они резолвятся, и прятать их было бы выхолащиванием.
# shellcheck disable=SC2329  # зовётся из probe_05 и probe_08; те сами зовутся косвенно (имя из имени проверки), и «никогда не вызвана» распространяется на вызываемое ими транзитивно
fixture() {   # <каталог песочницы> <подкаталог .claude> <имя файла> -> путь
    printf '%s/.claude/%s/%s' "$1" "$2" "$3"
}
...
probe_05() {   # маршрутизация: дописан файл исполнителя, которого база не знает
    cat > "$(fixture "$1" agents probe-monotone.md)" <<'AGENT'
---
name: probe-monotone
...
probe_08() {   # frontmatter: дописан файл правила без frontmatter
    printf 'Файл правил, дописанный пробой монотонности, без frontmatter.\n' \
        > "$(fixture "$1" rules probe-monotone.md)"
}
...
# ── ЗАМЕР ────────────────────────────────────────────────────────────────────
#
# НОМЕР ПРОВЕРКИ — КЛЮЧ ПРИВЯЗКИ ПРОБЫ, И ОН ОБЯЗАН БЫТЬ УНИКАЛЕН. Связь
# «check-NN-… → probe_NN» держится тем, что номер в наборе один. Две полосы,
# заведшие по check-10 каждая, сводятся в дерево, где номер носят ДВЕ проверки, —
# и тогда проба, написанная под предмет первой, молча прогоняется по второй.
# Исход у такого замера один из двух, и оба ложны: зелёное прочтётся как
# «монотонна», красное — как «немонотонна», а относиться будет к чужому
# предмету. Поэтому номер сперва пересчитывается по глобу; повторившийся — не
# вердикт, а «НЕ ИЗМЕРЕНО» с названной причиной. Снимается это переномерацией
# одной из проверок, и решение принимает сведение волны, не прибор.
dup_nums=""
for c in "$HERE"/check-*; do
    [ -f "$c" ] || continue
    dup_nums="$dup_nums$(basename "$c" | sed -n 's/^check-\([0-9][0-9]*\)-.*/\1/p')\n"
done
dup_nums="$(printf '%b' "$dup_nums" | grep -v '^$' | sort | uniq -d)"
...
for c in "$HERE"/check-*; do
    ...
    if [ -n "$num" ] && printf '%s\n' "$dup_nums" | grep -qx -- "$num"; then
        unmeasured=$((unmeasured + 1))
        row "$name" "НЕ ИЗМЕРЕНО" \
            "номер $num носят несколько проверок набора — вердикт относился бы к чужому предмету"
        continue
    fi

...
# ── ось A: перевес ориентира ПЕЧАТАЕТСЯ И НЕ БЛОКИРУЕТ ──────────────────────
# До 2026-09-22 эта ось утверждала обратное — «перевесил потолок, краснеет».
# Предел снят с блокировки: он судил СУММУ, которой не грузит никто (максимум по
# агенту 85 559 при сумме 197 784). Ось не снята вместе с ним, а ПЕРЕВЕДЕНА на
# наблюдение: молчание проверки о перевесе неотличимо от молчания о его
# отсутствии, поэтому проверяется и код, и НАПЕЧАТАННОЕ.
...
PY
capture "$d" "$C6"
assert_code 0 "НАБЛЮДЕНИЕ: корпус перевесил ориентир 200 000 — НЕ блокирует"
assert_says "перейдён на" "  ...и перевес НАПЕЧАТАН числом, а не проглочен"
assert_says "НЕ блокирует" "  ...и сказано прямо, что это наблюдение"
...
PY
assert_fixture_changed "$d" "$b" "БЛИЗНЕЦ: корпус вырос на 841 символ (28 симв × 30 + перевод строки — измерено, а не из метки)"
capture "$d" "$C6"
assert_code 0 "БЛИЗНЕЦ: рост под ориентиром — молчит"
assert_says "до прежнего ориентира" "  ...и остаток до ориентира напечатан в ЭТУ сторону тоже"

...
узнаётся по УСТРОЙСТВУ, и оснований ровно два, каждое — свойство, а не имя:
  ...
  (1) ФАЙЛ-РЕФЕРЕНТ ИЗГОТАВЛИВАЕТ КОПИЮ КОРПУСА, А НЕ ЧИТАЕТ ЭТОТ. Барьеров
      ЧЕТЫРЕ, и каждый — свойство, а не имя. Первые три взяты у
      `check-11-rule-address-exists-as-written.sh` соседней полосы после того, как
      приёмка опытом показала: без них роль выдавалась ОДНОЙ СТРОКОЙ КОММЕНТАРИЯ —
      дописанное в хук `# пример пути: "$SOME_ROOT/.claude/rules/"` снимало с него
      суд целиком (RC 1 -> 0). Барьеры:
        · ФОРМАТ — разметке (`.md`) роль не выдаётся никогда: её читают, а не
          исполняют, и `$VAR/.claude/` в ней всегда цитата;
        · ИСПОЛНЯЕМАЯ ЧАСТЬ — строки, начинающиеся с `#`, отбрасываются до
          разбора. Гейт судит то, что исполняется, а не то, что написано
          (`testing.md` §«Гейт читает исполняемую часть, а не текст»);
        · СОЗДАЮЩИЙ АКТ, А НЕ УПОМИНАНИЕ — нужен `>`/`>>`, либо
          `mkdir`/`cp`/`mv`/`ln`/`install`/`rsync`, действующий В `…/.claude/` под
          корнем-переменной. Прежняя редакция брала любое вхождение
          `$VAR/.claude/`, и под роль попадали ДВА ЖИВЫХ ГЕЙТА, которые корпус
          читают под `$RULES_GATE_ROOT`, а не изготавливают:
          `scripts/rules-gate/check-01-corpus-matches-manifest.sh` и
          `scripts/skills-gate/check-03-roster-matches-tree.sh`;
        · SHEBANG — файл исполняется, а не читается.
      Литерал `.claude/rules/<имя>.md` в таком файле — ВХОД, который аппаратура
      изготавливает, а не адрес в этом дереве. Никакой `inject-NN` и никакой
      `prove.sh` в перечне не назван: их берёт это свойство, и оно же берёт
      производителя под посторонним именем (`measure-monotonicity.sh`);
...
ГРАНИЦА ПРИЗНАКА (1) НАЗВАНА НЕ СЛОВОМ, А ЧИСЛОМ: он освобождает ВСЕ координаты
такого файла, в том числе настоящую висячую. Граница, названная словом, не
перемеряется, поэтому перепись печатает ТРИ числа, а не одно: сколько координат
снято ролью, СКОЛЬКО ИЗ НИХ ВИСЯЧИЕ (вот она, цена) и сколько живых. Корень
обхода печатается там же: прогон из чужого каталога судил бы чужое дерево и
выходил бы нулём, а по одному знаменателю это не отличить.
...
СОСЕДНЯЯ ПОЛОСА ДЕРЖИТ ТУ ЖЕ АДРЕСНУЮ ПОЛОВИНУ, И ЭТО СКАЗАНО ЗДЕСЬ, А НЕ ПОСЛЕ
СВЕДЕНИЯ. Ветка `lane/rules-gate-root-and-ceiling-20260922` (голова 7da752e6, в стволе
её НЕТ) несёт `check-11-rule-address-exists-as-written.sh`: тот же класс, четыре формы
написанного адреса и признак фикстуры, выведенный из СОЗДАЮЩЕГО АКТА над `.claude/` под
своим корнем. Перекрытие названо поимённо:
  · адрес `.claude/rules/<имя>.md` и путь скила — держат ОБЕ; их признак фикстуры
    строже (он различает создающий акт и простое чтение вроде
    `"$CLAUDE_PROJECT_DIR/.claude/hooks/…"`), и при сведении адресную половину
    разумнее оставить за check-11;
  · строку таблицы `MANIFEST.md` не держит ни одна из четырёх форм check-11
    (предикат: `grep -c MANIFEST rule-address-refs.py` = 0) — ТРОЙКА целиком держится
    только здесь, и ради неё эта проверка и заведена.
Решение, какая из двух держит что после сведения волны, принимает владелец: до сведения
адресную половину держит эта, потому что check-11 в стволе нет. Номер уступлен по
правилу соседей (номер уступает позже пришедший): 10 занят `check-10-owner-escalation…`,
11 — их проверкой.
...
# Корень-переменная над `.claude/`: `$d01/.claude/`, `${WS}/.claude/`.
VAR_CLAUDE = r'\$\{?[A-Za-z_][A-Za-z0-9_]*\}?/+[^"\'\s]*\.claude/'
# СОЗДАЮЩИЙ АКТ над таким путём — цель перенаправления либо команда, заводящая
# запись. Простое чтение (`RULES_DIR="$WS/.claude/rules"`, `[ -f … ]`,
# `bash "$CLAUDE_PROJECT_DIR/.claude/hooks/…"`) актом не является.
CREATING_ACT = re.compile(
    r'(?:>>?\s*"?' + VAR_CLAUDE + r')'
    r'|(?:\b(?:mkdir|cp|mv|ln|install|rsync)\b[^\n]*?' + VAR_CLAUDE + r')')
# Строка-комментарий целиком: отбрасывается до разбора.
...
def is_apparatus(rel, text):
    """Файл ИЗГОТАВЛИВАЕТ копию корпуса. Четыре барьера — см. шапку."""
    if rel.endswith('.md'):                      # барьер формата
        return False
    if not text.startswith('#!'):                # барьер исполнимости
        return False
    code = COMMENT_LINE.sub('', text)            # барьер исполняемой части
    return bool(CREATING_ACT.search(code))       # барьер создающего акта
...
def main():
    ...
    by_marker = 0
    exempt_dangling = 0   # ЦЕНА освобождения: снятые координаты, которых в корпусе НЕТ
    exempt_live = 0
    exempt_dead_names = set()

    def exists(n):
        return os.path.isfile(os.path.join(base, RULES, n + '.md'))
        ...
        for m in COORD.finditer(text):
            ...
            if app:
                if exists(name):
                    ...
                    exempt_dangling += 1
                    exempt_dead_names.add(name)
                continue
            coords.append((rel, text[:m.start()].count('\n') + 1, name))

    names = sorted({n for _, _, n in coords})
    print('корень обхода: %s' % base)
    print('осмотрено отслеживаемых файлов: %d (архив .claude/backup/ исключён как объявленный);'
          ' координат живых %d, различных имён %d; снято маркером `@` %d'
          % (walked, len(coords), len(names), by_marker))
    print('ЦЕНА РОЛИ АППАРАТУРЫ — числом, а не словом: файлов-аппаратуры %d, снято ими координат'
          ' %d, из них ВИСЯЧИХ %d (различных имён %d: %s), резолвящихся %d'
          % (len(apparatus_files), exempt_dangling + exempt_live, exempt_dangling,
             len(exempt_dead_names), ', '.join(sorted(exempt_dead_names)) or '—', exempt_live))

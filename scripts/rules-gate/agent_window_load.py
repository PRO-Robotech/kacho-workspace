...
доля корпуса, доезжающая одному, 33,5 %; медиана 49 585. Величина, которую судит
...
ЧТО ИМЕННО НЕ БЛОКИРУЕТ — ПОРОГ, А НЕ ПРОВЕРКА. Прежняя редакция этой шапки
говорила «считает и не блокирует», и это было ложью о собственном поведении:
без порога проверка выходит единицей на двух осях РАЗБОРА — скил назван без
переходника и исполнитель без правил без объявления. Блокирует РАЗБОР; не
блокирует ВЕЛИЧИНА, пока порог не задан.

ВЕРДИКТ — В КОДЕ ВЫХОДА: 0 молчит; 1 находка разбора (а при заданном пороге — и
превышение величины); 2 обход беспредметен либо усечён.
"""
import glob
import os
import statistics
...
def skills_of(path):
    ...
    НЕЧИТАЕМОЕ ОПРЕДЕЛЕНИЕ ВОЗВРАЩАЕТ None, А НЕ БРОСАЕТ. Прежняя редакция
    ловила только `OSError`, и определение не в UTF-8 роняло прогон
    трассировкой. Цена хуже самого падения: «не выполнилось» приходило под кодом
    1, а `inject.sh` засчитывает код 1 в `proof_find` — сорванный прогон
    становился неотличим от законной находки, и доказанность «умеет находить»
    удовлетворялась падением. Перепись при этом не печаталась вовсе.
    """
    try:
...
def main():
    ...
    for a in agents:
        ...
        own = chars(a)
        sk = skills_of(a)
        # ОБА ВОПРОСА ЗАДАНЫ ДО РАЗБОРА: любое `None` означает «не прочитано», и
        # такой агент не считается ни пустым, ни загруженным — он роняет вердикт
        # кодом 2 ниже. Тихо пропустить его нельзя: максимум мог остаться именно
        # в нём.
    ...
    # ВТОРАЯ ПОЛОВИНА ПАРЫ: объявление, которому нечего исключать, — находка.
    # Запись `LAWFUL_EMPTY` переживает свой предмет молча: агент снят, строка
    # осталась, и следующий пустой агент под тем же именем получил бы штатность
    # по наследству. Имена берутся из ДЕРЕВА, а не из перечня.
    seen = {n for _, n, _, _ in rows} | set(unread)
    stale = sorted(n for n in LAWFUL_EMPTY if n not in seen)
    ...
    print('порог: %s' % ('%d (задан средой RULES_GATE_AGENT_WINDOW_MAX)' % limit
                         if limit else 'НЕ ЗАДАН — ВЕЛИЧИНА не блокирует; разбор блокирует '
                                       'по-прежнему. Выводить порог сегодня не из чего'))
    ...
    for n in stale:
        rc = 1
        print('КРАСНОЕ объявление штатной пустоты `%s` пережило свой предмет — агента '
              'с таким именем в дереве нет; запись обязана быть снята вместе с ним' % n)
    if unresolved:
        rc = 1
        for x in unresolved:

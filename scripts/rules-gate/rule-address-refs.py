...
#
# НАБЛЮДАЕМОЕ СВОЙСТВО РОЛИ. Производитель фикстуры отличается от прочей
# оснастки одним ДЕЙСТВИЕМ: он САМ СОЗДАЁТ дерево `.claude/` под корнем, который
# вычислил, — `mkdir -p "$d/.claude"`, `> "$d/.claude/rules/<проба>.md"`,
# `cp -a "$WS/.claude/rules" "$dir/.claude/"`. Обычный файл оснастки `.claude/`
# только ЧИТАЕТ и АДРЕСУЕТ: `RULES_DIR="$WS/.claude/rules"`, `[ -f … ]`,
...
# который он же и материализует. Замер на дереве 02b3e6f2: создающий акт есть у
# 12 файлов, они держат 44 из 359 висячих координат; из исключения по этому
# признаку не выпадает НИ ОДИН файл, исключавшийся именем (20 файлов `inject*`
# и `prove.sh` создающего акта не содержат — и висячих координат у них 0, то
# есть исключение им было не нужно), а прибавляется ровно тот производитель,
# из-за которого гейт покраснел. Предикат замера выписан в возврате полосы.
#
# ГРАНИЦА ПРИЗНАКА НАЗВАНА, А НЕ ЗАМОЛЧАНА. Совпадение требует разделителя пути
# ПЕРЕД `.claude` — то есть корня, отличного от корня репозитория. Производитель,
# который сперва зайдёт `cd` в своё дерево и напишет `> .claude/rules/<проба>.md`
# без корня, признаком не покрыт: его координата неотличима от координаты
# корпуса, и различить их текстом нельзя в принципе. Исход для такого — тот же,
# что выбрала соседняя полоса для самой фикстуры: СОБИРАТЬ координату, а не
# выписывать её адресом правила.
#
...
def builds_fixture_tree(root, p):
    ...
    Читается ИСПОЛНЯЕМАЯ часть, а не текст: строка-комментарий отбрасывается до
    сверки. Иначе создающий акт, ПРОЦИТИРОВАННЫЙ в пояснении, давал бы файлу роль
    производителя — и этот самый распознаватель, объясняющий признак примерами,
    исключил бы СЕБЯ из собственного обхода. Слепая зона тем опаснее, что она
    ровно там, где её никто не ищет (`.claude/rules/testing.md`, gate-reads-code-not-text).
    """
    try:
...
def main():
    ...
    print(f"[CENSUS] check-10: файлов оснастки {files_read} "
          ...
          f"не перечислен); строк {lines_read}; "

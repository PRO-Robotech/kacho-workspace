...
#   (1) формы взяты ТОЛЬКО ОБОЛОЧЕЧНЫЕ — перенаправление, `mkdir`, `cp`-семейство.
#       Производитель на python, который делает то же самое ЯВНЫМ корнем через
#       `open(os.path.join(d, ".claude", …), "w")` или `pathlib.Path(d)/".claude"`,
#       признаком не опознан, хотя роль у него та же;
...
# ДОМА СУДИМОГО: корпус и определения агентов. Роль производителя им не выдаётся
# НИКОГДА — см. барьер 2 в `builds_fixture_tree`.
JUDGED_HOMES = (".claude/rules/", ".claude/agents/")
...
def builds_fixture_tree(root, p):
    ...
    2. СУДИМОЕ НЕ ИЗЫМАЕТ СЕБЯ ИЗ СУДА. Даже со shebang'ом файл из `.claude/rules/`
       или `.claude/agents/` роли не получает. Барьер отдельный, потому что
       предмет у него отдельный: перепись существует, чтобы судить ИМЕННО эти
       два дома, и право на самоисключение обесценило бы её целиком.
...
def main():
    ...
    # выдать». Число судимых домов — рядом: их самоисключение невозможно барьером.
    runnable = [p for p in area if not p.startswith(JUDGED_HOMES)
                and _starts_with_shebang(root, p)]
    judged_homes = [p for p in area if p.startswith(JUDGED_HOMES)]
    ...
    print(f"[CENSUS] check-10: файлов оснастки {files_read} "
          ...
          f"не перечислен); роль применима к {len(runnable)} исполняемым файлам, "
          f"домам судимого ({len(judged_homes)} файлов корпуса и агентов) — никогда; "
          f"строк {lines_read}; "

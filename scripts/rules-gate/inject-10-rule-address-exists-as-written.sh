...
# i10_role_file <каталог> <имя файла> <первая строка> <префикс перед актом>
...
I10_SCRIPT=zz-proba-roli-proizvoditelya.sh
I10_MARKUP=zz-proba-roli-proizvoditelya.md
...
i10_role_file "$d" "$I10_SCRIPT" '#!/usr/bin/env bash' '# '
capture "$d" "$C10"
...
i10_role_file "$d" "$I10_SCRIPT" '# пояснение, а не запуск' ''
capture "$d" "$C10"
...
d="$(sandbox i10_markup_shebang)"
i10_role_file "$d" "$I10_MARKUP" '#!/usr/bin/env bash' ''
capture "$d" "$C10"
assert_code 1 "ДЕФЕКТ: разметка со строкой запуска скриптом не становится — роли нет, адрес краснеет"

d="$(sandbox i10_script_shebang)"; b="$(sandbox_digest "$d")"
i10_role_file "$d" "$I10_SCRIPT" '#!/usr/bin/env bash' ''

...
# ИМЯ ФАЙЛА — СО СТРОЧНОЙ, И ЭТО ИЗМЕРЕННЫЙ КОМПРОМИСС, А НЕ НЕДОСМОТР.
# Расширение до `[A-Za-z0-9]` пробовалось 2026-09-20 и ОТОЗВАНО: резолв идёт ПО
# БАЗОВОМУ ИМЕНИ, а в архиве лежит `README.md` — и две ссылки на ЧУЖОЙ README
# (набора гейтов) немедленно стали резолвиться о архивный: гейт зеленел о том,
# чего не проверял. Цена узкого шаблона названа вслух и ПЕЧАТАЕТСЯ переписью: файл
# корпуса с заглавной первой буквой НЕАДРЕСУЕМ — ссылка на его раздел не считается
# адресом и потому не может ни резолвиться, ни висеть. Живых таких ссылок 0 (замер
# 2026-09-20), но молчать нельзя: без строки переписи «ВИСИТ 0» неотличимо от
# «часть дерева не читали».
...
def corpus_names():
    """Базовые имена корпуса и архива — корпус старше при совпадении имени."""
    names = []
    for f in sorted(glob.glob(RULES + '/*.md')) + sorted(glob.glob(ARCHIVE + '/*.md')):
        b = os.path.basename(f)
        if b not in names:
...
def main():
    ...
    blind = [b for b in corpus_names() if not re.match(r'[a-z0-9]', b)]
    if blind:
        print('НЕАДРЕСУЕМЫХ файлов (имя не со строчной — ссылка на их раздел не считается '
              'адресом вовсе): %d — %s' % (len(blind), ', '.join(blind)))

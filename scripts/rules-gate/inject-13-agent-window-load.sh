...
# ── ось G1: ОДНО определение не в UTF-8 — ОТКАЗ кодом 2, а не трассировка ──
# Возврат приёмки: прежняя редакция ловила только `OSError`, и не-UTF-8 роняло
# прогон наружу. Цена была хуже падения: «не выполнилось» приходило под кодом 1,
# а `inject.sh` засчитывает код 1 в `proof_find` — сорванный прогон становился
# неотличим от законной находки. Ось закрепляет именно КОД и ИМЯ в переписи.
d="$(sandbox i13_badenc)"
_V="$(cd "$d" && ls .claude/agents/*.md | head -1)"
printf '\xff\xfe' >> "$d/$_V"
capture "$d" "$C13"
assert_code 2 "ОТКАЗ: одно определение не в UTF-8 — код 2, а не трассировка под кодом 1"
assert_says "не прочитано определений 1" "  ...и непрочитанное названо числом"
assert_says "перепись усечена" "  ...и сказано, почему вердикт беспредметен"
unset -v _V

# ── ось G2: ВСЕ определения нечитаемы — тот же отказ, не «агентов ноль» ────
d="$(sandbox i13_allbad)"
for _f in "$d"/.claude/agents/*.md; do printf '\xff\xfe' >> "$_f"; done
unset -v _f
capture "$d" "$C13"
assert_code 2 "ОТКАЗ: ни одно определение не прочитано — вердикт беспредметен"

# ── ось H: объявление штатной пустоты ПЕРЕЖИЛО свой предмет ───────────────
# Вторая половина пары к оси E: перечень штатно пустых обязан истекать сам.
d="$(sandbox i13_stale)"
rm -f "$d/.claude/agents/client-simulator.md"
capture "$d" "$C13"
assert_code 1 "ДЕФЕКТ: запись штатной пустоты без агента — краснеет"
assert_says "объявлений без предмета 1" "  ...и она посчитана отдельно от штатных"
assert_says "пережило свой предмет" "  ...и названа причиной, а не симптомом"
